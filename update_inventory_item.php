    ...
    try {
        ...
        if ($update_image) {
            ...
            if ($stmt->execute()) {
                ...
                logActivity($conn, 'update', 'INVENTORY_ITEM_UPDATE',
                    "Inventory item #{$id} '{$product_name}' updated (with new image) by {$_actor}",
                    ['item_id' => $id, 'name' => $product_name, 'price' => $price, 'stock' => $stock, 'updated_by' => $_actor]
                );
            } else {
            ...
            if ($stmt->execute()) {
                ...
                $response['message'] = 'Item updated successfully!';
                logActivity($conn, 'update', 'INVENTORY_ITEM_UPDATE',
                    "Inventory item #{$id} '{$product_name}' updated by {$_actor}",
                    ['item_id' => $id, 'name' => $product_name, 'price' => $price, 'stock' => $stock, 'updated_by' => $_actor]
                );
