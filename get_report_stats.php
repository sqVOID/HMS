...
// Log report filter/view
$_rs_first = trim($_SESSION['first_name'] ?? '');
$_rs_last  = trim($_SESSION['last_name'] ?? '');
$_rs_user  = ($_rs_first !== '' || $_rs_last !== '') ? trim($_rs_first . ' ' . $_rs_last) : trim($_SESSION['username'] ?? 'Unknown');
logActivity($conn, 'report', 'REPORT_FILTER',
    "Report stats viewed/filtered by {$_rs_user} — range: {$selectedRangeKey}" . ($selectedRangeKey === 'custom' ? " ({$customStart} to {$customEnd})" : ''),
    ['range' => $selectedRangeKey, 'start' => $customStart, 'end' => $customEnd, 'viewed_by' => $_rs_user]
);
