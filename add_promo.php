...
try {
    $stmt = $conn->prepare("
        INSERT INTO promos (title, description, room_size, bed_type, guest_capacity, facilities, price_12hrs, price_24hrs, image_path)
        VALUES (:title, :description, :room_size, :bed_type, :guest_capacity, :facilities, :price_12hrs, :price_24hrs, :image_path)
    ");
    ...
    $stmt->execute();
    $newPromoId = (int) $conn->lastInsertId();

    logActivity($conn, 'promo', 'PROMO_CREATE',
        "Promo '{$title}' (12h: ₱" . number_format($price12, 2) . ", 24h: ₱" . number_format($price24, 2) . ") created by {$_actor}",
        [
            'promo_id'    => $newPromoId,
            'title'       => $title,
            'price_12hrs' => $price12,
    ...
    logError($conn, 'promo', 'PROMO_CREATE', "Failed to add promo '{$title}': " . $e->getMessage(), [
        'title'      => $title,
        'created_by' => $_actor,
        'error'      => $e->getMessage()
    ]);
