...
try {
    $stmt = $conn->prepare("SELECT title, image_path FROM promos WHERE id = :id");
    ...
    $promoTitle = trim($promo['title'] ?? '') ?: 'Unknown Promo';
    ...
    logActivity($conn, 'delete', 'PROMO_DELETE',
        "Promo '{$promoTitle}' (ID: #{$promoId}) deleted by {$_actor}",
        ['promo_id' => $promoId, 'title' => $promoTitle, 'deleted_by' => $_actor]
    );
