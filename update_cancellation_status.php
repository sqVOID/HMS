...
try {
    ...
    logActivity($conn, 'update', 'CANCELLATION_STATUS_UPDATE',
        "Cancellation request for booking #{$bookingId} {$status} by {$_actor}",
        ['booking_id' => $bookingId, 'status' => $status, 'updated_by' => $_actor]
    );
