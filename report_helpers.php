...
if (!function_exists('ensureReportFinancialColumns')) {
    ...
    function ensureReportFinancialColumns(PDO $conn): void
    {
        try {
            ...
            if ($hasReports) {
                $reportColumns = [
                    'second_guest_name' => "ALTER TABLE reports ADD COLUMN second_guest_name VARCHAR(255) NULL DEFAULT NULL",
                    'additional_guest_names' => "ALTER TABLE reports ADD COLUMN additional_guest_names TEXT NULL DEFAULT NULL",
    ...
    if (!function_exists('ensureNewDepositColumns')) {
        /**
         * Ensure bookings/reports have generic refundable-deposit columns (new_deposit_*).
         */
        function ensureNewDepositColumns(PDO $conn): void
        {
            $columns = [
    ...
    if (!function_exists('applySlipperStatusToNewDeposit')) {
        /**
         * When slippers are returned, clear held new_deposit.
         * When missing, forfeit new_deposit into missing_items_fees.
         *
         * @return array{new_deposit: float, new_deposit_cash: float, new_deposit_g_cash: float, new_deposit_maya: float, new_deposit_instapay: float, new_deposit_online_banking: float, new_deposit_airbnb: float, new_deposit_details: ?string, new_deposit_gcash_ref: ?string, new_deposit_maya_ref: ?string, new_deposit_instapay_ref: ?string, new_deposit_online_banking_ref: ?string, new_deposit_airbnb_ref: ?string, missing_items_fees: float, missing_items_list: ?string}
         */
        function applySlipperStatusToNewDeposit(
            string $slipperStatus,
            array $newDeposit,
            float $missingItemsFees,
            ?string $missingItemsList,
            int $additionalSlippers = 0
        ): array {
            $totalHeld = floatval($newDeposit['new_deposit'] ?? 0);
            if ($totalHeld <= 0) {
                $totalHeld = floatval($newDeposit['new_deposit_cash'] ?? 0)
                    + floatval($newDeposit['new_deposit_g_cash'] ?? 0)
                    + floatval($newDeposit['new_deposit_maya'] ?? 0)
                    + floatval($newDeposit['new_deposit_instapay'] ?? 0)
                    + floatval($newDeposit['new_deposit_online_banking'] ?? 0)
                    + floatval($newDeposit['new_deposit_airbnb'] ?? 0);
            }

            if ($slipperStatus === 'missing' && $totalHeld > 0) {
                $missingItemsFees += $totalHeld;
                $items = [];
                if ($missingItemsList && $missingItemsList !== 'null') {
                    $decoded = json_decode($missingItemsList, true);
                    if (is_array($decoded)) {
                        $items = array_values(array_filter($decoded, static function ($item): bool {
                            if (!is_array($item)) {
                                return false;
                            }
                            return !isSlipperForfeitureMissingItemName((string) ($item['name'] ?? ''));
                        }));
                    }
                }

                $slipperCount = $additionalSlippers > 0
                    ? $additionalSlippers
                    : max(1, (int) round($totalHeld / 100));

                $items[] = [
                    'name' => formatSlipperMissingItemLabel($slipperCount),
                    'price' => $totalHeld,
                ];
                $missingItemsList = json_encode($items);

                // Stash per-method forfeiture for checkout payment history (checked_out_at timestamp).
                $newDeposit['forfeited_new_deposit_cash'] = floatval($newDeposit['new_deposit_cash'] ?? 0);
                $newDeposit['forfeited_new_deposit_g_cash'] = floatval($newDeposit['new_deposit_g_cash'] ?? 0);
                $newDeposit['forfeited_new_deposit_maya'] = floatval($newDeposit['new_deposit_maya'] ?? 0);
                $newDeposit['forfeited_new_deposit_instapay'] = floatval($newDeposit['new_deposit_instapay'] ?? 0);
                $newDeposit['forfeited_new_deposit_online_banking'] = floatval($newDeposit['new_deposit_online_banking'] ?? 0);
                $newDeposit['forfeited_new_deposit_airbnb'] = floatval($newDeposit['new_deposit_airbnb'] ?? 0);
            }

            if ($slipperStatus === 'returned') {
                $newDeposit['forfeited_new_deposit_cash'] = 0;
                $newDeposit['forfeited_new_deposit_g_cash'] = 0;
                $newDeposit['forfeited_new_deposit_maya'] = 0;
                $newDeposit['forfeited_new_deposit_instapay'] = 0;
                $newDeposit['forfeited_new_deposit_online_banking'] = 0;
                $newDeposit['forfeited_new_deposit_airbnb'] = 0;
            }

            if ($slipperStatus === 'returned' || $slipperStatus === 'missing') {
                $newDeposit['new_deposit'] = 0;
                $newDeposit['new_deposit_cash'] = 0;
                $newDeposit['new_deposit_g_cash'] = 0;
                $newDeposit['new_deposit_maya'] = 0;
                $newDeposit['new_deposit_instapay'] = 0;
                $newDeposit['new_deposit_online_banking'] = 0;
                $newDeposit['new_deposit_airbnb'] = 0;
                $newDeposit['new_deposit_details'] = null;
                $newDeposit['new_deposit_gcash_ref'] = null;
                $newDeposit['new_deposit_maya_ref'] = null;
                $newDeposit['new_deposit_instapay_ref'] = null;
                $newDeposit['new_deposit_online_banking_ref'] = null;
                $newDeposit['new_deposit_airbnb_ref'] = null;
            }

            $newDeposit['missing_items_fees'] = $missingItemsFees;
            $newDeposit['missing_items_list'] = $missingItemsList;

            return $newDeposit;
        }
    }

    if (!function_exists('formatSlipperMissingItemLabel')) {
        function formatSlipperMissingItemLabel(int $count): string
        {
            $count = max(1, $count);
            return $count === 1
                ? '1 Slipper (Missing)'
                : "{$count} Slippers (Missing)";
        }
    }

    if (!function_exists('resolveSlipperMissingItemLabel')) {
        function resolveSlipperMissingItemLabel(array $payment, float $price = 0.0): string
        {
            $count = intval($payment['additional_slippers'] ?? 0);
            if ($count <= 0 && $price > 0) {
                $count = max(1, (int) round($price / 100));
            }
            return formatSlipperMissingItemLabel($count);
        }
    }

    if (!function_exists('isSlipperForfeitureMissingItemName')) {
        function isSlipperForfeitureMissingItemName(string $name): bool
        {
            $normalized = strtolower(trim($name));
            if ($normalized === '') {
                return false;
            }
            if ($normalized === 'new deposit (forfeited)') {
                return true;
            }
            if (str_contains($normalized, 'new deposit') && str_contains($normalized, 'forfeit')) {
                return true;
            }
            return str_contains($normalized, 'slipper') && str_contains($normalized, 'missing');
        }
    }

    if (!function_exists('normalizeMissingItemNameForDisplay')) {
        function normalizeMissingItemNameForDisplay(string $name, array $payment, float $price = 0.0): string
        {
            if (isSlipperForfeitureMissingItemName($name)) {
                return resolveSlipperMissingItemLabel($payment, $price);
            }
            return $name;
        }
    }

    if (!function_exists('formatMissingItemReportLine')) {
        function formatMissingItemReportLine(array $item, array $payment): string
        {
            $rawName = (string) ($item['name'] ?? 'Item');
            $price = floatval($item['price'] ?? 0);
            $name = normalizeMissingItemNameForDisplay($rawName, $payment, $price);

            if (isSlipperForfeitureMissingItemName($rawName) || isSlipperForfeitureMissingItemName($name)) {
                return $name . ' = ' . number_format($price, 2);
            }
    ...
    if (!function_exists('buildMissingItemsReportDisplay')) {
        function buildMissingItemsReportDisplay(array $payment, bool $includeCheckoutFees): string
        {
            if (!$includeCheckoutFees) {
                return '—';
            }

            $missingItemsFees = floatval($payment['missing_items_fees'] ?? 0);
            if (!empty($payment['missing_items_list'])) {
                $mItems = json_decode((string) $payment['missing_items_list'], true);
                if (is_array($mItems) && count($mItems) > 0) {
                    ...
                    foreach ($mItems as $m) {
                        ...
                        $mStrings[] = formatMissingItemReportLine($m, $payment);
                    }
                    if (!empty($mStrings)) {
                        return implode(', ', $mStrings);
                    }
                }
            }
            ...
            if ($missingItemsFees > 0) {
                ...
                if ($slipperStatus === 'missing' && $slipperCount > 0) {
                    return formatSlipperMissingItemLabel($slipperCount) . ' = ' . number_format($missingItemsFees, 2);
                }
                return 'Missing Items (' . number_format($missingItemsFees, 2) . ')';
            }
    ...
    if (!function_exists('getHeldNewDepositAmountsByMethod')) {
        /**
         * Refundable new_deposit held per payment method — excluded from room revenue until forfeited at checkout.
         *
         * @return array{cash: float, gcash: float, maya: float, instapay: float, online_banking: float, airbnb: float}
         */
        function getHeldNewDepositAmountsByMethod(array $payment): array
        {
            $status = strtolower(trim((string) ($payment['slipper_status'] ?? '')));
            if (in_array($status, ['returned', 'missing'], true)) {
                return [
                    'cash' => 0.0,
                    'gcash' => 0.0,
                    'maya' => 0.0,
            ...
            $amounts = [
                'cash' => floatval($payment['new_deposit_cash'] ?? 0),
                'gcash' => floatval($payment['new_deposit_g_cash'] ?? 0),
                'maya' => floatval($payment['new_deposit_maya'] ?? 0),
                'instapay' => floatval($payment['new_deposit_instapay'] ?? 0),
                'online_banking' => floatval($payment['new_deposit_online_banking'] ?? 0),
                'airbnb' => floatval($payment['new_deposit_airbnb'] ?? 0),
            ];
            $heldTotal = array_sum($amounts);

            // Legacy bookings: slipper deposit was stored inside deposit_* before new_deposit_* existed.
            if ($heldTotal <= 0.005) {
                $count = intval($payment['additional_slippers'] ?? 0);
                if ($count > 0 && ($status === 'added' || $status === '' || $status === 'none')) {
                    $legacyHeld = $count * 100;
                    $depositTotals = [
                        'cash' => floatval($payment['deposit_cash'] ?? 0),
                        'gcash' => floatval($payment['deposit_g_cash'] ?? 0),
                        'maya' => floatval($payment['deposit_maya'] ?? 0),
                        'instapay' => floatval($payment['deposit_instapay'] ?? 0),
                        'online_banking' => floatval($payment['deposit_online_banking'] ?? 0),
                        'airbnb' => floatval($payment['deposit_airbnb'] ?? 0),
                    ];
                    $depSum = array_sum($depositTotals);
                    if ($depSum > 0.005 && $legacyHeld > 0.005) {
                        $subtract = min($legacyHeld, $depSum);
                        foreach ($depositTotals as $key => $val) {
                            $amounts[$key] = $subtract * ($val / $depSum);
                        }
                    }
                }
            }
    ...
    if (!function_exists('appendForfeitedNewDepositToPaymentHistory')) {
        /**
         * When slipper/new deposit is forfeited (missing), append per-method amounts to
         * payment_amount_*_history aligned with checked_out_at in payment_date_time.
         */
        function appendForfeitedNewDepositToPaymentHistory(PDO $conn, array $booking, string $checkedOutAt): array
        {
            $slipperStatus = strtolower(trim((string) ($booking['slipper_status'] ?? '')));
            if ($slipperStatus !== 'missing') {
                return $booking;
            }

            $methodConfigs = [
                'forfeited_new_deposit_cash' => 'payment_amount_cash_history',
                'forfeited_new_deposit_g_cash' => 'payment_amount_g_cash_history',
                'forfeited_new_deposit_maya' => 'payment_amount_maya_history',
                'forfeited_new_deposit_instapay' => 'payment_amount_instapay_history',
                'forfeited_new_deposit_online_banking' => 'payment_amount_online_banking_history',
                'forfeited_new_deposit_airbnb' => 'payment_amount_airbnb_history',
            ];

            $forfeitedTotal = 0.0;
            foreach (array_keys($methodConfigs) as $forfeitedCol) {
                $forfeitedTotal += floatval($booking[$forfeitedCol] ?? 0);
            }
            if ($forfeitedTotal <= 0.005) {
                return $booking;
            }

            $histColumns = array_values($methodConfigs);
            foreach (['bookings', 'reports'] as $table) {
                foreach ($histColumns as $colName) {
                    try {
                        $chk = $conn->query("SHOW COLUMNS FROM {$table} LIKE '{$colName}'");
                        if ($chk && $chk->rowCount() === 0) {
                            $conn->exec("ALTER TABLE {$table} ADD COLUMN {$colName} TEXT NULL DEFAULT NULL");
                        }
                    } catch (PDOException $e) {
                        // best-effort
                    }
            ...
            $existingPaymentDateTime = trim((string) ($booking['payment_date_time'] ?? ''));
            $updatedPaymentDateTime = $existingPaymentDateTime !== ''
                ? ($existingPaymentDateTime . '|' . $checkedOutAt)
                : $checkedOutAt;

            $updatedHistories = [];
            foreach ($methodConfigs as $forfeitedCol => $histCol) {
                $delta = max(0, floatval($booking[$forfeitedCol] ?? 0));
                $existingHist = trim((string) ($booking[$histCol] ?? ''));
                $entry = number_format($delta, 2, '.', '');
                $updatedHistories[$histCol] = $existingHist !== '' ? ($existingHist . '|' . $entry) : $entry;
                $booking[$histCol] = $updatedHistories[$histCol];
            }

            $booking['payment_date_time'] = $updatedPaymentDateTime;

            $bookingIdValue = trim((string) ($booking['booking_id'] ?? ''));
            $bookingDbId = intval($booking['id'] ?? 0);
            ...
            try {
                $bookingUpdate = $conn->prepare("
                    ...
                    SET payment_date_time = :payment_date_time,
                        ...
                        forfeited_new_deposit_cash = 0,
                        forfeited_new_deposit_g_cash = 0,
                        forfeited_new_deposit_maya = 0,
                        forfeited_new_deposit_instapay = 0,
                        forfeited_new_deposit_online_banking = 0,
                        forfeited_new_deposit_airbnb = 0
                    WHERE id = :id
                ");
                $bookingUpdate->execute([
                    ':payment_date_time' => $updatedPaymentDateTime,
                    ':cash_hist' => $updatedHistories['payment_amount_cash_history'],
                    ':gcash_hist' => $updatedHistories['payment_amount_g_cash_history'],
                    ':maya_hist' => $updatedHistories['payment_amount_maya_history'],
                    ':instapay_hist' => $updatedHistories['payment_amount_instapay_history'],
                    ':online_banking_hist' => $updatedHistories['payment_amount_online_banking_history'],
                    ':airbnb_hist' => $updatedHistories['payment_amount_airbnb_history'],
                    ':id' => $bookingDbId,
                ]);
            } catch (PDOException $e) {
                error_log('appendForfeitedNewDepositToPaymentHistory bookings: ' . $e->getMessage());
            }

            if ($bookingIdValue !== '') {
                try {
                    $reportsUpdate = $conn->prepare("
                        UPDATE reports
                        SET payment_date_time = :payment_date_time,
                            payment_amount_cash_history = :cash_hist,
                            payment_amount_g_cash_history = :gcash_hist,
                    ...
                    error_log('appendForfeitedNewDepositToPaymentHistory reports: ' . $e->getMessage());
                }
            }

            foreach (array_keys($methodConfigs) as $forfeitedCol) {
                $booking[$forfeitedCol] = 0;
            }
    ...
    /**
     * Format one discount_amount_history entry: "TYPE amount:YYYY-MM-DD HH:MM:SS"
     */
    function formatDiscountHistoryEntry(string $type, float $amount, string $timestamp): string
    {
        $type = trim($type);
        $prefix = $type !== '' ? $type . ' ' : '';
        return $prefix . number_format($amount, 2, '.', '') . ':' . $timestamp;
    }

    /**
     * Parse one discount_amount_history entry (supports typed and legacy formats).
     */
    function parseDiscountHistoryEntry(string $entry): array
    {
        $entry = trim($entry);
        $result = ['type' => '', 'amount' => 0.0, 'datetime' => ''];
        if ($entry === '') {
            return $result;
        }

        if (preg_match('/^(SC\/PWD|LGP|LPG|VIP|Long Term)\s+(-?\d+(?:\.\d+)?):(.+)$/i', $entry, $matches)) {
            $rawType = strtoupper(trim($matches[1]));
            if ($rawType === 'SC/PWD' || strpos($rawType, 'SC') !== false) {
                $type = 'SC/PWD';
            } elseif ($rawType === 'LGP' || $rawType === 'LPG') {
                $type = 'LGP';
            } elseif ($rawType === 'LONG TERM' || preg_match('/^LONG\s*TERM$/i', $matches[1])) {
                $type = 'Long Term';
            } else {
                $type = 'VIP';
            }
        ...
        return [
            'type' => '',
            'amount' => floatval($parts[0] ?? 0),
            'datetime' => trim($parts[1] ?? ''),
        ];
    }

    /**
     * Parse full discount_amount_history string into structured entries.
     */
    function parseDiscountAmountHistory(string $historyRaw): array
    {
        $entries = [];
        $historyRaw = trim($historyRaw);
        if ($historyRaw === '') {
            return $entries;
        }

        foreach (explode('|', $historyRaw) as $entry) {
            $parsed = parseDiscountHistoryEntry($entry);
            if (abs($parsed['amount']) > 0.001) {
                $entries[] = $parsed;
            }
        }
    ...
    /**
     * Build initial discount_amount_history from typed discount amounts (SC/PWD, LGP, VIP).
     */
    function buildDiscountAmountHistoryEntries(array $typedAmounts, string $timestamp): string
    {
        $entries = [];
        foreach ($typedAmounts as $type => $amount) {
            $value = floatval($amount);
            if ($value > 0.001) {
                $entries[] = formatDiscountHistoryEntry((string) $type, $value, $timestamp);
            }
        }
    ...
    /**
     * Append incremental typed discount deltas to existing discount_amount_history.
     */
    function appendDiscountHistoryDeltaEntries(?string $existingHistory, array $typedDeltas, string $timestamp): ?string
    {
        $entries = [];
        $existingHistory = trim((string) ($existingHistory ?? ''));
        if ($existingHistory !== '') {
            $entries = explode('|', $existingHistory);
        }
        foreach ($typedDeltas as $type => $delta) {
            $value = floatval($delta);
            if (abs($value) > 0.001) {
                $entries[] = formatDiscountHistoryEntry((string) $type, $value, $timestamp);
            }
        }
        return !empty($entries) ? implode('|', $entries) : null;
    }

    /**
     * Sum of all discount amounts stored on a booking row (SC/PWD + LGP + VIP).
     */
    function getBookingTotalDiscountFromRecord(array $record): float
    {
        $total = 0.0;
        if (!empty($record['discount_enabled'])) {
            $total += floatval($record['discount_amount'] ?? 0);
        }
        if (!empty($record['lgp_discount_enabled'])) {
            $total += floatval($record['lgp_discount_amount'] ?? 0);
        }
        if (!empty($record['vip_discount_enabled'])) {
            $total += floatval($record['vip_discount_amount'] ?? 0);
        }
        if (!empty($record['long_discount_enabled'])) {
            $total += floatval($record['long_discount_amount'] ?? 0);
        }
        // Fallback for rows saved before enabled flags existed
        if ($total <= 0.001) {
            $total = floatval($record['discount_amount'] ?? 0)
                + floatval($record['lgp_discount_amount'] ?? 0)
                + floatval($record['vip_discount_amount'] ?? 0)
                + floatval($record['long_discount_amount'] ?? 0);
        }
...
if (!function_exists('normalizeReservationPaymentExportRows')) {
    ...
    function normalizeReservationPaymentExportRows(array $payment, array $rows): array
    {
        ...
        foreach ($rows as $rowIndex => $row) {
            ...
            // Applies ONLY when downpayment is partial ($dpCash < $depositCash - 0.01).
            if (
                $dpCash > 0
                && $depositCash > 0
                && $dpCash < $depositCash - 0.01
                && strcasecmp($method, 'Cash') === 0
                && abs($amt - $dpCash) < 0.02
            ) {
                $rowDate = paymentExportRowCalendarDate($row);
                $suppressDownpayment = false;
                foreach ($rows as $otherIdx => $other) {
                    if ($otherIdx === $rowIndex) {
                        continue;
                    }
