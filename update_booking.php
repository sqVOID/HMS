...
// ── SESSION GUARD ────────────────────────────────────────────────────────────
// Block the request if the user has no active session or their session was
// terminated externally (e.g. via "Logout All Accounts").
(function () {
    ...
    if (empty($_SESSION['user_id'])) {
        ...
        echo json_encode([
            ...
            'message' => 'Your account has an inactive session. Please logout first then re-login again!'
        ]);
        exit;
    }

    // Check if session was terminated server-side
    if (!empty($_SESSION['session_id'])) {
        global $conn;
        try {
            $sStmt = $conn->prepare("SELECT session_status FROM user_sessions WHERE id = ? LIMIT 1");
            $sStmt->execute([$_SESSION['session_id']]);
            $sStatus = $sStmt->fetchColumn();
            if ($sStatus === 'logged_out') {
                $_SESSION = [];
            ...
            // Session table unavailable — allow request to continue
        }
    }
})();
// ── END SESSION GUARD ────────────────────────────────────────────────────────
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Additional Slippers
    $additional_slippers = isset($_POST['additional_slippers']) ? intval($_POST['additional_slippers']) : 0;
    $slipper_status = isset($_POST['slipper_status']) ? trim($_POST['slipper_status']) : 'added';
    // Validate slipper_status
    if (!in_array($slipper_status, ['added', 'missing', 'returned'])) {
        $slipper_status = 'added';
    }
    $booking_id = $_POST['booking_id'] ?? null;
    // Guest-info fields use CSS uppercase in the UI; persist as real UPPERCASE in DB.
    $guest_name = strtoupper(trim($_POST['guest_name'] ?? ''));
    ...
    $new_deposit = floatval($_POST['new_deposit'] ?? 0);
    $new_deposit_cash = floatval($_POST['new_deposit_cash'] ?? 0);
    $new_deposit_g_cash = floatval($_POST['new_deposit_g_cash'] ?? 0);
    $new_deposit_maya = floatval($_POST['new_deposit_maya'] ?? 0);
    $new_deposit_instapay = floatval($_POST['new_deposit_instapay'] ?? 0);
    $new_deposit_online_banking = floatval($_POST['new_deposit_online_banking'] ?? 0);
    $new_deposit_airbnb = floatval($_POST['new_deposit_airbnb'] ?? 0);
    $new_deposit_details = $_POST['new_deposit_details'] ?? null;
    $new_deposit_gcash_ref = trim($_POST['new_deposit_gcash_ref'] ?? '');
    $new_deposit_maya_ref = trim($_POST['new_deposit_maya_ref'] ?? '');
    $new_deposit_instapay_ref = trim($_POST['new_deposit_instapay_ref'] ?? '');
    $new_deposit_online_banking_ref = trim($_POST['new_deposit_online_banking_ref'] ?? '');
    $new_deposit_airbnb_ref = trim($_POST['new_deposit_airbnb_ref'] ?? '');
    if ($new_deposit <= 0 && ($new_deposit_cash + $new_deposit_g_cash + $new_deposit_maya + $new_deposit_instapay + $new_deposit_online_banking + $new_deposit_airbnb) > 0) {
        $new_deposit = $new_deposit_cash + $new_deposit_g_cash + $new_deposit_maya + $new_deposit_instapay + $new_deposit_online_banking + $new_deposit_airbnb;
    }
    ...
    $second_guest_name = strtoupper(trim($_POST['second_guest_name'] ?? ''));
    $additional_guest_names = strtoupper(trim($_POST['additional_guest_names'] ?? ''));
    $contact_person_name = strtoupper(trim($_POST['contact_person_name'] ?? ''));
    ...
    $vehicle_description = strtoupper(trim($_POST['vehicle_description'] ?? ''));
    ...
    // LGP Discount fields
    $lgp_discount_enabled = isset($_POST['lgp_discount_enabled']) && $_POST['lgp_discount_enabled'] === '1' ? 1 : 0;
    $lgp_discount_amount = floatval($_POST['lgp_discount_amount'] ?? 0);
    $lgp_approver_name = trim($_POST['lgp_approver_name'] ?? '');

    // VIP Discount fields
    $vip_discount_enabled = isset($_POST['vip_discount_enabled']) && $_POST['vip_discount_enabled'] === '1' ? 1 : 0;
    $vip_discount_amount = floatval($_POST['vip_discount_amount'] ?? 0);
    $vip_approver_name = trim($_POST['vip_approver_name'] ?? '');

    // Long Term Discount fields (accept long_discount_* and long_term_discount_*)
    $long_discount_enabled = (
        (isset($_POST['long_discount_enabled']) && $_POST['long_discount_enabled'] === '1')
        || (isset($_POST['long_term_discount_enabled']) && $_POST['long_term_discount_enabled'] === '1')
    ) ? 1 : 0;
    $long_discount_amount = floatval($_POST['long_discount_amount'] ?? $_POST['long_term_discount_amount'] ?? 0);
    $long_discount_percent = floatval($_POST['long_discount_percent'] ?? $_POST['long_term_discount_percent'] ?? 0);

    if (!$discount_enabled) {
        $discount_amount = 0;
    }
    if (!$lgp_discount_enabled) {
        $lgp_discount_amount = 0;
        $lgp_approver_name = '';
    }
    if (!$vip_discount_enabled) {
        $vip_discount_amount = 0;
        $vip_approver_name = '';
    }
    if (!$long_discount_enabled) {
        $long_discount_amount = 0;
        $long_discount_percent = 0;
    }

    $total_discount_amount = ($discount_enabled ? $discount_amount : 0)
        + ($lgp_discount_enabled ? $lgp_discount_amount : 0)
        + ($vip_discount_enabled ? $vip_discount_amount : 0)
        + ($long_discount_enabled ? $long_discount_amount : 0);
    ...
    error_log("lgp_discount_enabled: " . $lgp_discount_enabled);
    error_log("lgp_discount_amount: " . $lgp_discount_amount);
    error_log("lgp_approver_name: " . $lgp_approver_name);
    error_log("vip_discount_enabled: " . $vip_discount_enabled);
    error_log("vip_discount_amount: " . $vip_discount_amount);
    error_log("vip_approver_name: " . $vip_approver_name);
    error_log("long_discount_enabled: " . $long_discount_enabled);
    error_log("long_discount_amount: " . $long_discount_amount);
    error_log("long_discount_percent: " . $long_discount_percent);
    error_log("total_discount_amount: " . $total_discount_amount);
    ...
    // Also load stored duration: Reservation + Regular edit often posts duration=0 after extend/pay,
    // and checkout must not be computed before that duration is restored.
    $existing_extend_hours = 0;
    $existing_extend_minutes = 0;
    $existing_duration = 0;
    $existing_duration_unit = 'hours';
    try {
        ...
        if ($extRow) {
            $existing_duration = intval($extRow['duration'] ?? 0);
            $existing_duration_unit = $extRow['duration_unit'] ?? 'hours';
            if ($withdraw_extension) {
                ...
                if (!empty($boot)) {
                    ...
                    $existing_extend_price = $remAggEarly['price'];
                    $existing_extend_additional_guest = $remAggEarly['eg'];
                } else {
                    $existing_extend_hours = 0;
                    $existing_extend_minutes = 0;
                    $existing_extend_price = 0;
                    $existing_extend_additional_guest = 0;
                }
            } else {
                $existing_extend_hours = intval($extRow['extend_hours'] ?? 0);
                $existing_extend_minutes = intval($extRow['extend_minutes'] ?? 0);
                $existing_extend_price = floatval($extRow['extend_price'] ?? 0);
                $existing_extend_additional_guest = intval($extRow['extend_additional_guest'] ?? 0);
            }
        }
    } catch (Exception $e) {
    ...
    // Restore duration BEFORE checkout math when the modal posts 0 (common after extend → pay → Update).
    if (intval($duration) <= 0 && intval($existing_duration) > 0) {
        $duration = intval($existing_duration);
        $duration_unit = $existing_duration_unit ?: $duration_unit;
        error_log("=== DURATION PRESERVED EARLY ===");
        error_log("POST duration was 0; using stored duration={$duration} unit={$duration_unit}");
    }
    ...
    if ($check_in) {
        ...
        if ($duration_unit === 'day' || $duration_unit === 'days') {
            $baseHours = $duration * 24;
        }
    ...
    try {
        ...
        $requiredColumns = [
            ...
            'additional_guest_names' => "TEXT NULL DEFAULT NULL",
        ...
        $getOriginalStmt = $conn->prepare("
            SELECT b.booking_id, b.room_id, b.duration, b.duration_unit, b.promo, b.paid_status, b.hygiene_kit_used, 
                   ...
                   COALESCE(b.lgp_discount_amount, 0) AS old_lgp_discount_amount,
                   COALESCE(b.vip_discount_amount, 0) AS old_vip_discount_amount,
                   COALESCE(b.long_discount_amount, 0) AS old_long_discount_amount,
                   b.deposit, b.deposit_cash, b.deposit_g_cash, b.deposit_maya,
                   b.deposit_instapay, b.deposit_online_banking, b.deposit_airbnb,
                   b.deposit_gcash_ref, b.deposit_maya_ref, b.deposit_instapay_ref,
                   b.deposit_online_banking_ref, b.deposit_airbnb_ref,
                   COALESCE(b.room_price, 0) AS original_room_price,
                   b.total_amount,
                   COALESCE(b.slipper_status, 'added') AS original_slipper_status,
                   COALESCE(b.new_deposit, 0) AS original_new_deposit,
                   COALESCE(b.new_deposit_cash, 0) AS original_new_deposit_cash,
                   COALESCE(b.new_deposit_g_cash, 0) AS original_new_deposit_g_cash,
                   COALESCE(b.new_deposit_maya, 0) AS original_new_deposit_maya,
                   COALESCE(b.new_deposit_instapay, 0) AS original_new_deposit_instapay,
                   COALESCE(b.new_deposit_online_banking, 0) AS original_new_deposit_online_banking,
                   COALESCE(b.new_deposit_airbnb, 0) AS original_new_deposit_airbnb,
                   b.new_deposit_details AS original_new_deposit_details,
                   b.new_deposit_gcash_ref AS original_new_deposit_gcash_ref,
                   b.new_deposit_maya_ref AS original_new_deposit_maya_ref,
                   b.new_deposit_instapay_ref AS original_new_deposit_instapay_ref,
                   b.new_deposit_online_banking_ref AS original_new_deposit_online_banking_ref,
                   b.new_deposit_airbnb_ref AS original_new_deposit_airbnb_ref,
                   COALESCE(b.forfeited_new_deposit_cash, 0) AS original_forfeited_new_deposit_cash,
                   COALESCE(b.forfeited_new_deposit_g_cash, 0) AS original_forfeited_new_deposit_g_cash,
                   COALESCE(b.forfeited_new_deposit_maya, 0) AS original_forfeited_new_deposit_maya,
                   COALESCE(b.forfeited_new_deposit_instapay, 0) AS original_forfeited_new_deposit_instapay,
                   COALESCE(b.forfeited_new_deposit_online_banking, 0) AS original_forfeited_new_deposit_online_banking,
                   COALESCE(b.forfeited_new_deposit_airbnb, 0) AS original_forfeited_new_deposit_airbnb,
                   b.missing_items_list AS original_missing_items_list,
                   COALESCE(b.missing_items_fees, 0) AS original_missing_items_fees,
                   b.penalty_list AS original_penalty_list,
                   COALESCE(b.penalty_amount, 0) AS original_penalty_amount,
                   b.additional_fees_status AS original_additional_fees_status,
                   b.additional_fees_payment_method AS original_additional_fees_payment_method,
                   b.additional_fees_reference_no AS original_additional_fees_reference_no,
                   b.additional_fees_paid_date AS original_additional_fees_paid_date
            FROM bookings b
        ...
        // Restore downpayment amounts from payment history if history shows a higher original downpayment amount
        // (Prevents scaled down DB columns from losing the guest's recorded payment history)
        $firstValFromHist = function ($histVal) {
            if (empty($histVal))
                return 0.0;
            $parts = explode('|', trim((string) $histVal));
            $first = trim($parts[0] ?? '');
            $val = floatval(preg_replace('/[^0-9.-]+/', '', $first));
            return is_finite($val) ? $val : 0.0;
        };

        if ($originalBooking) {
            $histDpCash = $firstValFromHist($originalBooking['payment_amount_cash_history'] ?? '');
            $histDpGCash = $firstValFromHist($originalBooking['payment_amount_g_cash_history'] ?? '');
            $histDpMaya = $firstValFromHist($originalBooking['payment_amount_maya_history'] ?? '');
            $histDpInstapay = $firstValFromHist($originalBooking['payment_amount_instapay_history'] ?? '');
            $histDpOnlineBanking = $firstValFromHist($originalBooking['payment_amount_online_banking_history'] ?? '');
            $histDpAirbnb = $firstValFromHist($originalBooking['payment_amount_airbnb_history'] ?? '');
            $histDpTotal = $histDpCash + $histDpGCash + $histDpMaya + $histDpInstapay + $histDpOnlineBanking + $histDpAirbnb;

            if ($histDpTotal > 0) {
                if ($downpayment_amount < $histDpTotal)
                    $downpayment_amount = $histDpTotal;
                if ($downpayment_cash < $histDpCash)
                    $downpayment_cash = $histDpCash;
                if ($downpayment_gcash < $histDpGCash)
                    $downpayment_gcash = $histDpGCash;
                if ($downpayment_maya < $histDpMaya)
                    $downpayment_maya = $histDpMaya;
                if ($downpayment_instapay < $histDpInstapay)
                    $downpayment_instapay = $histDpInstapay;
                if ($downpayment_online_banking < $histDpOnlineBanking)
                    $downpayment_online_banking = $histDpOnlineBanking;
                if ($downpayment_airbnb < $histDpAirbnb)
                    $downpayment_airbnb = $histDpAirbnb;
            }
        }
        ...
        // 5. Missing items + penalty fees (same treatment as Additional Guest/Pet).
        // Prefer POST when present (e.g. same-request apply), otherwise use values already
        // saved by update_checkout_fees.php on the booking row.
        $missing_items_fees = isset($_POST['missing_items_fees'])
            ? floatval($_POST['missing_items_fees'])
            : floatval($originalBooking['original_missing_items_fees'] ?? 0);
        $missing_items_list = array_key_exists('missing_items_list', $_POST)
            ? ($_POST['missing_items_list'] ?? null)
            : ($originalBooking['original_missing_items_list'] ?? null);
        $penalty_amount = isset($_POST['penalty_amount'])
            ? floatval($_POST['penalty_amount'])
            : floatval($originalBooking['original_penalty_amount'] ?? 0);
        $penalty_list = array_key_exists('penalty_list', $_POST)
            ? ($_POST['penalty_list'] ?? null)
            : ($originalBooking['original_penalty_list'] ?? null);

        // Preserve any active extension hours and guest extension fees when updating
        $extend_price = isset($existing_extend_price) ? $existing_extend_price : floatval($originalBooking['extend_price'] ?? 0);
        $extend_additional_guest = isset($existing_extend_additional_guest) ? $existing_extend_additional_guest : intval($originalBooking['extend_additional_guest'] ?? 0);
        $extend_guest_charges = $extend_additional_guest * 300;

        // Promo breakfast surplus: computeBookingTotalAmount returns 0 for promo breakfasts
        // because they are "included" in the bundle — but items costing more than ₱150 each
        // carry a surplus that the frontend charges. Compute it here by looking up the
        // actual item prices from the breakfast table (same ₱150 bundle-base the UI uses).
        $promo_breakfast_surplus = 0.0;
        if (!empty($breakfast) && stripos($breakfast, '(Promo)') !== false) {
            // Use frontend-reported value if available (most reliable)
            $breakfast_charges_from_frontend = floatval($_POST['breakfast_charges'] ?? 0);
            if ($breakfast_charges_from_frontend > 0) {
                $promo_breakfast_surplus = $breakfast_charges_from_frontend;
            } else {
                // Fallback: compute surplus from DB prices
                try {
                    $bundleBase = 150.0;
                    $bfItems = explode('|', $breakfast);
                    foreach ($bfItems as $bfItem) {
                        $bfItem = trim($bfItem);
                        if (empty($bfItem) || stripos($bfItem, '(Promo)') === false)
                            continue;
                        // Extract quantity and name — e.g. "2 Bangus Sisig (Promo)"
                        $qty = 1;
                        $itemName = $bfItem;
                        if (preg_match('/^(\d+)\s+(.+)/u', $bfItem, $m)) {
                            $qty = intval($m[1]);
                            $itemName = $m[2];
                        }
                        $itemName = trim(preg_replace('/\s*\(Promo\)\s*$/i', '', $itemName));
                        // Lookup price in breakfast table
                        $bfStmt = $conn->prepare("SELECT price FROM breakfast WHERE UPPER(TRIM(food_name)) = UPPER(TRIM(:name)) LIMIT 1");
                        $bfStmt->execute([':name' => $itemName]);
                        $bfRow = $bfStmt->fetch(PDO::FETCH_ASSOC);
                        if ($bfRow) {
                            $unitPrice = floatval($bfRow['price']);
                            $surplus = max(0, ($unitPrice - $bundleBase) * $qty);
                            $promo_breakfast_surplus += $surplus;
                        }
                    }
                } catch (Exception $e) {
                    error_log("Failed to compute promo breakfast surplus: " . $e->getMessage());
                }
            }
        }
        error_log("Promo breakfast surplus: " . $promo_breakfast_surplus);

        // Full booking amount = room + guests + pets + additional items + extension + missing/penalty + promo breakfast surplus
        // CRITICAL: missing/penalty MUST be included here. Otherwise paying those fees makes
        // total_payments look "overpaid" vs room-only total, which scales down deposits and
        // payment_*_history (e.g. ₱1,490 → ₱1,115.63 after a ₱500 beds sheet fee).
        $full_booking_amount = $room_total + $guest_charges + $pet_charges + $additional_items_total
            + $extend_price + $extend_guest_charges + $missing_items_fees + $penalty_amount + $promo_breakfast_surplus;
        ...
        error_log("Missing items fees: " . $missing_items_fees);
        ...
        $full_booking_amount_after_discount = $full_booking_amount - $total_discount_amount;
        ...
        $originalBookingAmount = $room_total + $guest_charges + $pet_charges - $total_discount_amount;
        ...
        // Preserve actual (unscaled) deposit values for payment history tracking.
        // History must record what the guest actually paid, not scaled-down amounts.
        $actual_deposit_cash = floatval($deposit_cash);
        $actual_deposit_g_cash = floatval($deposit_g_cash);
        $actual_deposit_maya = floatval($deposit_maya);
        $actual_deposit_instapay = floatval($deposit_instapay ?? 0);
        $actual_deposit_online_banking = floatval($deposit_online_banking ?? 0);
        $actual_deposit_airbnb = floatval($deposit_airbnb ?? 0);

        // CRITICAL FIX: Do NOT scale down deposits or downpayments in the DB when total_payments > full_booking_amount_after_discount.
        // Mutating DB deposit/downpayment columns to smaller numbers permanently destroys the guest's recorded payment,
        // which causes spurious Amount Due (e.g. ₱239.00) if the room rate/duration is later edited back to normal.
        if (
            !$withdraw_extension
            && $total_payments >= $full_booking_amount_after_discount
            && $full_booking_amount_after_discount > 0
        ) {
            $amount_due = 0.0;
            $paid_status = 'Paid';
        }
        ...
        // Keep additional fees status in sync with the payment status.
        // If there are pending additional fees, and the booking is now fully settled (Paid),
        // we can mark the additional fees as Paid.
        $additional_fees_status = $originalBooking['original_additional_fees_status'] ?? 'None';
        $additional_fees_payment_method = $originalBooking['original_additional_fees_payment_method'] ?? null;
        $additional_fees_reference_no = $originalBooking['original_additional_fees_reference_no'] ?? null;
        $additional_fees_paid_date = $originalBooking['original_additional_fees_paid_date'] ?? null;

        if ($additional_fees_status === 'Pending' && ($amount_due <= 0.01 || $paid_status === 'Paid')) {
            $additional_fees_status = 'Paid';
            $additional_fees_paid_date = date('Y-m-d H:i:s');

            // Try to detect the payment method from the new payments
            $origDepCash = floatval($originalBooking['deposit_cash'] ?? 0);
            $origDepGCash = floatval($originalBooking['deposit_g_cash'] ?? 0);
            $origDepMaya = floatval($originalBooking['deposit_maya'] ?? 0);
            $origDepInstapay = floatval($originalBooking['deposit_instapay'] ?? 0);
            $origDepOnlineBanking = floatval($originalBooking['deposit_online_banking'] ?? 0);
            $origDepAirbnb = floatval($originalBooking['deposit_airbnb'] ?? 0);

            if ($deposit_cash > $origDepCash) {
                $additional_fees_payment_method = 'Cash';
            } elseif ($deposit_g_cash > $origDepGCash) {
                $additional_fees_payment_method = 'G-cash';
                $additional_fees_reference_no = $deposit_gcash_ref;
            } elseif ($deposit_maya > $origDepMaya) {
                $additional_fees_payment_method = 'Maya';
                $additional_fees_reference_no = $deposit_maya_ref;
            } elseif ($deposit_instapay > $origDepInstapay) {
                $additional_fees_payment_method = 'Instapay';
                $additional_fees_reference_no = $deposit_instapay_ref;
            } elseif ($deposit_online_banking > $origDepOnlineBanking) {
                $additional_fees_payment_method = 'Online Banking';
                $additional_fees_reference_no = $deposit_online_banking_ref;
            } elseif ($deposit_airbnb > $origDepAirbnb) {
                $additional_fees_payment_method = 'Airbnb';
                $additional_fees_reference_no = $deposit_airbnb_ref;
            }
            error_log("✓ SYNC: booking is Paid, setting additional_fees_status to Paid via $additional_fees_payment_method");
        }
        ...
        if ($newPaymentMade) {
            ...
            if (!empty($booking_id_value)) {
                try {
                    ...
                    // Use actual (unscaled) deposit values so history reflects real payments.
                    $deltaCash = max(0, $actual_deposit_cash - $oldCashBase);
                    $deltaGCash = max(0, $actual_deposit_g_cash - $oldGCashBase);
                    $deltaMaya = max(0, $actual_deposit_maya - $oldMayaBase);
                    $deltaInstapay = max(0, $actual_deposit_instapay - $oldInstapayBase);
                    $deltaOnlineBanking = max(0, $actual_deposit_online_banking - $oldOnlineBankingBase);
                    $deltaAirbnb = max(0, $actual_deposit_airbnb - $oldAirbnbBase);
        ...
        $oldLgpDiscountAmount = floatval($originalBooking['old_lgp_discount_amount'] ?? 0);
        $oldVipDiscountAmount = floatval($originalBooking['old_vip_discount_amount'] ?? 0);
        $oldLongDiscountAmount = floatval($originalBooking['old_long_discount_amount'] ?? 0);

        $newScPwdDiscountAmount = $discount_enabled ? $discount_amount : 0;
        $newLgpDiscountAmount = $lgp_discount_enabled ? $lgp_discount_amount : 0;
        $newVipDiscountAmount = $vip_discount_enabled ? $vip_discount_amount : 0;
        $newLongDiscountAmount = $long_discount_enabled ? $long_discount_amount : 0;

        $scPwdDiscountDelta = $newScPwdDiscountAmount - $oldDiscountAmount;
        $lgpDiscountDelta = $newLgpDiscountAmount - $oldLgpDiscountAmount;
        $vipDiscountDelta = $newVipDiscountAmount - $oldVipDiscountAmount;
        $longDiscountDelta = $newLongDiscountAmount - $oldLongDiscountAmount;
        $oldTotalDiscountAmount = $oldDiscountAmount + $oldLgpDiscountAmount + $oldVipDiscountAmount + $oldLongDiscountAmount;
        $newTotalDiscountAmount = $newScPwdDiscountAmount + $newLgpDiscountAmount + $newVipDiscountAmount + $newLongDiscountAmount;

        if (abs($scPwdDiscountDelta) > 0.001 || abs($lgpDiscountDelta) > 0.001 || abs($vipDiscountDelta) > 0.001 || abs($longDiscountDelta) > 0.001) {
            error_log("=== DISCOUNT CHANGED (history left unchanged) ===");
            error_log("old_sc_pwd=$oldDiscountAmount new_sc_pwd=$newScPwdDiscountAmount delta=$scPwdDiscountDelta");
            error_log("old_lgp=$oldLgpDiscountAmount new_lgp=$newLgpDiscountAmount delta=$lgpDiscountDelta");
            error_log("old_vip=$oldVipDiscountAmount new_vip=$newVipDiscountAmount delta=$vipDiscountDelta");
            error_log("old_long=$oldLongDiscountAmount new_long=$newLongDiscountAmount delta=$longDiscountDelta");
        ...
        // Each entry represents the incremental discount applied at that time (SC/PWD, LGP, VIP, Long Term).
        if ((abs($scPwdDiscountDelta) > 0.001 || abs($lgpDiscountDelta) > 0.001 || abs($vipDiscountDelta) > 0.001 || abs($longDiscountDelta) > 0.001) && !empty($booking_id_value)) {
            try {
                ...
                $updatedDiscHist = appendDiscountHistoryDeltaEntries(
                    $existingDiscHist,
                    [
                        'SC/PWD' => $scPwdDiscountDelta,
                        'LGP' => $lgpDiscountDelta,
                        'VIP' => $vipDiscountDelta,
                        'Long Term' => $longDiscountDelta,
                    ],
                    $discountTimestamp
                );
                ...
                error_log("Discount history updated (sc_pwd_delta=$scPwdDiscountDelta, lgp_delta=$lgpDiscountDelta, vip_delta=$vipDiscountDelta, long_delta=$longDiscountDelta): $updatedDiscHist");
            } catch (Exception $e) {
                error_log("Failed to update discount_amount_history: " . $e->getMessage());
            }
        } elseif ($newTotalDiscountAmount <= 0 && $oldTotalDiscountAmount > 0 && !empty($booking_id_value)) {
        ...
        // missing_items_* / penalty_* already resolved above (POST or original booking).
        // Keep those values so totals, payment history, and DB stay aligned with Guest/Pet extras.

        $hasNewDepositPost = isset($_POST['new_deposit']) || isset($_POST['new_deposit_cash']) || isset($_POST['new_deposit_g_cash']);
        if (!$hasNewDepositPost && $originalBooking) {
            $new_deposit = floatval($originalBooking['original_new_deposit'] ?? 0);
            $new_deposit_cash = floatval($originalBooking['original_new_deposit_cash'] ?? 0);
            $new_deposit_g_cash = floatval($originalBooking['original_new_deposit_g_cash'] ?? 0);
            $new_deposit_maya = floatval($originalBooking['original_new_deposit_maya'] ?? 0);
            $new_deposit_instapay = floatval($originalBooking['original_new_deposit_instapay'] ?? 0);
            $new_deposit_online_banking = floatval($originalBooking['original_new_deposit_online_banking'] ?? 0);
            $new_deposit_airbnb = floatval($originalBooking['original_new_deposit_airbnb'] ?? 0);
            $new_deposit_details = $originalBooking['original_new_deposit_details'] ?? null;
            $new_deposit_gcash_ref = trim((string) ($originalBooking['original_new_deposit_gcash_ref'] ?? ''));
            $new_deposit_maya_ref = trim((string) ($originalBooking['original_new_deposit_maya_ref'] ?? ''));
            $new_deposit_instapay_ref = trim((string) ($originalBooking['original_new_deposit_instapay_ref'] ?? ''));
            $new_deposit_online_banking_ref = trim((string) ($originalBooking['original_new_deposit_online_banking_ref'] ?? ''));
            $new_deposit_airbnb_ref = trim((string) ($originalBooking['original_new_deposit_airbnb_ref'] ?? ''));
        }

        if (in_array($slipper_status, ['returned', 'missing'], true)) {
            $newDepositApplied = applySlipperStatusToNewDeposit(
                $slipper_status,
                [
                    'new_deposit' => $new_deposit,
                    'new_deposit_cash' => $new_deposit_cash,
                    'new_deposit_g_cash' => $new_deposit_g_cash,
                    'new_deposit_maya' => $new_deposit_maya,
                    'new_deposit_instapay' => $new_deposit_instapay,
                    'new_deposit_online_banking' => $new_deposit_online_banking,
                    'new_deposit_airbnb' => $new_deposit_airbnb,
                    'new_deposit_details' => $new_deposit_details,
                    'new_deposit_gcash_ref' => $new_deposit_gcash_ref,
                    'new_deposit_maya_ref' => $new_deposit_maya_ref,
                    'new_deposit_instapay_ref' => $new_deposit_instapay_ref,
                    'new_deposit_online_banking_ref' => $new_deposit_online_banking_ref,
                    'new_deposit_airbnb_ref' => $new_deposit_airbnb_ref,
                ],
                $missing_items_fees,
                $missing_items_list,
                $additional_slippers
            );
            $new_deposit = floatval($newDepositApplied['new_deposit'] ?? 0);
            $new_deposit_cash = floatval($newDepositApplied['new_deposit_cash'] ?? 0);
            $new_deposit_g_cash = floatval($newDepositApplied['new_deposit_g_cash'] ?? 0);
            $new_deposit_maya = floatval($newDepositApplied['new_deposit_maya'] ?? 0);
            $new_deposit_instapay = floatval($newDepositApplied['new_deposit_instapay'] ?? 0);
            $new_deposit_online_banking = floatval($newDepositApplied['new_deposit_online_banking'] ?? 0);
            $new_deposit_airbnb = floatval($newDepositApplied['new_deposit_airbnb'] ?? 0);
            $new_deposit_details = $newDepositApplied['new_deposit_details'] ?? null;
            $new_deposit_gcash_ref = trim((string) ($newDepositApplied['new_deposit_gcash_ref'] ?? ''));
            $new_deposit_maya_ref = trim((string) ($newDepositApplied['new_deposit_maya_ref'] ?? ''));
            $new_deposit_instapay_ref = trim((string) ($newDepositApplied['new_deposit_instapay_ref'] ?? ''));
            $new_deposit_online_banking_ref = trim((string) ($newDepositApplied['new_deposit_online_banking_ref'] ?? ''));
            $new_deposit_airbnb_ref = trim((string) ($newDepositApplied['new_deposit_airbnb_ref'] ?? ''));
            $missing_items_fees = floatval($newDepositApplied['missing_items_fees'] ?? $missing_items_fees);
            $missing_items_list = $newDepositApplied['missing_items_list'] ?? $missing_items_list;
            // Keep total in sync when slipper forfeiture adds missing fees after the main calc.
            $full_booking_amount = $room_total + $guest_charges + $pet_charges + $additional_items_total
                + $extend_price + $extend_guest_charges + $missing_items_fees + $penalty_amount + $promo_breakfast_surplus;
            $full_booking_amount_after_discount = $full_booking_amount - $total_discount_amount;
            $total_amount = $full_booking_amount_after_discount;
            if ($paid_status === 'Paid' && !$durationChanged && !$promoChanged && $total_payments > 0) {
                $total_amount = max($total_amount, $total_payments);
            }
            $forfeited_new_deposit_cash = floatval($newDepositApplied['forfeited_new_deposit_cash'] ?? 0);
            $forfeited_new_deposit_g_cash = floatval($newDepositApplied['forfeited_new_deposit_g_cash'] ?? 0);
            $forfeited_new_deposit_maya = floatval($newDepositApplied['forfeited_new_deposit_maya'] ?? 0);
            $forfeited_new_deposit_instapay = floatval($newDepositApplied['forfeited_new_deposit_instapay'] ?? 0);
            $forfeited_new_deposit_online_banking = floatval($newDepositApplied['forfeited_new_deposit_online_banking'] ?? 0);
            $forfeited_new_deposit_airbnb = floatval($newDepositApplied['forfeited_new_deposit_airbnb'] ?? 0);
        } else {
            $forfeited_new_deposit_cash = floatval($originalBooking['original_forfeited_new_deposit_cash'] ?? 0);
            $forfeited_new_deposit_g_cash = floatval($originalBooking['original_forfeited_new_deposit_g_cash'] ?? 0);
            $forfeited_new_deposit_maya = floatval($originalBooking['original_forfeited_new_deposit_maya'] ?? 0);
            $forfeited_new_deposit_instapay = floatval($originalBooking['original_forfeited_new_deposit_instapay'] ?? 0);
            $forfeited_new_deposit_online_banking = floatval($originalBooking['original_forfeited_new_deposit_online_banking'] ?? 0);
            $forfeited_new_deposit_airbnb = floatval($originalBooking['original_forfeited_new_deposit_airbnb'] ?? 0);
        }

        // Ensure additional_fees columns exist (best-effort, in case update_checkout_fees.php was never called)
        foreach ([
            "additional_fees_status VARCHAR(50) DEFAULT 'None'",
            "additional_fees_payment_method VARCHAR(50) NULL DEFAULT NULL",
            "additional_fees_reference_no VARCHAR(255) NULL DEFAULT NULL",
            "additional_fees_paid_date DATETIME NULL DEFAULT NULL",
        ] as $_colDef) {
            $_colName = strtok($_colDef, ' ');
            try {
                $chk = $conn->query("SHOW COLUMNS FROM bookings LIKE '$_colName'");
                if ($chk && $chk->rowCount() == 0) {
                    $conn->exec("ALTER TABLE bookings ADD COLUMN $_colDef");
                }
            } catch (Exception $e) {
            }
        }
    ...
    SET guest_name = :guest_name,
        ...
        additional_slippers = :additional_slippers,
        slipper_status = :slipper_status,
        hygiene_kit_used = :hygiene_kit_used,
        hygiene_kit_price = :hygiene_kit_price,
        supplier = :supplier,
        room_price = :room_price,
        ...
        new_deposit_online_banking_ref = :new_deposit_online_banking_ref,
        new_deposit_airbnb_ref = :new_deposit_airbnb_ref,
        forfeited_new_deposit_cash = :forfeited_new_deposit_cash,
        forfeited_new_deposit_g_cash = :forfeited_new_deposit_g_cash,
        forfeited_new_deposit_maya = :forfeited_new_deposit_maya,
        forfeited_new_deposit_instapay = :forfeited_new_deposit_instapay,
        forfeited_new_deposit_online_banking = :forfeited_new_deposit_online_banking,
        forfeited_new_deposit_airbnb = :forfeited_new_deposit_airbnb,
        ...
        id_number = :id_number,
        lgp_discount_enabled = :lgp_discount_enabled,
        lgp_discount_amount = :lgp_discount_amount,
        lgp_approver_name = :lgp_approver_name,
        vip_discount_enabled = :vip_discount_enabled,
        vip_discount_amount = :vip_discount_amount,
        vip_approver_name = :vip_approver_name,
        long_discount_enabled = :long_discount_enabled,
        long_discount_amount = :long_discount_amount,
        long_discount_percent = :long_discount_percent,
...
");
        ...
        $stmt->bindParam(':additional_slippers', $additional_slippers, PDO::PARAM_INT);
        $stmt->bindParam(':slipper_status', $slipper_status);
        ...
        $stmt->bindParam(':new_deposit_cash', $new_deposit_cash);
        $stmt->bindParam(':new_deposit_g_cash', $new_deposit_g_cash);
        $stmt->bindParam(':new_deposit_maya', $new_deposit_maya);
        $stmt->bindParam(':new_deposit_instapay', $new_deposit_instapay);
        $stmt->bindParam(':new_deposit_online_banking', $new_deposit_online_banking);
        $stmt->bindParam(':new_deposit_airbnb', $new_deposit_airbnb);
        $stmt->bindParam(':new_deposit_details', $new_deposit_details);
        $stmt->bindParam(':new_deposit_gcash_ref', $new_deposit_gcash_ref);
        $stmt->bindParam(':new_deposit_maya_ref', $new_deposit_maya_ref);
        $stmt->bindParam(':new_deposit_instapay_ref', $new_deposit_instapay_ref);
        $stmt->bindParam(':new_deposit_online_banking_ref', $new_deposit_online_banking_ref);
        $stmt->bindParam(':new_deposit_airbnb_ref', $new_deposit_airbnb_ref);
        $stmt->bindParam(':forfeited_new_deposit_cash', $forfeited_new_deposit_cash);
        $stmt->bindParam(':forfeited_new_deposit_g_cash', $forfeited_new_deposit_g_cash);
        $stmt->bindParam(':forfeited_new_deposit_maya', $forfeited_new_deposit_maya);
        $stmt->bindParam(':forfeited_new_deposit_instapay', $forfeited_new_deposit_instapay);
        $stmt->bindParam(':forfeited_new_deposit_online_banking', $forfeited_new_deposit_online_banking);
        $stmt->bindParam(':forfeited_new_deposit_airbnb', $forfeited_new_deposit_airbnb);
        ...
        $stmt->bindParam(':lgp_discount_enabled', $lgp_discount_enabled, PDO::PARAM_INT);
        $stmt->bindParam(':lgp_discount_amount', $lgp_discount_amount);
        $stmt->bindParam(':lgp_approver_name', $lgp_approver_name);
        $stmt->bindParam(':vip_discount_enabled', $vip_discount_enabled, PDO::PARAM_INT);
        $stmt->bindParam(':vip_discount_amount', $vip_discount_amount);
        $stmt->bindParam(':vip_approver_name', $vip_approver_name);
        $stmt->bindParam(':long_discount_enabled', $long_discount_enabled, PDO::PARAM_INT);
        $stmt->bindParam(':long_discount_amount', $long_discount_amount);
        $stmt->bindParam(':long_discount_percent', $long_discount_percent);

        $stmt->bindParam(':guest_type', $guest_type);
        $stmt->bindParam(':second_guest_name', $second_guest_name);
        $stmt->bindParam(':additional_guest_names', $additional_guest_names);
        ...
        if ($stmt->execute()) {
            ...
            if (!empty($withdraw_extension)) {
                ...
                // second withdraw). If POST omitted withdraw_refund_amount, infer from segment cost ONLY
                // when the booking was already fully Paid.
                //
                // IMPORTANT: Do NOT treat "deposit >= extension cost" as a refund signal for Unpaid
                // bookings. Example: room ₱1500 + extension ₱1500 with partial deposit ₱1500 (Unpaid)
                // — that deposit is payment toward the room, not the extension. Withdrawing the
                // extension should keep the ₱1500 deposit and mark the booking Paid (balance ₱0).
                ...
                if ($withdraw_refund_amount < 0.009 && $segmentCost > 0.009) {
                    ...
                    if ($ps) {
                        ...
                        error_log('=== EXTENSION WITHDRAW: withdraw_refund_amount inferred from segment cost=' . $segmentCost . ' (POST missing; booking was Paid) ===');
                    } else {
                        error_log('=== EXTENSION WITHDRAW: no refund inferred (booking Unpaid/partial; deposit kept toward remaining balance) ===');
                    }
                }
                ...
                // Fetch current deposit (after main UPDATE) for refund / paid_status sync
                $getDepStmt = $conn->prepare("SELECT deposit, deposit_cash, deposit_g_cash, deposit_maya, deposit_instapay, deposit_online_banking, deposit_airbnb, downpayment_amount FROM bookings WHERE id = :id");
                ...
                $curDownpayment = floatval($depRow['downpayment_amount'] ?? $downpayment_amount ?? 0);

                // If Update posted empty payment fields, deposit can be wiped to 0 even though
                // the guest already paid. Restore from the pre-update row when no cash refund applies.
                $origDeposit = floatval($originalBooking['deposit'] ?? 0);
                if ($withdraw_refund_amount < 0.009 && $curDeposit < 0.009 && $origDeposit > 0.009) {
                    $curDeposit = $origDeposit;
                    $curDepCash = floatval($originalBooking['deposit_cash'] ?? 0);
                    $curDepGCash = floatval($originalBooking['deposit_g_cash'] ?? 0);
                    $curDepMaya = floatval($originalBooking['deposit_maya'] ?? 0);
                    $curDepInstapay = floatval($originalBooking['deposit_instapay'] ?? 0);
                    $curDepOnlineBanking = floatval($originalBooking['deposit_online_banking'] ?? 0);
                    $curDepAirbnb = floatval($originalBooking['deposit_airbnb'] ?? 0);
                    // If breakdown was empty historically, attribute restored total to cash
                    if (($curDepCash + $curDepGCash + $curDepMaya + $curDepInstapay + $curDepOnlineBanking + $curDepAirbnb) < 0.009) {
                        $curDepCash = $curDeposit;
                    }
                    $restoreDepStmt = $conn->prepare(
                        "UPDATE bookings
                         SET deposit = :dep,
                    ...
                    $restoreDepStmt->execute([
                        ':dep' => $curDeposit,
                        ':dep_cash' => $curDepCash,
                        ':dep_gcash' => $curDepGCash,
                        ':dep_maya' => $curDepMaya,
                        ':dep_instapay' => $curDepInstapay,
                        ':dep_online_banking' => $curDepOnlineBanking,
                        ':dep_airbnb' => $curDepAirbnb,
                        ':id' => $booking_id
                    ]);
                    if (!empty($booking_id_value)) {
                        $restoreRepStmt = $conn->prepare(
                            "UPDATE reports
                             SET deposit = :dep,
                                 deposit_cash = :dep_cash,
                                 deposit_g_cash = :dep_gcash,
                    ...
                    error_log("=== EXTENSION WITHDRAW: restored deposit={$curDeposit} wiped by empty Update payment fields ===");
                }
                ...
                // Always start from the PRE-UPDATE deposit (originalBooking) so we never
                // double-deduct after any incidental scaling/wipe in the main UPDATE.
                if ($withdraw_refund_amount > 0) {
                    error_log("=== EXTENSION REFUND: deducting {$withdraw_refund_amount} from deposit ===");

                    $baseDeposit = floatval($originalBooking['deposit'] ?? 0);
                    $baseDepCash = floatval($originalBooking['deposit_cash'] ?? 0);
                    $baseDepGCash = floatval($originalBooking['deposit_g_cash'] ?? 0);
                    $baseDepMaya = floatval($originalBooking['deposit_maya'] ?? 0);
                    $baseDepInstapay = floatval($originalBooking['deposit_instapay'] ?? 0);
                    $baseDepOnlineBanking = floatval($originalBooking['deposit_online_banking'] ?? 0);
                    $baseDepAirbnb = floatval($originalBooking['deposit_airbnb'] ?? 0);
                    if ($baseDeposit < 0.009) {
                        $baseDeposit = $curDeposit;
                        $baseDepCash = $curDepCash;
                        $baseDepGCash = $curDepGCash;
                        $baseDepMaya = $curDepMaya;
                        $baseDepInstapay = $curDepInstapay;
                        $baseDepOnlineBanking = $curDepOnlineBanking;
                        $baseDepAirbnb = $curDepAirbnb;
                    }
                    if (($baseDepCash + $baseDepGCash + $baseDepMaya + $baseDepInstapay + $baseDepOnlineBanking + $baseDepAirbnb) < 0.009 && $baseDeposit > 0.009) {
                        $baseDepCash = $baseDeposit;
                    }

                    // Subtract refund from the original deposit (room+extension payment)
                    $newDeposit = max(0, $baseDeposit - $withdraw_refund_amount);

                    // Proportionally reduce breakdown columns
                    $ratio = ($baseDeposit > 0) ? ($newDeposit / $baseDeposit) : 0;
                    $newDepCash = round($baseDepCash * $ratio, 2);
                    $newDepGCash = round($baseDepGCash * $ratio, 2);
                    $newDepMaya = round($baseDepMaya * $ratio, 2);
                    $newDepInstapay = round($baseDepInstapay * $ratio, 2);
                    $newDepOnlineBanking = round($baseDepOnlineBanking * $ratio, 2);
                    $newDepAirbnb = round($baseDepAirbnb * $ratio, 2);
                    ...
                    error_log("=== EXTENSION REFUND applied: baseDeposit={$baseDeposit}, new deposit={$newDeposit}, change_amount +{$withdraw_refund_amount} ===");

                    // Keep in-memory deposit vars in sync so the later reports sync doesn't
                    // overwrite the refunded amounts with the pre-withdraw totals.
                    $deposit = $newDeposit;
                    $deposit_cash = $newDepCash;
                    $deposit_g_cash = $newDepGCash;
                    $deposit_maya = $newDepMaya;
                    $deposit_instapay = $newDepInstapay;
                    $deposit_online_banking = $newDepOnlineBanking;
                    $deposit_airbnb = $newDepAirbnb;
                    $original_deposit_cash = $newDepCash;
                    $original_deposit_g_cash = $newDepGCash;
                    $original_deposit_maya = $newDepMaya;
                    $original_deposit_instapay = $newDepInstapay;
                    $original_deposit_online_banking = $newDepOnlineBanking;
                    $original_deposit_airbnb = $newDepAirbnb;

                    // Reduce payment history by the refund amount (do NOT delete the only payment
                    // when a single tender covered room+extension — that would wipe history to null).
                    error_log("=== ADJUSTING PAYMENT HISTORY FOR EXTENSION REFUND ===");
                    ...
                    $reduceHistoryByRefund = function (?string $historyString, float $refundLeft) {
                        if ($historyString === null || trim($historyString) === '' || $refundLeft < 0.009) {
                            return [$historyString, $refundLeft];
                        }
                        $segments = explode('|', $historyString);
                        for ($i = count($segments) - 1; $i >= 0 && $refundLeft > 0.009; $i--) {
                            $val = floatval(trim($segments[$i]));
                            if ($val <= 0.009) {
                                continue;
                            }
                            if ($val <= $refundLeft + 0.009) {
                                $refundLeft = max(0, $refundLeft - $val);
                                array_splice($segments, $i, 1);
                            } else {
                                $segments[$i] = number_format($val - $refundLeft, 2, '.', '');
                                $refundLeft = 0;
                            }
                        }
                        if (empty($segments)) {
                            return [null, $refundLeft];
                        }
                        return [implode('|', $segments), $refundLeft];
                    };

                    $refundLeft = $withdraw_refund_amount;
                    list($newCashHistory, $refundLeft) = $reduceHistoryByRefund($histRow['payment_amount_cash_history'] ?? null, $refundLeft);
                    list($newGCashHistory, $refundLeft) = $reduceHistoryByRefund($histRow['payment_amount_g_cash_history'] ?? null, $refundLeft);
                    list($newMayaHistory, $refundLeft) = $reduceHistoryByRefund($histRow['payment_amount_maya_history'] ?? null, $refundLeft);
                    list($newInstapayHistory, $refundLeft) = $reduceHistoryByRefund($histRow['payment_amount_instapay_history'] ?? null, $refundLeft);
                    list($newOnlineBankingHistory, $refundLeft) = $reduceHistoryByRefund($histRow['payment_amount_online_banking_history'] ?? null, $refundLeft);
                    list($newAirbnbHistory, $refundLeft) = $reduceHistoryByRefund($histRow['payment_amount_airbnb_history'] ?? null, $refundLeft);

                    // Keep payment_date_time segments aligned with remaining amount history length
                    $newPaymentDateTime = $histRow['payment_date_time'] ?? null;
                    $amountHistCombined = array_filter([
                        $newCashHistory,
                        $newGCashHistory,
                        $newMayaHistory,
                        $newInstapayHistory,
                        $newOnlineBankingHistory,
                        $newAirbnbHistory
                    ]);
                    if ($newPaymentDateTime && !empty($amountHistCombined)) {
                        // Prefer cash history length as primary timeline; fall back to longest remaining
                        $primaryHist = $newCashHistory ?: ($newGCashHistory ?: ($newMayaHistory ?: ($newInstapayHistory ?: ($newOnlineBankingHistory ?: $newAirbnbHistory))));
                        $keepCount = $primaryHist ? count(explode('|', $primaryHist)) : 0;
                        $dateSegments = explode('|', $newPaymentDateTime);
                        if ($keepCount > 0 && count($dateSegments) > $keepCount) {
                            $newPaymentDateTime = implode('|', array_slice($dateSegments, 0, $keepCount));
                        } elseif ($keepCount === 0) {
                            $newPaymentDateTime = null;
                        }
                    } elseif (empty($amountHistCombined)) {
                        $newPaymentDateTime = null;
                    }
                    ...
                    error_log("=== PAYMENT HISTORY UPDATED (refund-reduced) ===");
                }

                // Always recalculate total_amount + paid_status after extension withdrawal.
                // Unpaid/partial case: no refund, keep deposit → if deposit covers new total, mark Paid.
                $newExtendPrice = $remAgg['price'];
                $newExtendAdditionalGuest = $remAgg['eg'];
                ...
                $newFullBookingAmount = $newRoomTotal + $guest_charges + $pet_charges + $additional_items_total
                    + $missing_items_fees + $penalty_amount;
                $newFullBookingAmountAfterDiscount = $newFullBookingAmount - $total_discount_amount;

                $newAmountDue = $newFullBookingAmountAfterDiscount - ($curDownpayment + $newDeposit);
                ...
                error_log("newDeposit: " . $newDeposit);
                error_log("curDownpayment: " . $curDownpayment);
                error_log("newAmountDue: " . $newAmountDue);
                error_log("Updated total_amount: " . $total_amount);
                error_log("=== END RECALCULATION ===");

                $newPaidStatus = ($newAmountDue <= 0.02) ? 'Paid' : 'Unpaid';
                $hasAdditional = ($additional_items_total > 0 || ($additional_guest * 300) > 0 || ($additional_pet * 500) > 0);
                if ($newPaidStatus === 'Paid') {
                    $newAdditionalPaidStatus = $hasAdditional ? 'Paid' : 'None';
                } else {
                    $newAdditionalPaidStatus = $originalBooking['additional_paid_status'] ?? 'None';
                }
                ...
                $updateTotalStmt->execute([
                    ':total_amount' => $total_amount,
                    ':paid_status' => $newPaidStatus,
                    ':additional_paid_status' => $newAdditionalPaidStatus,
                    ':id' => $booking_id
                ]);

                if (!empty($booking_id_value)) {
                    $updateReportsTotalStmt = $conn->prepare("
                        UPDATE reports 
                        SET total_amount = :total_amount, 
                            paid_status = :paid_status,
                            additional_paid_status = :additional_paid_status 
                        WHERE booking_id = :booking_id
                    ");
                    $updateReportsTotalStmt->execute([
                        ':total_amount' => $total_amount,
                        ':paid_status' => $newPaidStatus,
                        ':additional_paid_status' => $newAdditionalPaidStatus,
            ...
            try {
                if ($booking_id_value) {
                    ...
                    if ($hasReportsTable) {
                        ...
                        $getAdditionalFeesStmt = $conn->prepare("SELECT missing_items_list, missing_items_fees, penalty_list, penalty_amount, additional_fees_status, additional_fees_payment_method, additional_fees_reference_no FROM bookings WHERE id = :booking_id");
                        ...
                        $penalty_list = $additionalFeesData['penalty_list'] ?? null;
                        $penalty_amount = floatval($additionalFeesData['penalty_amount'] ?? 0);
                    ...
                    ");
                        ...
                        $updateReportsStmt->bindParam(':second_guest_name', $second_guest_name);
                        $updateReportsStmt->bindParam(':additional_guest_names', $additional_guest_names);
                        ...
                        $updateReportsStmt->bindParam(':additional_slippers', $additional_slippers, PDO::PARAM_INT);
                        $updateReportsStmt->bindParam(':slipper_status', $slipper_status);
                        if ($additional_food === null) {
                        ...
                        $updateReportsStmt->bindParam(':penalty_amount', $penalty_amount);
                        if ($penalty_list === null || $penalty_list === '' || $penalty_list === '[]') {
                            $updateReportsStmt->bindValue(':penalty_list', null, PDO::PARAM_NULL);
                        } else {
                            $updateReportsStmt->bindParam(':penalty_list', $penalty_list);
                        }
                        ...
                        $updateReportsStmt->bindParam(':new_deposit', $new_deposit);
                        $updateReportsStmt->bindParam(':new_deposit_cash', $new_deposit_cash);
                        $updateReportsStmt->bindParam(':new_deposit_g_cash', $new_deposit_g_cash);
                        $updateReportsStmt->bindParam(':new_deposit_maya', $new_deposit_maya);
                        $updateReportsStmt->bindParam(':new_deposit_instapay', $new_deposit_instapay);
                        $updateReportsStmt->bindParam(':new_deposit_online_banking', $new_deposit_online_banking);
                        $updateReportsStmt->bindParam(':new_deposit_airbnb', $new_deposit_airbnb);
                        $updateReportsStmt->bindParam(':new_deposit_details', $new_deposit_details);
                        $updateReportsStmt->bindParam(':new_deposit_gcash_ref', $new_deposit_gcash_ref);
                        $updateReportsStmt->bindParam(':new_deposit_maya_ref', $new_deposit_maya_ref);
                        $updateReportsStmt->bindParam(':new_deposit_instapay_ref', $new_deposit_instapay_ref);
                        $updateReportsStmt->bindParam(':new_deposit_online_banking_ref', $new_deposit_online_banking_ref);
                        $updateReportsStmt->bindParam(':new_deposit_airbnb_ref', $new_deposit_airbnb_ref);
                        ...
                        $updateReportsStmt->bindParam(':long_discount_enabled', $long_discount_enabled, PDO::PARAM_INT);
                        $updateReportsStmt->bindParam(':long_discount_amount', $long_discount_amount);
                        $updateReportsStmt->bindParam(':long_discount_percent', $long_discount_percent);
            ...
            // Calculate total paid across all 6 payment history / breakdown channels
            $_sumHist = function ($histStr, $fallbackVal = 0.0) {
                if ($histStr && trim((string) $histStr) !== '') {
                    $sum = 0.0;
                    foreach (explode('|', (string) $histStr) as $p) {
                        $v = floatval(trim($p));
                        if ($v > 0)
                            $sum += $v;
                    }
                ...
                return floatval($fallbackVal);
            };

            $_cashPaid = $_sumHist($payment_amount_cash_history ?? null, ($deposit_cash ?? 0) + ($downpayment_cash ?? 0));
            $_gcashPaid = $_sumHist($payment_amount_g_cash_history ?? null, ($deposit_g_cash ?? 0) + ($downpayment_gcash ?? 0));
            $_mayaPaid = $_sumHist($payment_amount_maya_history ?? null, ($deposit_maya ?? 0) + ($downpayment_maya ?? 0));
            $_instapayPaid = $_sumHist($payment_amount_instapay_history ?? null, ($deposit_instapay ?? 0) + ($downpayment_instapay ?? 0));
            $_onlinePaid = $_sumHist($payment_amount_online_banking_history ?? null, ($deposit_online_banking ?? 0) + ($downpayment_online_banking ?? 0));
            $_airbnbPaid = $_sumHist($payment_amount_airbnb_history ?? null, ($deposit_airbnb ?? 0) + ($downpayment_airbnb ?? 0));

            $_totalPaidSum = $_cashPaid + $_gcashPaid + $_mayaPaid + $_instapayPaid + $_onlinePaid + $_airbnbPaid;

            // Booking total for the log — use the computed booking total (not undefined)
            $_bookingTotal = $total_amount ?? 0.0;

            // Build [Methods: ...] using only the payment made in THIS update (the delta),
            // so the log reflects what was just paid rather than the lifetime cumulative total.
            $_logMethodsList = [];
            $_logDeltaCash        = isset($deltaCash)        ? floatval($deltaCash)        : 0.0;
            $_logDeltaGCash       = isset($deltaGCash)       ? floatval($deltaGCash)       : 0.0;
            $_logDeltaMaya        = isset($deltaMaya)        ? floatval($deltaMaya)        : 0.0;
            $_logDeltaInstapay    = isset($deltaInstapay)    ? floatval($deltaInstapay)    : 0.0;
            $_logDeltaOnline      = isset($deltaOnlineBanking) ? floatval($deltaOnlineBanking) : 0.0;
            $_logDeltaAirbnb      = isset($deltaAirbnb)      ? floatval($deltaAirbnb)      : 0.0;
            $_logDeltaTotal = $_logDeltaCash + $_logDeltaGCash + $_logDeltaMaya + $_logDeltaInstapay + $_logDeltaOnline + $_logDeltaAirbnb;

            if ($_logDeltaTotal > 0.001) {
                // A new payment was collected in this update — log only the new amounts
                if ($_logDeltaCash > 0)
                    $_logMethodsList[] = "Cash: ₱" . number_format($_logDeltaCash, 2);
                if ($_logDeltaGCash > 0)
                    $_logMethodsList[] = "GCash: ₱" . number_format($_logDeltaGCash, 2);
                if ($_logDeltaMaya > 0)
                    $_logMethodsList[] = "Maya: ₱" . number_format($_logDeltaMaya, 2);
                if ($_logDeltaInstapay > 0)
                    $_logMethodsList[] = "Instapay: ₱" . number_format($_logDeltaInstapay, 2);
                if ($_logDeltaOnline > 0)
                    $_logMethodsList[] = "Online Banking: ₱" . number_format($_logDeltaOnline, 2);
                if ($_logDeltaAirbnb > 0)
                    $_logMethodsList[] = "Airbnb: ₱" . number_format($_logDeltaAirbnb, 2);
            } else {
                // No new payment this update — fall back to cumulative totals
                if ($_cashPaid > 0)
                    $_logMethodsList[] = "Cash: ₱" . number_format($_cashPaid, 2);
                if ($_gcashPaid > 0)
                    $_logMethodsList[] = "GCash: ₱" . number_format($_gcashPaid, 2);
                if ($_mayaPaid > 0)
                    $_logMethodsList[] = "Maya: ₱" . number_format($_mayaPaid, 2);
                if ($_instapayPaid > 0)
                    $_logMethodsList[] = "Instapay: ₱" . number_format($_instapayPaid, 2);
                if ($_onlinePaid > 0)
                    $_logMethodsList[] = "Online Banking: ₱" . number_format($_onlinePaid, 2);
                if ($_airbnbPaid > 0)
                    $_logMethodsList[] = "Airbnb: ₱" . number_format($_airbnbPaid, 2);
            }

            $_paymentMethodsSummary = !empty($_logMethodsList) ? implode(', ', $_logMethodsList) : 'None/Unpaid';
            // Build Discounts Summary
            $_discountsList = [];
            if (!empty($discount_enabled) && ($discount_amount ?? 0) > 0) {
                $_scLabel = "SC/PWD";
                if (!empty($sc_pwd_count))
                    $_scLabel .= " (x{$sc_pwd_count})";
                $_scLabel .= ": -₱" . number_format($discount_amount, 2);
                if (!empty($id_number))
                    $_scLabel .= " [ID: {$id_number}]";
                $_discountsList[] = $_scLabel;
            }
            if (!empty($lgp_discount_enabled) && ($lgp_discount_amount ?? 0) > 0) {
                $_discountsList[] = "LGP: -₱" . number_format($lgp_discount_amount, 2) . (!empty($lgp_approver_name) ? " [By: {$lgp_approver_name}]" : "");
            }
            if (!empty($vip_discount_enabled) && ($vip_discount_amount ?? 0) > 0) {
                $_discountsList[] = "VIP: -₱" . number_format($vip_discount_amount, 2) . (!empty($vip_approver_name) ? " [By: {$vip_approver_name}]" : "");
            }
            if (!empty($long_discount_enabled) && ($long_discount_amount ?? 0) > 0) {
                $_discountsList[] = "Long Term" . (!empty($long_discount_percent) ? " ({$long_discount_percent}%)" : "") . ": -₱" . number_format($long_discount_amount, 2);
            }
            $_discountsSummary = !empty($_discountsList) ? implode('; ', $_discountsList) : 'None';

            // Build Extras Summary (Additional Guest, Pet, Food, Items, Breakfast)
            $_extrasList = [];
            if (!empty($additional_guest) && $additional_guest > 0)
                $_extrasList[] = "Guests: +{$additional_guest}";
            if (!empty($additional_pet) && $additional_pet > 0)
                $_extrasList[] = "Pets: +{$additional_pet}";
            if (!empty($breakfast) && $breakfast !== 'None' && $breakfast !== 'Select Breakfast')
                $_extrasList[] = "Breakfast: {$breakfast}";
            if (!empty($additional_food) && trim((string) $additional_food) !== '')
                $_extrasList[] = "Food: " . str_replace("\n", ", ", trim($additional_food));
            if (!empty($additional_items) && trim((string) $additional_items) !== '')
                $_extrasList[] = "Items: " . str_replace("\n", ", ", trim($additional_items));
            $_extrasSummary = !empty($_extrasList) ? implode(' | ', $_extrasList) : 'None';

            // Effective Duration (including Promo indicator if applicable)
            $_effectiveDuration = "{$duration} {$duration_unit}";
            if (!empty($promo) && $promo !== 'None' && $promo !== 'Select Promo' && $promo !== 'Regular') {
                $_effectiveDuration .= " [Promo: {$promo}]";
            }
            ...
            logActivity(
                ...
                'update',
                'BOOKING_UPDATE',
                "Booking #{$booking_id} updated by {$encoder} — Guest: " . ($guest_name ?? 'Guest') . ", Room: " . ($room_id ?? 'N/A') . ", Duration: {$_effectiveDuration}, Booking Total: ₱" . number_format($_bookingTotal, 2) . ", Total Paid: ₱" . number_format($_totalPaidSum, 2) . " [Methods: {$_paymentMethodsSummary}], Discounts: {$_discountsSummary}, Extras: {$_extrasSummary}, Status: {$paid_status}",
                [
                    ...
                    'additional_guest' => $additional_guest ?? 0,
                    'additional_pet' => $additional_pet ?? 0,
                    'additional_food' => $additional_food ?? null,
                    'additional_items' => $additional_items ?? null,
                    'discounts_summary' => $_discountsSummary,
                    'sc_pwd_discount' => $discount_amount ?? 0,
                    'sc_pwd_id' => $id_number ?? null,
                    'lgp_discount' => $lgp_discount_amount ?? 0,
                    'lgp_approver' => $lgp_approver_name ?? null,
                    'vip_discount' => $vip_discount_amount ?? 0,
                    'vip_approver' => $vip_approver_name ?? null,
                    'long_term_discount' => $long_discount_amount ?? 0,
                    'booking_total' => $_bookingTotal,
                    'total_paid' => $_totalPaidSum,
                    'payment_methods' => $_paymentMethodsSummary,
                    'cash_paid' => $_cashPaid,
                    'gcash_paid' => $_gcashPaid,
                    'maya_paid' => $_mayaPaid,
                    'instapay_paid' => $_instapayPaid,
                    'online_bank_paid' => $_onlinePaid,
                    'airbnb_paid' => $_airbnbPaid,
                    'paid_status' => $paid_status ?? '',
                    'contact_no' => $contact_no ?? '',
                    'address' => $address ?? '',
                    'updated_by' => $encoder
            ...
            logError($conn, 'update', 'UPDATE', "Failed to update Booking #{$booking_id}", [
                'booking_id' => $booking_id ?? null
            ], $booking_id ?? null);
        }
    } catch (PDOException $e) {
        $response['message'] = 'Database error: ' . $e->getMessage();
        logError($conn, 'update', 'UPDATE', "Error updating Booking #{$booking_id}: " . $e->getMessage(), [
            'booking_id' => $booking_id ?? null,
            'error' => $e->getMessage()
        ], $booking_id ?? null);
    }
