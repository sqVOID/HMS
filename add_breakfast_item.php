...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            logActivity($conn, 'menu', 'CREATE', "Breakfast item '{$food_name}' added (₱" . number_format($price, 2) . ")" . (!empty($food_image) ? " with image" : "") . " by {$encoder}", [
                'food_id'     => $newId,
                'food_name'   => $food_name,
                'price'       => $price,
                'description' => $description,
                'has_image'   => !empty($food_image),
                'added_by'    => $encoder
            ], $newId);
        } else {
            $response['message'] = 'Failed to add food item!';
            logError($conn, 'menu', 'CREATE', "Failed to add breakfast item '{$food_name}'", [
                'food_name' => $food_name
            ]);
        }
    } catch(PDOException $e) {
        ...
        logError($conn, 'menu', 'CREATE', "Error adding breakfast item '{$food_name}': " . $e->getMessage(), [
            'food_name' => $food_name ?? '',
            'error'     => $e->getMessage()
        ]);
    }
