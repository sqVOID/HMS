...
try {
    ...
    logActivity($conn, 'report', 'REPORT_EXPORT',
        "Booking report exported (XLS) by {$_exp_user} for range: {$selectedRangeKey}" . ($selectedRangeKey === 'custom' ? " ({$customStart} to {$customEnd})" : ''),
        ['range' => $selectedRangeKey, 'start' => $customStart, 'end' => $customEnd, 'exported_by' => $_exp_user, 'format' => 'XLS']
    );
    ...
    if (!empty($dataRows)) {
        foreach ($dataRows as $row) {
            ...
            echo '<td style="' . $revenueTdBase . '">' . nl2br(htmlspecialchars($row['guest_name'] ?? '')) . '</td>';
    ...
    if (count($nonRefundRevenueRecords) > 0) {
        foreach ($nonRefundRevenueRecords as $record) {
            ...
            // Convert newlines to <br> for guest name display
            echo '<td style="padding:5px;">' . nl2br(htmlspecialchars($record['guest_name'] ?? '')) . '</td>';
            ...
            foreach ($additionalFeesRecords as $feeRecord) {
                ...
                if (!empty($items)) {
                    ...
                    foreach ($items as $item) {
                        ...
                        $itemStrings[] = formatMissingItemReportLine($item, $feeRecord);
                    }
                ...
                // Convert newlines to <br> for guest name display
                echo '<td style="padding: 5px;">' . nl2br(htmlspecialchars($feeRecord['guest_name'] ?? '')) . '</td>';
