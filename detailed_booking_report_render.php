...
if (!function_exists('renderDetailedBookingReportTable')) {
    ...
    function renderDetailedBookingReportTable(
        ...
        $headers = [
            ...
            'Breakfast', 'Promo',
            'SC/PWD Discount Amount', 'LGP Discount Amount', 'LGP Approver Name',
            'VIP Discount Amount', 'VIP Approver Name', 'Long Term Discount', 'Discount Date',
            'Additional Items', 'Additional Foods',
        ...
        if (empty($dataRows)) {
            ...
            foreach ($dataRows as $row) {
                ...
                $guestNameDisplay = nl2br(htmlspecialchars($row['guest_name'] ?? '—'));
                echo '<td style="' . $tdBase . '">' . $guestNameDisplay . '</td>';
                ...
                echo '<td style="' . $tdBase . '">' . formatDetailedReportDiscountAmountDisplay(floatval($row['sc_pwd_discount_amount'] ?? 0)) . '</td>';
                echo '<td style="' . $tdBase . '">' . formatDetailedReportDiscountAmountDisplay(floatval($row['lgp_discount_amount'] ?? 0)) . '</td>';
                echo '<td style="' . $tdBase . '">' . htmlspecialchars(($row['lgp_approver_name'] ?? '') !== '' ? $row['lgp_approver_name'] : '—') . '</td>';
                echo '<td style="' . $tdBase . '">' . formatDetailedReportDiscountAmountDisplay(floatval($row['vip_discount_amount'] ?? 0)) . '</td>';
                echo '<td style="' . $tdBase . '">' . htmlspecialchars(($row['vip_approver_name'] ?? '') !== '' ? $row['vip_approver_name'] : '—') . '</td>';
                echo '<td style="' . $tdBase . '">' . htmlspecialchars(formatDetailedReportLongTermDiscountDisplay(
