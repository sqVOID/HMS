// ── Load global error capture (sends all JS errors → SystemLog.php) ──────────
(function () {
    if (!window.__hmsErrorCaptureLoaded) {
        window.__hmsErrorCaptureLoaded = true;
        const s = document.createElement('script');
        s.src = 'hms_error_capture.js?v=' + Date.now();
        s.async = false; // load before rest of scripts so onerror is set ASAP
...
(function () {
    ...
    // System Log — Super Admin only
    const SYSTEM_LOG_HIDE_SELECTORS = [
        '[data-page="SystemLog.php"]',
        '[data-page="systemlog.php"]'
    ];
    ...
    const PAGE_ACCESS_RULES = {
        ...
        'systemlog.php': [ROLE_SUPER_ADMIN]
    };

    // Hide all menu items initially to prevent flash - but with visibility instead of display
    ...
    // Remove the temporary hiding style and make items visible
    ...
    function applyRoleRules(level) {
        ...
        switch (normalizedLevel) {
            ...
            case ROLE_STAFF:
                ...
                // Hide System Log (super_admin only)
                hideElements(SYSTEM_LOG_HIDE_SELECTORS);
                break;
            ...
            default:
                ...
                showElements(SYSTEM_LOG_HIDE_SELECTORS);
                break;
        }
    }
