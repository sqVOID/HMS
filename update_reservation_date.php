...
try {
    $selectedDt = new DateTime($reservation_date);
    if ((int)$selectedDt->format('Y') !== (int)date('Y')) {
        $response['message'] = 'Reservation date must be in the current year.';
        echo json_encode($response);
        exit;
    }
} catch (Exception $e) {
    $response['message'] = 'Invalid reservation date.';
    echo json_encode($response);
    exit;
}
...
try {
    ...
    logActivity($conn, 'update', 'RESERVATION_DATE_UPDATE',
        "Reservation date for booking #{$id} updated to {$reservation_date} by {$_actor}",
        ['booking_id' => $id, 'reservation_date' => $reservation_date, 'updated_by' => $_actor]
    );
    echo json_encode($response);
    exit;
} catch (Exception $e) {
