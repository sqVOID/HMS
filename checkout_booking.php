...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Forfeited new_deposit (missing slipper) → append to payment_amount_*_history at checkout date
        $booking = appendForfeitedNewDepositToPaymentHistory($conn, $booking, $checkedOutAtValue);
            ...
            try {
                ...
                if ($hasReportsTable) {
                    ...
                    if ($existingReport) {
                        ...
                        $paInstapayHist = $booking['payment_amount_instapay_history'] ?? null;
                        $paOnlineBankingHist = $booking['payment_amount_online_banking_history'] ?? null;
                        $paAirbnbHist = $booking['payment_amount_airbnb_history'] ?? null;
                        if ($paInstapayHist === null) {
                            $updateReportsStmt->bindValue(':payment_amount_instapay_history', null, PDO::PARAM_NULL);
                        } else {
                            $updateReportsStmt->bindParam(':payment_amount_instapay_history', $paInstapayHist);
                        }
                        if ($paOnlineBankingHist === null) {
                            $updateReportsStmt->bindValue(':payment_amount_online_banking_history', null, PDO::PARAM_NULL);
                        } else {
                            $updateReportsStmt->bindParam(':payment_amount_online_banking_history', $paOnlineBankingHist);
                        }
                        if ($paAirbnbHist === null) {
                            $updateReportsStmt->bindValue(':payment_amount_airbnb_history', null, PDO::PARAM_NULL);
                        } else {
                            $updateReportsStmt->bindParam(':payment_amount_airbnb_history', $paAirbnbHist);
                        }
                        ...
                        $paAirbnbHist = $booking['payment_amount_airbnb_history'] ?? null;
                        if ($paInstapayHist === null) {
                            $insertReportsStmt->bindValue(':payment_amount_instapay_history', null, PDO::PARAM_NULL);
                        } else {
                            $insertReportsStmt->bindParam(':payment_amount_instapay_history', $paInstapayHist);
                        }
                        if ($paOnlineBankingHist === null) {
                            $insertReportsStmt->bindValue(':payment_amount_online_banking_history', null, PDO::PARAM_NULL);
                        } else {
                            $insertReportsStmt->bindParam(':payment_amount_online_banking_history', $paOnlineBankingHist);
                        }
                        if ($paAirbnbHist === null) {
                            $insertReportsStmt->bindValue(':payment_amount_airbnb_history', null, PDO::PARAM_NULL);
                        } else {
                            $insertReportsStmt->bindParam(':payment_amount_airbnb_history', $paAirbnbHist);
                        }
        ...
        if (!empty($booking['booking_type']) && $booking['booking_type'] === 'Reservation') {
            ...
            $updateBookingStatusStmt->bindParam(':additional_fees_status', $additionalFeesStatusValue);
            $updateBookingStatusStmt->bindParam(':additional_fees_payment_method', $additionalFeesPaymentMethodValue);
            $updateBookingStatusStmt->bindParam(':additional_fees_reference_no', $additionalFeesReferenceNoValue);
            $additionalFeesPaidDate = ($additionalFeesStatusValue === 'Paid') ? date('Y-m-d H:i:s') : ($booking['additional_fees_paid_date'] ?? null);
            $updateBookingStatusStmt->bindParam(':additional_fees_paid_date', $additionalFeesPaidDate);
        ...
        logActivity($conn, 'checkout', 'CHECKOUT', "Guest " . ($booking['guest_name'] ?? 'Unknown') . " checked out from Room " . ($booking['room_id'] ?? 'N/A') . " (Booking #{$booking_id})", [
            'booking_id' => $booking_id,
            'guest_name' => $booking['guest_name'] ?? '',
            'room_id'    => $booking['room_id'] ?? null,
            'encoder'    => $encoder_checkout ?? ''
        ], $booking_id);

    } catch (PDOException $e) {
        $response['message'] = 'Database error: ' . $e->getMessage();
        logError($conn, 'checkout', 'CHECKOUT', "Checkout error for Booking #{$booking_id}: " . $e->getMessage(), [
            'booking_id' => $booking_id,
            'error'      => $e->getMessage()
        ], $booking_id);
    }
