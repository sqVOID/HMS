...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($update_image) {
            ...
            if ($stmt->execute()) {
                updateRoomDurations($conn, $id, $durations);
                logActivity($conn, 'update', 'ROOM_UPDATE',
                    "Room '{$room_id}' (Type: {$room_type}) updated by {$_actor}",
                    ['db_id' => $id, 'room_id' => $room_id, 'room_type' => $room_type, 'updated_by' => $_actor]
                );
                $response['success'] = true;
                $response['message'] = 'Room updated successfully!';
            } else {
...
function updateRoomDurations($conn, $roomId, $durations) {
    ...
    $insertStmt = $conn->prepare("INSERT INTO room_durations (room_id, duration_hours, price, duration_unit) VALUES (:room_id, :duration_hours, :price, :duration_unit)");
