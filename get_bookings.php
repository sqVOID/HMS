...
try {
    ...
    if ($hasBookingsTable) {
        ...
        // Ensure additional_slippers column exists
        try {
        ...
        // Ensure slipper_status column exists
        try {
        ...
        // Ensure LGP/VIP/Long Term discount columns exist
        $extraDiscountCols = [
            'lgp_discount_enabled' => "ALTER TABLE bookings ADD COLUMN lgp_discount_enabled TINYINT(1) DEFAULT 0",
            'lgp_discount_amount' => "ALTER TABLE bookings ADD COLUMN lgp_discount_amount DECIMAL(10,2) DEFAULT 0",
            'lgp_approver_name' => "ALTER TABLE bookings ADD COLUMN lgp_approver_name VARCHAR(255) NULL DEFAULT NULL",
            'vip_discount_enabled' => "ALTER TABLE bookings ADD COLUMN vip_discount_enabled TINYINT(1) DEFAULT 0",
            'vip_discount_amount' => "ALTER TABLE bookings ADD COLUMN vip_discount_amount DECIMAL(10,2) DEFAULT 0",
            'vip_approver_name' => "ALTER TABLE bookings ADD COLUMN vip_approver_name VARCHAR(255) NULL DEFAULT NULL",
            'long_discount_enabled' => "ALTER TABLE bookings ADD COLUMN long_discount_enabled TINYINT(1) DEFAULT 0",
            'long_discount_amount' => "ALTER TABLE bookings ADD COLUMN long_discount_amount DECIMAL(10,2) DEFAULT 0",
            'long_discount_percent' => "ALTER TABLE bookings ADD COLUMN long_discount_percent DECIMAL(5,2) DEFAULT 0"
        ];
        foreach ($extraDiscountCols as $col => $sql) {
            try {
                $checkColumn = $conn->query("SHOW COLUMNS FROM bookings LIKE '$col'");
                if ($checkColumn->rowCount() == 0) {
                    $conn->exec($sql);
...
");
        ...
        try {
            ...
            // Table might not exist or empty — fallback without unit column
            try {
