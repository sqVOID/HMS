...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $new_deposit_details = isset($_POST['new_deposit_details']) && $_POST['new_deposit_details'] !== '' ? $_POST['new_deposit_details'] : null;
    $new_deposit_gcash_ref = isset($_POST['new_deposit_gcash_ref']) && $_POST['new_deposit_gcash_ref'] !== '' ? $_POST['new_deposit_gcash_ref'] : null;
    $new_deposit_maya_ref = isset($_POST['new_deposit_maya_ref']) && $_POST['new_deposit_maya_ref'] !== '' ? $_POST['new_deposit_maya_ref'] : null;
    $new_deposit_instapay_ref = isset($_POST['new_deposit_instapay_ref']) && $_POST['new_deposit_instapay_ref'] !== '' ? $_POST['new_deposit_instapay_ref'] : null;
    $new_deposit_online_banking_ref = isset($_POST['new_deposit_online_banking_ref']) && $_POST['new_deposit_online_banking_ref'] !== '' ? $_POST['new_deposit_online_banking_ref'] : null;
    $new_deposit_airbnb_ref = isset($_POST['new_deposit_airbnb_ref']) && $_POST['new_deposit_airbnb_ref'] !== '' ? $_POST['new_deposit_airbnb_ref'] : null;
    ...
    // Long Term Discount fields (accept long_discount_* and long_term_discount_* from frontend)
    ...
    // Initialize discount amount history — one entry per active discount type (SC/PWD, LGP, VIP)
    $discountTimestamp = date('Y-m-d H:i:s');
    $discount_amount_history = buildDiscountAmountHistoryEntries([
        'SC/PWD' => $discount_enabled ? $discount_amount : 0,
        'LGP' => $lgp_discount_enabled ? $lgp_discount_amount : 0,
        'VIP' => $vip_discount_enabled ? $vip_discount_amount : 0,
        'Long Term' => $long_discount_enabled ? $long_discount_amount : 0,
    ], $discountTimestamp);
    ...
    try {
        ...
        foreach ($discountColumns as $columnName => $columnDefinition) {
            try {
                ...
                $checkReportsColumn = $conn->query("SHOW COLUMNS FROM reports LIKE '$columnName'");
                if ($checkReportsColumn->rowCount() == 0) {
                    $conn->exec("ALTER TABLE reports ADD COLUMN $columnName $columnDefinition");
                    error_log("Added reports discount column: $columnName");
                }
            } catch (PDOException $e) {
        ...
        try {
            $checkColumn = $conn->query("SHOW COLUMNS FROM bookings LIKE 'additional_slippers'");
            if ($checkColumn->rowCount() == 0) {
                $conn->exec("ALTER TABLE bookings ADD COLUMN additional_slippers INT DEFAULT 0 AFTER additional_pet");
            }
        } catch (PDOException $e) {
            error_log("Failed to check/add additional_slippers column: " . $e->getMessage());
        }
        try {
            $checkColumn = $conn->query("SHOW COLUMNS FROM bookings LIKE 'slipper_status'");
            if ($checkColumn->rowCount() == 0) {
                $conn->exec("ALTER TABLE bookings ADD COLUMN slipper_status VARCHAR(20) DEFAULT 'added' AFTER additional_slippers");
            }
        } catch (PDOException $e) {
            error_log("Failed to check/add slipper_status column: " . $e->getMessage());
        }
        ...
        $total_amount = max(0, $total_amount - $total_discount_amount);
    ...
    INSERT INTO bookings (
        booking_id, room_id, room_type, guest_name, reason_for_stay, contact_no, address, request, additional_guest, additional_pet, additional_slippers, slipper_status,
        ...
        new_deposit, new_deposit_cash, new_deposit_g_cash, new_deposit_maya, new_deposit_instapay, new_deposit_online_banking, new_deposit_airbnb,
        new_deposit_details, new_deposit_gcash_ref, new_deposit_maya_ref, new_deposit_instapay_ref, new_deposit_online_banking_ref, new_deposit_airbnb_ref,
        ...
        guest_type, second_guest_name, additional_guest_names, contact_person_name, number_of_adults, number_of_children, tin_number, email,
        ...
        '', :room_id, :room_type, :guest_name, :reason_for_stay, :contact_no, :address, :request, :additional_guest, :additional_pet, :additional_slippers, :slipper_status,
        :check_in, :check_out, :duration, :duration_unit, :hours,
        :status, :booking_type, :room_image,    
        :referral_name, :promo, :breakfast, :breakfast_date, :payment_status, :reference_no, :additional, :additional_fees_items, :paid_status,
        :additional_paid_status,
...
");
        ...
        if ($stmt->execute()) {
            ...
            try {
                ...
                $lgp_discount_enabled_from_booking = $lgp_discount_enabled;
                $lgp_discount_amount_from_booking = $lgp_discount_amount;
                $lgp_approver_name_from_booking = $lgp_approver_name;
                $vip_discount_enabled_from_booking = $vip_discount_enabled;
                $vip_discount_amount_from_booking = $vip_discount_amount;
                $vip_approver_name_from_booking = $vip_approver_name;
                $long_discount_enabled_from_booking = $long_discount_enabled;
                $long_discount_amount_from_booking = $long_discount_amount;
                $long_discount_percent_from_booking = $long_discount_percent;
                ...
                try {
                    ...
                    if ($existingBooking) {
                        ...
                        $lgp_discount_enabled_from_booking = intval($existingBooking['lgp_discount_enabled'] ?? 0);
                        $lgp_discount_amount_from_booking = floatval($existingBooking['lgp_discount_amount'] ?? 0);
                        $lgp_approver_name_from_booking = $existingBooking['lgp_approver_name'] ?? '';
                        $vip_discount_enabled_from_booking = intval($existingBooking['vip_discount_enabled'] ?? 0);
                        $vip_discount_amount_from_booking = floatval($existingBooking['vip_discount_amount'] ?? 0);
                        $vip_approver_name_from_booking = $existingBooking['vip_approver_name'] ?? '';
                        $long_discount_enabled_from_booking = intval($existingBooking['long_discount_enabled'] ?? 0);
                        $long_discount_amount_from_booking = floatval($existingBooking['long_discount_amount'] ?? 0);
                        $long_discount_percent_from_booking = floatval($existingBooking['long_discount_percent'] ?? 0);
    ...
    INSERT INTO reports (
        id, booking_id, room_id, room_type, guest_name, second_guest_name, additional_guest_names, guest_type, contact_person_name, tin_number, reason_for_stay, address, request,
        promo, breakfast, breakfast_date, additional_guest, additional_pet, additional_slippers, slipper_status, payment_status, reference_no, referral_name, supplier, additional, additional_food, additional_items, paid_status,
        check_in, check_out, duration, duration_unit, hours, 
...
");
                ...
                $reportsStmt->bindParam(':second_guest_name', $second_guest_name);
                $reportsStmt->bindParam(':additional_guest_names', $additional_guest_names);
                ...
                $reportsStmt->bindParam(':additional_slippers', $additional_slippers);
                $reportsStmt->bindParam(':slipper_status', $slipper_status);
                ...
                $reportsStmt->bindParam(':new_deposit', $new_deposit);
                $reportsStmt->bindParam(':new_deposit_cash', $new_deposit_cash);
                $reportsStmt->bindParam(':new_deposit_g_cash', $new_deposit_g_cash);
                $reportsStmt->bindParam(':new_deposit_maya', $new_deposit_maya);
                $reportsStmt->bindParam(':new_deposit_instapay', $new_deposit_instapay);
                $reportsStmt->bindParam(':new_deposit_online_banking', $new_deposit_online_banking);
                $reportsStmt->bindParam(':new_deposit_airbnb', $new_deposit_airbnb);
                $reportsStmt->bindParam(':new_deposit_details', $new_deposit_details);
                $reportsStmt->bindParam(':new_deposit_gcash_ref', $new_deposit_gcash_ref);
                $reportsStmt->bindParam(':new_deposit_maya_ref', $new_deposit_maya_ref);
                $reportsStmt->bindParam(':new_deposit_instapay_ref', $new_deposit_instapay_ref);
                $reportsStmt->bindParam(':new_deposit_online_banking_ref', $new_deposit_online_banking_ref);
                $reportsStmt->bindParam(':new_deposit_airbnb_ref', $new_deposit_airbnb_ref);
                ...
                $reportsStmt->bindParam(':lgp_discount_enabled', $lgp_discount_enabled_from_booking, PDO::PARAM_INT);
                $reportsStmt->bindParam(':lgp_discount_amount', $lgp_discount_amount_from_booking);
                $reportsStmt->bindParam(':lgp_approver_name', $lgp_approver_name_from_booking);
                $reportsStmt->bindParam(':vip_discount_enabled', $vip_discount_enabled_from_booking, PDO::PARAM_INT);
                $reportsStmt->bindParam(':vip_discount_amount', $vip_discount_amount_from_booking);
                $reportsStmt->bindParam(':vip_approver_name', $vip_approver_name_from_booking);
                $reportsStmt->bindParam(':long_discount_enabled', $long_discount_enabled_from_booking, PDO::PARAM_INT);
                $reportsStmt->bindParam(':long_discount_amount', $long_discount_amount_from_booking);
                $reportsStmt->bindParam(':long_discount_percent', $long_discount_percent_from_booking);
            ...
            $_methodsList = [];
            if ($_cashPaid > 0)
                $_methodsList[] = "Cash: ₱" . number_format($_cashPaid, 2);
            if ($_gcashPaid > 0)
                $_methodsList[] = "GCash: ₱" . number_format($_gcashPaid, 2);
            if ($_mayaPaid > 0)
                $_methodsList[] = "Maya: ₱" . number_format($_mayaPaid, 2);
            if ($_instapayPaid > 0)
                $_methodsList[] = "Instapay: ₱" . number_format($_instapayPaid, 2);
            if ($_onlinePaid > 0)
                $_methodsList[] = "Online Banking: ₱" . number_format($_onlinePaid, 2);
            if ($_airbnbPaid > 0)
                $_methodsList[] = "Airbnb: ₱" . number_format($_airbnbPaid, 2);

            $_paymentMethodsSummary = !empty($_methodsList) ? implode(', ', $_methodsList) : 'None/Unpaid';
            $_bookingTotal = max($full_amount_before_deposit ?? 0, $room_price ?? 0, $total_amount ?? 0);
            ...
            logActivity(
                ...
                'booking',
                'CREATE',
                "New {$booking_type} (ID: {$booking_id}) created by {$encoder} — Guest: {$guest_name}, Room: {$room_id} ({$room_type}), Duration: {$_effectiveDuration}, Booking Total: ₱" . number_format($_bookingTotal, 2) . ", Total Paid: ₱" . number_format($_totalPaidSum, 2) . " [Methods: {$_paymentMethodsSummary}], Discounts: {$_discountsSummary}, Extras: {$_extrasSummary}, Status: {$paid_status}",
                [
            ...
            logError($conn, 'booking', 'CREATE', "Failed to create booking for {$guest_name}", [
                'guest_name' => $guest_name,
                'room_id' => $room_id
            ]);
        }
        ...
        logError($conn, 'booking', 'CREATE', "Booking creation error for {$guest_name}: " . $e->getMessage(), [
            'guest_name' => $guest_name ?? '',
            'error' => $e->getMessage()
        ]);
    }
