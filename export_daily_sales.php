...
logActivity($conn, 'report', 'REPORT_EXPORT',
    "Daily Sales 2.0 report exported ({$_fmt}) by {$_exp_user} for {$startDate} to {$endDate}",
    ['start_date' => $startDate, 'end_date' => $endDate, 'format' => $_fmt, 'exported_by' => $_exp_user]
);
...
try {
    ...
    function getGuestPetCountsForPaymentRow(
        ...
        if ($guestCount === 0 && $cumulativeGuest > 0 && $nPaymentTimestamps <= 1) {
            if (
                shouldIncludeAdditionalOnPaymentRow(
    ...
    function buildBookingMetaFields(array $payment): array
    {
        ...
        return [
            ...
            'status' => (static function (string $s): string{
                if ($s === '')
                    return '—';
                return strcasecmp($s, 'Confirmed') === 0 ? 'Check-In' : $s;
    ...
    function getMethodAmountsForTimestamp(array $payment, array $timestampRows, int $idx): array
    {
        ...
        foreach ($methodConfigs as $key => $cfg) {
            $deposit = max(0, floatval($payment[$cfg['deposit']] ?? 0) - floatval($heldByMethod[$key] ?? 0));
...
if (empty($dataRows)) {
    ...
    foreach ($dataRows as $row) {
        ...
        echo '<td style="padding: 5px;">' . formatDetailedReportDiscountAmountDisplay(floatval($row['sc_pwd_discount_amount'] ?? 0)) . '</td>';
        echo '<td style="padding: 5px;">' . formatDetailedReportDiscountAmountDisplay(floatval($row['lgp_discount_amount'] ?? 0)) . '</td>';
        echo '<td style="padding: 5px;">' . htmlspecialchars(($row['lgp_approver_name'] ?? '') !== '' ? $row['lgp_approver_name'] : '—') . '</td>';
        echo '<td style="padding: 5px;">' . formatDetailedReportDiscountAmountDisplay(floatval($row['vip_discount_amount'] ?? 0)) . '</td>';
        echo '<td style="padding: 5px;">' . htmlspecialchars(($row['vip_approver_name'] ?? '') !== '' ? $row['vip_approver_name'] : '—') . '</td>';
        echo '<td style="padding: 5px;">' . htmlspecialchars(formatDetailedReportLongTermDiscountDisplay(
            floatval($row['long_discount_amount'] ?? 0),
            floatval($row['long_discount_percent'] ?? 0)
        )) . '</td>';
        echo '<td style="padding: 5px;">' . htmlspecialchars($row['discount_date'] ?? '—') . '</td>';
        // Convert newlines to <br> for guest name display
        echo '<td style="padding: 5px;">' . nl2br(htmlspecialchars($row['guest_name'])) . '</td>';
    ...
    echo '<td colspan="26" style="padding: 8px; text-align: left;">GRAND TOTAL</td>';
