...
header('Content-Disposition: attachment; filename="payment_type_report_' . date('Y-m-d') . '.xls"');
...
logActivity($conn, 'report', 'REPORT_EXPORT',
    "Payment Type report exported (XLS) by {$_exp_user} for {$startDate} to {$endDate}",
    ['start_date' => $startDate, 'end_date' => $endDate, 'format' => 'XLS', 'exported_by' => $_exp_user]
);

try {
    // Ensure payment amount history columns exist.
    // Without these, we can’t split amounts per payment_date_time timestamp accurately.
    $histColumns = [
    ...
    // Query to get payment data from reports table.
    // Do NOT require paid_status = 'Paid': add-ons can leave the booking "Unpaid" while prior payments
    // must still appear. Do not use DATE(payment_date_time) in SQL when values are pipe-separated
    // (MySQL may return NULL). Filter by calendar day in PHP instead.
    $stmt = $conn->prepare("
        SELECT 
            r.booking_id,
            r.payment_date_time,
    ...
    /**
     * Include a reports row when any recorded payment time (or downpayment / check-in) falls in the range.
     */
    ...
    foreach ($payments as $payment) {
        ...
        if (is_array($cashHistoryArr) && count($cashHistoryArr) === $nTimestamps) {
            ...
            // Self-heal: update_booking used to store the cumulative deposit as the last segment
            // when status flipped Unpaid → Paid after partial payments (e.g. "800|150|1250" vs total 1250).
            ...
            // Partial history backfill: infer first amount as (total - last).
            $lastCash = floatval($cashHistoryArr[0] ?? 0);
            $firstCash = max(0, $totalCash - $lastCash);
            $cashAmountsByTimestamp = [$firstCash, $lastCash];
        } else {
            // Fallback heuristic (legacy data without full history)
            $cashMethodAmounts = parsePaymentAmountsAll($payment['payment_status_cash']);

            // Extra fallback: when there are exactly 2 timestamps, many legacy flows store
            // the "latest payment amount" in payment_status_*. If so, split as (total - last), last.
    ...
    // Filter rows by the Payment Date & Time column to match the selected date range
    $filteredRows = [];
    try {
        $start = new DateTime($startDate . ' 00:00:00');
        $end = new DateTime($endDate . ' 23:59:59');
        ...
        foreach ($dataRows as $row) {
            $rowDateTime = $row['payment_date_time'];
            // Parse the date time in m/d/Y h:i a format
            try {
                $dt = DateTime::createFromFormat('m/d/Y h:i a', $rowDateTime);
                if (!$dt) {
                    // Try without time if parsing fails
                    $dt = DateTime::createFromFormat('m/d/Y', $rowDateTime);
                }
    ...
    echo '<tr><td colspan="7" style="background-color: #4CAF50; color: white; font-weight: bold; font-size: 16px; text-align: center; padding: 10px;">Per Payment Report</td></tr>';
    echo '<tr><td colspan="7" style="text-align: center; padding: 5px;">Generated on: ' . $currentTime . '</td></tr>';
    echo '<tr><td colspan="7" style="text-align: center; padding: 5px; font-weight: bold; color: #256d27;">Date Range: ' . htmlspecialchars($startDate) . ' to ' . htmlspecialchars($endDate) . '</td></tr>';
    echo '<tr><td colspan="7"></td></tr>';

    // Table Header
    echo '<tr style="background-color: #4CAF50; color: white; font-weight: bold;">';
    echo '<th style="padding: 8px;">Booking ID</th>';
    echo '<th style="padding: 8px;">Payment Date & Time</th>';
    echo '<th style="padding: 8px;">Customer Name</th>';
    echo '<th style="padding: 8px;">Room Number</th>';
    echo '<th style="padding: 8px;">Status</th>';
    echo '<th style="padding: 8px;">Payment Method</th>';
    echo '<th style="padding: 8px;">Amount Paid</th>';
    echo '</tr>';

    // Data Rows
    if (empty($dataRows)) {
        echo '<tr><td colspan="7" style="text-align: center; padding: 20px; color: #999;">No payment records found for the selected date range.</td></tr>';
    } else {
        ...
        foreach ($groupedRows as $bookingId => $rows) {
            ...
            foreach ($rows as $index => $row) {
                echo '<tr>';
                // Only show booking_id in the first row of the group
                if ($index === 0) {
                    echo '<td style="padding: 8px; vertical-align: top;" rowspan="' . $rowCount . '">' . htmlspecialchars($row['booking_id']) . '</td>';
                }
                echo '<td style="padding: 8px; mso-number-format:\'\@\';">' . htmlspecialchars($row['payment_date_time']) . '</td>';
                // Convert newlines to <br> for guest name display
                echo '<td style="padding: 8px;">' . nl2br(htmlspecialchars($row['guest_name'])) . '</td>';
                echo '<td style="padding: 8px;">' . htmlspecialchars($row['room_id']) . '</td>';
                echo '<td style="padding: 8px;">' . htmlspecialchars($row['status']) . '</td>';
                echo '<td style="padding: 8px;">' . htmlspecialchars($row['payment_method']) . '</td>';
                echo '<td style="padding: 8px; text-align: right;">₱' . number_format($row['amount'], 2) . '</td>';
                echo '</tr>';
            }
        }
        ...
        echo '<td colspan="6" style="padding: 10px; text-align: right;">Grand Total:</td>';
        echo '<td style="padding: 10px; text-align: right; font-size: 14px;">₱' . number_format($grandTotal, 2) . '</td>';
        echo '</tr>';
    }
