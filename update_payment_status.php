...
try {
    ...
    if ($stmt->execute()) {
        ...
        logActivity($conn, 'update', 'PAYMENT_STATUS_UPDATE',
            "Payment status for booking #{$booking_id} updated to '{$paid_status}' by {$_actor}",
            ['booking_id' => $booking_id, 'paid_status' => $paid_status, 'updated_by' => $_actor]
        );
    } else {
