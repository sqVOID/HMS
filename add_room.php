...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Ensure duration_unit column exists for existing installs
    try {
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            $durationStmt = $conn->prepare("INSERT INTO room_durations (room_id, duration_hours, price, duration_unit) VALUES (:room_id, :duration_hours, :price, :duration_unit) ON DUPLICATE KEY UPDATE price = :update_price, duration_unit = :update_unit");

            foreach ($durations as $dur) {
                $hours = intval($dur['hours']);
                $price = floatval($dur['price']);
                $unit = (isset($dur['unit']) && $dur['unit'] === 'days') ? 'days' : 'hours';
            ...
            logActivity($conn, 'room', 'ROOM_CREATE',
                "Room '{$room_id}' (Type: {$room_type}, Capacity: {$guest_capacity}) created by {$_actor}",
                [
                    'room_db_id'     => $newRoomId,
                    'room_number'    => $room_id,
                    'room_type'      => $room_type,
        ...
        logError($conn, 'room', 'ROOM_CREATE', "Failed to add room '{$room_id}': " . $e->getMessage(), [
            'room_number' => $room_id,
            'created_by'  => $_actor,
            'error'       => $e->getMessage()
        ]);
