...
if (!function_exists('buildDetailedBookingReportData')) {
    ...
    $stmt = $conn->prepare("
        SELECT 
            ...
            COALESCE(r.second_guest_name, b.second_guest_name, '') AS second_guest_name,
            COALESCE(r.additional_guest_names, b.additional_guest_names, '') AS additional_guest_names,
            r.room_id,
            ...
            COALESCE(b.lgp_discount_enabled, 0) AS lgp_discount_enabled,
            COALESCE(b.lgp_discount_amount, 0) AS lgp_discount_amount,
            COALESCE(b.lgp_approver_name, '') AS lgp_approver_name,
            COALESCE(b.vip_discount_enabled, 0) AS vip_discount_enabled,
            COALESCE(b.vip_discount_amount, 0) AS vip_discount_amount,
            COALESCE(b.vip_approver_name, '') AS vip_approver_name,
            COALESCE(b.long_discount_enabled, 0) AS long_discount_enabled,
            COALESCE(b.long_discount_amount, 0) AS long_discount_amount,
            COALESCE(b.long_discount_percent, 0) AS long_discount_percent,
            ...
            GREATEST(COALESCE(r.additional_slippers, 0), COALESCE(b.additional_slippers, 0)) AS additional_slippers,
            COALESCE(r.slipper_status, b.slipper_status) AS slipper_status,
            COALESCE(r.new_deposit, b.new_deposit, 0) AS new_deposit,
            COALESCE(r.new_deposit_cash, b.new_deposit_cash, 0) AS new_deposit_cash,
            COALESCE(r.new_deposit_g_cash, b.new_deposit_g_cash, 0) AS new_deposit_g_cash,
            COALESCE(r.new_deposit_maya, b.new_deposit_maya, 0) AS new_deposit_maya,
            COALESCE(r.new_deposit_instapay, b.new_deposit_instapay, 0) AS new_deposit_instapay,
            COALESCE(r.new_deposit_online_banking, b.new_deposit_online_banking, 0) AS new_deposit_online_banking,
            COALESCE(r.new_deposit_airbnb, b.new_deposit_airbnb, 0) AS new_deposit_airbnb,
    ...
    foreach ($payments as &$paymentRow) {
        attachReportRoomRates($paymentRow, $durationMaps);
    }
    unset($paymentRow);
    ...
    foreach ($payments as $payment) {
        ...
        $cashAmountsByTimestamp = allocatePaymentMethodAmountsByHistory(
            $payment,
            $timestampRows,
            'payment_amount_cash_history',
            'payment_status_cash',
        ...
        $cashLikeMethodLabel = function_exists('resolvePaymentExportCashLikeLabel')
            ? resolvePaymentExportCashLikeLabel($payment)
            : 'Cash';
        ...
        foreach ($timestampRows as $idx => $tsRow) {
            ...
            if ($amt > 0 && isDateInRange($tsRow['date'], $startDate, $endDate)) {
                ...
                $discountBreakdown = getTypedDiscountBreakdownForPaymentTimestamp($payment, $paymentRaw, $nTimestamps);
                $row = array_merge([
                    ...
                    'check_in' => $checkIn,
                    'check_out' => $checkOut,
                    'sc_pwd_discount_amount' => $discountBreakdown['sc_pwd_discount_amount'],
                    'lgp_discount_amount' => $discountBreakdown['lgp_discount_amount'],
                    'lgp_approver_name' => $discountBreakdown['lgp_approver_name'],
                    'vip_discount_amount' => $discountBreakdown['vip_discount_amount'],
                    'vip_approver_name' => $discountBreakdown['vip_approver_name'],
                    'long_discount_amount' => $discountBreakdown['long_discount_amount'],
                    'long_discount_percent' => $discountBreakdown['long_discount_percent'],
        ...
        $paymentMethods = [
            ['name' => 'G-Cash', 'deposit' => 'deposit_g_cash', 'downpayment' => 'downpayment_gcash', 'status' => 'payment_status_g_cash', 'history' => 'payment_amount_g_cash_history', 'held' => 'gcash'],
            ['name' => 'Maya', 'deposit' => 'deposit_maya', 'downpayment' => 'downpayment_maya', 'status' => 'payment_status_maya', 'history' => 'payment_amount_maya_history', 'held' => 'maya'],
            ['name' => 'Instapay', 'deposit' => 'deposit_instapay', 'downpayment' => 'downpayment_instapay', 'status' => 'payment_status_instapay', 'history' => 'payment_amount_instapay_history', 'held' => 'instapay'],
            ['name' => 'Online Banking', 'deposit' => 'deposit_online_banking', 'downpayment' => 'downpayment_online_banking', 'status' => 'payment_status_online_banking', 'history' => 'payment_amount_online_banking_history', 'held' => 'online_banking'],
            ['name' => 'Airbnb', 'deposit' => 'deposit_airbnb', 'downpayment' => 'downpayment_airbnb', 'status' => 'payment_status_airbnb', 'history' => 'payment_amount_airbnb_history', 'held' => 'airbnb'],
        ];

        foreach ($paymentMethods as $method) {
            $total = resolveDetailedBookingMethodTotal($payment, $method['deposit'], $method['downpayment'], $method['held']);
            $amountsByTimestamp = allocatePaymentMethodAmountsByHistory(
                $payment,
                $timestampRows,
                $method['history'],
                $method['status'],
