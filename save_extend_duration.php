...
try {
    ...
    // Frontend often sends MySQL 24h ("Y-m-d H:i:s") via displayDateTimeToMysql();
    // older paths send 12h display strings. Reservation + Regular may have NULL check_out.
    $parseExtendCheckout = function ($value) {
        ...
        foreach ($formats as $fmt) {
            ...
            if ($dt instanceof DateTime) {
                ...
                $warn = is_array($errors) ? (($errors['warning_count'] ?? 0) + ($errors['error_count'] ?? 0)) : 0;
                if ($warn === 0) {
    ...
    $checkoutDateTime = $parseExtendCheckout($newCheckout);

    // Fallback: current check_out + this extension
    if (!$checkoutDateTime && !empty($booking['check_out'])) {
        try {
    ...
    // Fallback for Reservation + Regular (and any row with NULL check_out):
    // check_in + base duration + previous extensions + this extension
    if (!$checkoutDateTime && !empty($booking['check_in'])) {
        try {
            $baseDuration = intval($booking['duration'] ?? 0);
            $durationUnit = strtolower(trim((string)($booking['duration_unit'] ?? 'hours')));
            if ($durationUnit === 'night' || $durationUnit === 'nights') {
                $baseDuration = $baseDuration * 12;
            } elseif ($durationUnit === 'day' || $durationUnit === 'days') {
                $baseDuration = $baseDuration * 24;
            }

            // Bundle/promo hours when duration was not stored (common for Bundle)
            $promo = trim((string)($booking['promo'] ?? ''));
            if ($promo !== '' && !in_array($promo, ['None', 'Regular', 'Select Bundle', 'Select Promo'], true)) {
                if (preg_match('/(\d+)\s*hrs?/i', $promo, $promoMatch)) {
                    $promoHours = intval($promoMatch[1]);
                    if ($promoHours > 0) {
                        $baseDuration = $promoHours;
                    }
                }
            }

            $prevExtendHours = intval($booking['extend_hours'] ?? 0);
            $prevExtendMinutes = intval($booking['extend_minutes'] ?? 0);
            $totalHours = max(0, $baseDuration) + max(0, $prevExtendHours) + max(0, $extendHours);
            $totalMinutes = max(0, $prevExtendMinutes) + max(0, $extendMinutes);
            if ($totalMinutes >= 60) {
                $totalHours += intdiv($totalMinutes, 60);
                $totalMinutes = $totalMinutes % 60;
            }

            $computed = new DateTime($booking['check_in']);
            if ($totalHours > 0) {
                $computed->modify("+{$totalHours} hours");
            }
            if ($totalMinutes > 0) {
                $computed->modify("+{$totalMinutes} minutes");
            }
            $checkoutDateTime = $computed->format('Y-m-d H:i:s');
        } catch (Exception $e) {
            $checkoutDateTime = null;
        }
    }
    ...
    if (!$checkoutDateTime) {
        echo json_encode(['success' => false, 'message' => 'Unable to resolve new check-out time for this extension.']);
        exit;
    }
    ...
    if ($updateStmt->execute()) {
        ...
        logActivity($conn, 'update', 'EXTEND_DURATION_SAVE',
            "Extension saved for booking by {$currentEncoder} — +{$totalExtendHours}h {$totalExtendMinutes}m, Price: ₱" . number_format($totalExtendPrice, 2),
            ['extend_hours' => $totalExtendHours, 'extend_minutes' => $totalExtendMinutes, 'price' => $totalExtendPrice, 'saved_by' => $currentEncoder]
        );
        echo json_encode([
            'success'            => true,
