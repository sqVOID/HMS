...
// Verify that the current session has not been terminated (e.g. by LOGOUT ALL ACCOUNTS)
if (isset($_SESSION['session_id']) && !empty($_SESSION['session_id'])) {
    try {
        require_once 'config.php';
        $sCheckStmt = $conn->prepare("SELECT session_status FROM user_sessions WHERE id = ? LIMIT 1");
        $sCheckStmt->execute([$_SESSION['session_id']]);
        $sStatus = $sCheckStmt->fetchColumn();
        ...
        if ($sStatus === 'logged_out') {
            ...
            @session_destroy();
            header('Location: Login.html?logout=forced');
            exit;
        }
    } catch (Throwable $e) {
        // Silently continue if session status check fails
    }
}
...
function getPageAccessRules() {
    return [
        ...
        'SystemLog.php'      => ['super_admin'],           // Super Admin only
    ];
}
