...
try {
    ...
    $durationStmt = $conn->prepare("SELECT duration_hours, price, duration_unit FROM room_durations WHERE room_id = :room_id ORDER BY duration_hours ASC");
    
    foreach ($rooms as &$room) {
        $durationStmt->execute([':room_id' => $room['id']]);
        $durations = $durationStmt->fetchAll(PDO::FETCH_ASSOC);
        foreach ($durations as &$dur) {
            $dur['unit'] = (isset($dur['duration_unit']) && $dur['duration_unit'] === 'days') ? 'days' : 'hours';
        }
        unset($dur);
