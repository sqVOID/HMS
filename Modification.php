...
<head>
    ...
    <link rel="stylesheet" href="includes/global_layout.css?v=19">
    <!-- Page-specific Styles -->
    <link rel="stylesheet" href="Booking.css?v=19">
    <style>
    ...
    <script>
        ...
        function getBookingForPaymentLines(booking, paymentIndex, totalPayments) {
            ...
            const durSel = document.getElementById(`payHist_duration_${paymentIndex}`);
            const selectedOpt = durSel?.options?.[durSel.selectedIndex];
            // Option values are always hours (from room_durations); keep unit as hours for math
            return {
                ...booking,
                duration: meta.duration || booking.duration,
                duration_unit: selectedOpt?.dataset?.unit === 'days' ? 'hours' : (booking.duration_unit || 'hours'),
                // Preserve rates so getRoomRateInfo can still resolve Day labels
                rates: booking.rates,
                promo: meta.promo,
                breakfast: meta.breakfast,
                extend_bundle_breakfast: '',
                _selectedDurationUnit: selectedOpt?.dataset?.unit || ''
            };
        }

        function ensurePayHistDurationOption(durSel, hours, price, unit) {
            if (!durSel || !hours) return;
            const hoursStr = String(hours);
            let opt = Array.from(durSel.options).find(o => o.value === hoursStr);
            const resolvedUnit = unit || ((hours >= 24 && hours % 24 === 0) ? 'days' : 'hours');
            if (!opt) {
                opt = document.createElement('option');
                opt.value = hoursStr;
                durSel.appendChild(opt);
            }
            opt.dataset.unit = resolvedUnit;
            if (price > 0) opt.dataset.price = String(price);
            opt.textContent = formatPricingDurationOptionLabel(hours, price || 0, resolvedUnit);
        }

        function applyPayHistDurationSelect(idx, booking, totalPayments, preferredHours) {
            const durSel = document.getElementById(`payHist_duration_${idx}`);
            if (!durSel || !booking) return;
            ...
            if (isExt) {
                const ext = getPerPaymentExtendForDisplay(booking, idx, totalPayments);
                const extH = parseInt(ext.extend_hours, 10) || 0;
                const extM = parseInt(ext.extend_minutes, 10) || 0;
                const extPrice = parseFloat(ext.extend_regular_rate) || parseFloat(ext.extend_bundle_rate) || 0;
                if (extH > 0) {
                    const extBooking = {
                        ...booking,
                        extend_regular_rate: ext.extend_regular_rate || booking.extend_regular_rate,
                        extend_price: extPrice || booking.extend_price
                    };
                    const useDays = formatExtendDurationLabelMod(extBooking, extH, extM).includes('Day');
                    ensurePayHistDurationOption(durSel, extH, extPrice, useDays ? 'days' : 'hours');
                    durSel.value = String(extH);
                } else {
                    durSel.value = '';
                }
                return;
            ...
            const checkInIdx = getCheckInPaymentIndex(booking, totalPayments);
            const target = meta.duration || (idx === checkInIdx ? String(preferredHours || '') : '');
            if (target && target !== '0') {
                durSel.value = target;
                if (durSel.value !== String(target) && preferredHours && idx === checkInIdx) {
                    durSel.value = String(preferredHours);
                }
            } else {
                durSel.value = '';
            }
        ...
        function populatePayHistMetaSelects(idx, booking, totalPayments, preferredHours) {
            const meta = getPerPaymentMetaForDisplay(booking, idx, totalPayments);
            const durSel = document.getElementById(`payHist_duration_${idx}`);
            ...
            if (durSel && modalDur) {
                ...
                applyPayHistDurationSelect(idx, booking, totalPayments, preferredHours);
            }
        ...
        function updatePayHistExtend(idx) {
            ...
            if (booking) {
                ...
                const durSel = document.getElementById(`payHist_duration_${idx}`);
                if (durSel && isExtensionPaymentIndex(booking, idx, count)) {
                    const ext = getPayHistExtendForIndex(idx);
                    const extH = parseInt(ext.extend_hours, 10) || 0;
                    const extM = parseInt(ext.extend_minutes, 10) || 0;
                    const extPrice = parseFloat(ext.extend_regular_rate) || parseFloat(ext.extend_bundle_rate) || 0;
                    if (extH > 0) {
                        const extBooking = {
                            ...booking,
                            extend_regular_rate: ext.extend_regular_rate,
                            extend_price: calcExtendPriceFromFields(ext)
                        };
                        const useDays = formatExtendDurationLabelMod(extBooking, extH, extM).includes('Day');
                        ensurePayHistDurationOption(durSel, extH, extPrice, useDays ? 'days' : 'hours');
        ...
        function getModDurationHours(mod) {
            const dur = parseFloat(mod?.duration) || 0;
            const unit = String(mod?.duration_unit || 'hours').toLowerCase();
            if (unit === 'night' || unit === 'nights') return dur * 12;
            if (unit === 'day' || unit === 'days') return dur * 24;
            return dur;
        }

        /** Label hours using room rate unit (days vs hours), same rules as Booking.html */
        function resolveModDurationLabel(mod, hours) {
            const h = parseFloat(hours);
            const safeHours = !isNaN(h) ? h : getModDurationHours(mod);
            const rates = Array.isArray(mod?.rates) ? mod.rates : [];
            const isDayRate = (r) => !!(r && (r.unit === 'days' || r.duration_unit === 'days'));
            const match = rates.find(r => parseFloat(r.duration_hours) === safeHours);

            let useDays = false;
            if (match) {
                useDays = isDayRate(match);
            } else if (safeHours >= 24 && safeHours % 24 === 0) {
                const dayRates = rates.filter(isDayRate);
                const bookingUnit = String(mod?.duration_unit || '').toLowerCase();
                if (bookingUnit === 'day' || bookingUnit === 'days') {
                    useDays = true;
                } else if (dayRates.length > 0) {
                    const originalHours = getModDurationHours(mod);
                    const originalWasDays = dayRates.some(r => parseFloat(r.duration_hours) === originalHours);
                    const dayRateDividesTotal = dayRates.some(r => {
                        const rh = parseFloat(r.duration_hours) || 0;
                        return rh > 0 && safeHours % rh === 0;
                    });
                    useDays = originalWasDays || dayRateDividesTotal;
                }
            }

            if (useDays && safeHours >= 24 && safeHours % 24 === 0) {
        ...
        /** Label for extend_hours only — mirrors Booking.html formatExtendDurationLabel */
        function formatExtendDurationLabelMod(booking, extendHours, extendMinutes = 0) {
            const hours = parseInt(extendHours) || 0;
            const minutes = parseInt(extendMinutes) || 0;
            ...
            const resolved = resolveModDurationLabel(booking, hours);
            if (resolved.unit === 'days') return resolved.text;

            if (hours >= 24 && hours % 24 === 0) {
                const days = hours / 24;
                const rates = Array.isArray(booking?.rates) ? booking.rates : [];
                const extPrice = parseFloat(booking?.extend_regular_rate || booking?.extend_price || 0) || 0;
                const dayRates = rates.filter(r => r && (r.unit === 'days' || r.duration_unit === 'days'));
                const exactDayHours = dayRates.some(r => parseFloat(r.duration_hours) === hours);
                const priceMatchedDay = extPrice > 0 && dayRates.some(r =>
                    Math.abs(parseFloat(r.price) - extPrice) < 0.01
                );
                const dividesDayPackage = dayRates.some(r => {
                    const rh = parseFloat(r.duration_hours) || 0;
                    return rh > 0 && hours % rh === 0;
                });
        ...
        function formatExtendedRateLabel(booking, extendHours, extendMinutes = 0) {
            const durationText = formatExtendDurationLabelMod(booking, extendHours, extendMinutes);
            return durationText ? `Extended Rate (${durationText})` : 'Extended Rate';
        }

        function formatStayDurationDisplayMod(mod, wholeHours, remainingMinutes, extendHours, extendMinutes) {
            const totalHours = parseInt(wholeHours) || 0;
            const remMins = parseInt(remainingMinutes) || 0;
            const extH = parseInt(extendHours) || 0;
            const extM = parseInt(extendMinutes) || 0;
            const hasExtend = extH > 0 || extM > 0;

            if (hasExtend) {
                let baseHours = getModDurationHours(mod);
                const fromDatesBase = Math.max(0, totalHours - extH);
                if (!baseHours || baseHours <= 0) {
                    baseHours = fromDatesBase;
                } else if (Math.abs((baseHours + extH) - totalHours) > 1) {
                    baseHours = fromDatesBase;
                }

                const baseResolved = resolveModDurationLabel(mod, baseHours);
                const totalResolved = resolveModDurationLabel(mod, totalHours);
                ...
                if (baseResolved.unit === 'days') {
                    const parts = [baseResolved.text];
                    if (extH > 0) {
                        const extResolved = resolveModDurationLabel(mod, extH);
                        if (extResolved.unit === 'days') {
                            parts.push(extResolved.text);
                        } else {
                            parts.push(`${extH} Hr${extH !== 1 ? 's' : ''}`);
                        }
                    }
                    const leftoverMins = extM > 0 ? extM : remMins;
                    if (leftoverMins > 0) {
                        parts.push(`${leftoverMins} Min${leftoverMins !== 1 ? 's' : ''}`);
                    }
                    return `${parts.join(' ')} (Extended)`;
                }
            }

            let text;
            if (remMins > 0) {
                text = `${totalHours}:${String(remMins).padStart(2, '0')} Hours`;
            } else {
                text = resolveModDurationLabel(mod, totalHours).text;
            }
            if (hasExtend) {
                text += ' (Extended)';
            }
        ...
        function normalizePromoDisplay(promo) {
            const p = String(promo || '').trim();
            if (!p || p === 'None' || p === '-' || p === 'Regular' ||
                p === 'Select Bundle' || p === 'Select Promo') {
                return '-';
            }
            return p;
        }

        function formatDurationDisplay(durationOrMod, promo, extendHours, extendMinutes) {
            // Prefer full mod object (has duration_unit, rates, check_in/out)
            const mod = (durationOrMod && typeof durationOrMod === 'object')
                ? durationOrMod
                : {
                    duration: durationOrMod,
                    promo: promo,
            ...
            const extHrs = parseInt(mod.extend_hours) || 0;
            const extMins = parseInt(mod.extend_minutes) || 0;
            const promoVal = mod.promo;

            // Promo bookings stay hour-based
            if (promoVal && promoVal !== 'None' && promoVal !== '-' && promoVal !== 'Regular' && promoVal !== 'Select Bundle') {
                const promoMatch = String(promoVal).match(/(\d+)\s*hrs?/i);
                if (promoMatch) {
                    const base = parseInt(promoMatch[1], 10) || 0;
                    const total = base + extHrs;
                    if (extHrs > 0 || extMins > 0) {
                        return extMins > 0
                            ? `${total} Hrs ${extMins} Mins (Extended)`
                            : `${total} Hrs (Extended)`;
                    }
                    return `${base} Hrs`;
                }
            ...
            if (mod.check_in && mod.check_out && mod.check_in !== '-' && mod.check_out !== '-') {
                const start = new Date(mod.check_in);
                const end = new Date(mod.check_out);
                if (!isNaN(start.getTime()) && !isNaN(end.getTime()) && end > start) {
                    const diffHrs = (end - start) / (1000 * 60 * 60);
                    const wholeHours = Math.floor(diffHrs);
                    const remainingMinutes = Math.round((diffHrs - wholeHours) * 60);
                    return formatStayDurationDisplayMod(mod, wholeHours, remainingMinutes, extHrs, extMins);
                }
            }

            const baseHours = getModDurationHours(mod);
            return formatStayDurationDisplayMod(mod, baseHours + extHrs, extMins, extHrs, extMins);
        }
                ...
                .then(text => {
                    ...
                    if (data.modifications && data.modifications.length > 0) {
                        ...
                        data.modifications.forEach(mod => {
                            ...
                            } else if (mod.status === 'Checked Out') {
                                statusClass = 'checked-out';
                                statusStyle = 'background: #757575 !important; color: white !important; border: 1px solid #757575 !important;';
                            ...
                            row.innerHTML = `
                                ...
                                <td>${formatDurationDisplay(mod)}</td>
                                <td>${formatTableDateTime(mod.check_out)}</td>
                                <td>${mod.referral_code || '-'}</td>
                                <td>${normalizePromoDisplay(mod.promo)}</td>
                                ...
                                <td>
                                    <div class="action-buttons-container" style="display: flex; gap: 6px; justify-content: center;">
                                        ...
                                        ${(mod.status === 'Checked Out' || (mod.checked_out_at && mod.checked_out_at.trim() !== '')) ? `
                                        <button class="action-icon-btn cancel-checkout-btn" title="Cancel Check-Out" style="background: #e65100 !important; color: white !important;" onclick="cancelCheckout('${mod.id}', '${mod.source_table || mod.source || ''}')">
...
<body>
    <div class="split-container">
        ...
        <div class="right-panel">
            ...
            <div class="content-container">
                ...
                <div class="calendar-addroom-container"
                    style="width: 100%; justify-content: flex-start; margin-top: 8px; margin-bottom: 0px;">
    ...
    <script>
        ...
        function cancelCheckout(id, source) {
            if (!confirm('Are you sure you want to cancel the check-out for this booking?\nThe status will be set back to Occupied, room status will be updated to Occupied, and checked_out_at will be cleared.')) {
                return;
            }
            ...
            fetch('cancel_checkout.php', {
                ...
                .then(data => {
                    if (data.success) {
                        alert(data.message || 'Check-out canceled successfully.');
                        ...
                        alert('Error canceling check-out: ' + (data.error || 'Unknown error'));
                    }
                })
                .catch(err => {
                    console.error('Error canceling check-out:', err);
                    alert('An unexpected error occurred while canceling check-out.');
                });
        }

        function triggerModalCancelCheckout() {
            if (window.currentBooking && window.currentBooking.id) {
                cancelCheckout(window.currentBooking.id, window.currentBooking.source_table || window.currentBooking.source);
                closeEditModal();
            }
        }
        ...
        function showEditModal(booking) {
            ...
            // Guest Names – generate individual fields from saved data
            generateModalGuestNameFields(booking);
            generateModalAdditionalGuestNameFields(booking);
            ...
            document.getElementById('modalCheckedOutAt').value = booking.checked_out_at ? booking.checked_out_at.replace(' ', 'T').substring(0, 16) : '';

            // Toggle Cancel Check-Out button in edit modal
            const cancelBtn = document.getElementById('modalCancelCheckoutBtn');
            if (cancelBtn) {
                if (booking.status === 'Checked Out' || (booking.checked_out_at && booking.checked_out_at.trim() !== '')) {
            ...
            const modalSlippersEl = document.getElementById('modalAdditionalSlippers');
            if (modalSlippersEl) modalSlippersEl.value = booking.additional_slippers || '0';
            ...
            loadDurationOptions(booking.room, booking.room_id, currentDuration, booking);
            ...
            // Populate SCW/PWD discount hidden fields (synced from payment cards)
            try {
                const modalDiscCount = document.getElementById('modalDiscountCount');
                const modalDiscAmount = document.getElementById('modalDiscountAmount');
                const modalDiscId = document.getElementById('modalDiscountId');
                if (modalDiscCount) modalDiscCount.value = booking.sc_pwd_count || '';
                if (modalDiscAmount) modalDiscAmount.value = booking.discount_amount || '';
                if (modalDiscId) modalDiscId.value = booking.id_number || '';
            } catch (discPopulateErr) {
                console.warn('Discount fields populate skipped:', discPopulateErr);
            }
        ...
        function getRoomRateInfo(booking, paymentIndex) {
            let durationVal = parseFloat(booking.duration) || 0;
            ...
            if (booking.promo && booking.promo.trim() !== '' && booking.promo !== 'Regular' && booking.promo !== 'Select Bundle') {
                ...
                if (promoMatch) {
                    durationVal = parseInt(promoMatch[2], 10);
            ...
            // Duration select values are always hours; duration_unit days means stored amount is in days
            let hours = durationVal;
            if (!hasPromo && (durationUnit === 'day' || durationUnit === 'days')) {
                hours = durationVal * 24;
            } else if (!hasPromo && (durationUnit === 'night' || durationUnit === 'nights')) {
                hours = durationVal * 12;
            }

            // Prefer selected duration option (per-payment payHist / modal) for price + unit
            const payHistSel = paymentIndex !== undefined && paymentIndex !== null
                ? document.getElementById(`payHist_duration_${paymentIndex}`)
                : null;
            const durSel = payHistSel || document.getElementById('payHist_duration_0') || document.getElementById('modalDuration');
            const opt = durSel?.options?.[durSel.selectedIndex];
            if (opt && opt.value) {
                const optHours = parseFloat(opt.value) || 0;
                if (optHours > 0) hours = optHours;
                const dataPrice = parseFloat((opt.dataset?.price || '').toString().replace(/,/g, '')) || 0;
                if (dataPrice > 0) roomPrice = dataPrice;
                else if (roomPrice <= 0) {
                    const priceMatch = (opt.textContent || '').match(/₱\s*([0-9,.]+)/);
                    if (priceMatch) roomPrice = parseFloat(priceMatch[1].replace(/,/g, '')) || 0;
                }
            }

            if (roomPrice <= 0 && hours > 0) {
                // keep roomPrice from booking if option had none
                roomPrice = parseFloat(booking.room_price) || 0;
            }

            // Day vs Hours from room rates (same as Booking editDuration) or option dataset
            let useDays = !!(opt && opt.dataset && opt.dataset.unit === 'days')
                || booking?._selectedDurationUnit === 'days';
            const rates = Array.isArray(booking?.rates) ? booking.rates : [];
            if (!useDays && hours > 0 && rates.length > 0) {
                const match = rates.find(r => parseFloat(r.duration_hours) === hours);
                if (match && (match.unit === 'days' || match.duration_unit === 'days')) {
                    useDays = true;
                }
            }
            if (!useDays && (durationUnit === 'day' || durationUnit === 'days') && hours >= 24 && hours % 24 === 0) {
                useDays = true;
            }

            let displayDuration = hours;
            let unitLabel = hours === 1 ? 'Hour' : 'Hours';
            if (useDays && hours >= 24 && hours % 24 === 0) {
                displayDuration = hours / 24;
                unitLabel = displayDuration === 1 ? 'Day' : 'Days';
            } else if (durationUnit === 'night' || durationUnit === 'nights') {
                displayDuration = hours / 12;
                unitLabel = displayDuration === 1 ? 'Night' : 'Nights';
            }
            ...
            return { label, roomPrice, displayDuration, unitLabel, hasPromo, hours };
        }
        ...
        function getSlipperCountForBreakdown(booking) {
            let maxFromUi = 0;
            let i = 0;
            while (document.getElementById(`payHist_add_guest_${i}`) !== null || document.getElementById(`payHist_add_slippers_${i}`) !== null) {
                const slipEl = document.getElementById(`payHist_add_slippers_${i}`);
                if (slipEl) {
                    maxFromUi = Math.max(maxFromUi, parseInt(slipEl.value, 10) || 0);
                }
                i++;
            }
            if (i > 0) return maxFromUi;
            return Math.max(0, parseInt(booking?.additional_slippers, 10) || 0);
        }

        function getSlipperStatusLabel(booking) {
            const status = String(booking?.slipper_status || 'added').toLowerCase();
            if (status === 'missing') return 'Missing';
            if (status === 'returned') return 'Returned';
            return 'Deposit';
        }

        function getSlipperDepositAmount(booking) {
            const count = getSlipperCountForBreakdown(booking);
            if (count <= 0) return 0;
            if (getSlipperStatusLabel(booking) === 'Returned') return 0;

            const held =
                (parseFloat(booking?.new_deposit_cash) || 0) +
                (parseFloat(booking?.new_deposit_g_cash) || 0) +
                (parseFloat(booking?.new_deposit_maya) || 0) +
                (parseFloat(booking?.new_deposit_instapay) || 0) +
                (parseFloat(booking?.new_deposit_online_banking) || 0) +
                (parseFloat(booking?.new_deposit_airbnb) || 0);
            const stored = parseFloat(booking?.new_deposit) || 0;
            if (held > 0.005) return held;
            if (stored > 0.005) return stored;
            return count * 100;
        }

        function resolvePaymentTotalsForSlipper(paymentTotals, totalPayments, paymentTotal, paymentIndex) {
            if (Array.isArray(paymentTotals) && paymentTotals.length > 0) {
                return paymentTotals.map(v => parseFloat(v) || 0);
            }
            if (typeof collectPayHistPaymentTotals === 'function') {
                try {
                    const live = collectPayHistPaymentTotals();
                    if (Array.isArray(live) && live.length > 0) return live;
                } catch (e) { /* ignore */ }
            }
            const count = Math.max(1, totalPayments || 1);
            const totals = new Array(count).fill(0);
            totals[paymentIndex] = parseFloat(paymentTotal) || 0;
            return totals;
        }

        /** Prefer last payment whose amount equals the slipper deposit (e.g. 2nd = ₱100). */
        function findSlipperPaymentIndex(booking, paymentTotals) {
            const slipAmt = getSlipperDepositAmount(booking);
            if (slipAmt <= 0.005 || !Array.isArray(paymentTotals) || paymentTotals.length === 0) {
                return -1;
            }
            let match = -1;
            for (let i = 0; i < paymentTotals.length; i++) {
                if (Math.abs((parseFloat(paymentTotals[i]) || 0) - slipAmt) < 0.02) {
                    match = i;
                }
            }
            return match;
        ...
        function isPureSlipperPayment(booking, paymentIndex, paymentTotal, paymentTotals, totalPayments) {
            const slipAmt = getSlipperDepositAmount(booking);
            if (slipAmt <= 0.005) return false;
            if (Math.abs((parseFloat(paymentTotal) || 0) - slipAmt) >= 0.02) return false;

            const totals = resolvePaymentTotalsForSlipper(paymentTotals, totalPayments, paymentTotal, paymentIndex);
            const slipIdx = findSlipperPaymentIndex(booking, totals);
            if (slipIdx >= 0) return paymentIndex === slipIdx;

            // Single payment that is only slipper amount
            return totals.length <= 1 || paymentIndex === totals.length - 1;
        }

        function shouldShowSlipperOnPaymentCard(booking, paymentIndex, totalPayments, paymentTotals, paymentTotal) {
            const slipAmt = getSlipperDepositAmount(booking);
            if (slipAmt <= 0.005) return false;
            ...
            // No separate ₱100 payment yet — keep slipper off room-only payments
            if (totals.length <= 1) return true;
            return false;
        }

        function buildSlipperDepositChargeLine(booking) {
            const count = getSlipperCountForBreakdown(booking);
            if (count <= 0) return null;

            const statusLabel = getSlipperStatusLabel(booking);
            const amount = getSlipperDepositAmount(booking);
            const noun = count === 1 ? 'Slipper' : 'Slippers';

            return {
                label: `${count} ${noun} (${statusLabel})`,
                amount,
                category: 'slipper',
                isSlipperDeposit: true
            };
        }

        function appendSlipperDepositLine(lines, booking, paymentIndex, totalPayments, paymentTotals, paymentTotal) {
            if (!Array.isArray(lines)) return lines;
            if (!shouldShowSlipperOnPaymentCard(booking, paymentIndex, totalPayments, paymentTotals, paymentTotal)) {
                return lines;
            }
            if (lines.some(l => l && l.category === 'slipper')) return lines;
            const slipperLine = buildSlipperDepositChargeLine(booking);
            if (slipperLine) lines.push(slipperLine);
            return lines;
        }
        ...
        function getExtendRateLabel(booking) {
            ...
            return formatExtendedRateLabel(booking, extendHours, extendMins);
        }
        ...
        function getExtendBlockLabel(block, booking) {
            return formatExtendedRateLabel(booking, block.hours, block.minutes);
        }
        ...
        function buildWalkInExtensionPaymentLines(booking, paymentTotal, totalPayments) {
            ...
            const lines = [{
                label: getExtendBlockLabel(block, booking),
        ...
        function getPayHistAdditionalsForIndex(idx) {
            ...
            const slippers = parseInt(document.getElementById(`payHist_add_slippers_${idx}`)?.value, 10) || 0;
            ...
            return { guests: guest, pets: pet, slippers, charges };
        }
        ...
        function getPayHistOriginalAdditionalsForIndex(idx, booking) {
            ...
            // Only compare against what was originally shown on THIS payment card.
            // Do NOT use booking-level additional_guest_date / additional_pet_date
            // (date-only match): multiple payments on the same calendar day would
            // falsely show "Guest (Removed)" / "Pet (Removed)" on cards that never
            // had those additionals (e.g. 2nd payment that only added foods).
            const guests = parseInt(guestEl?.dataset?.originalValue ?? '0', 10) || 0;
            const pets = parseInt(petEl?.dataset?.originalValue ?? '0', 10) || 0;
            return { guests, pets };
        }
        ...
        function parseDiscountHistoryEntryJs(entry) {
            const trimmed = (entry || '').trim();
            if (!trimmed) return { type: '', amount: 0, datetime: '' };
            const typedMatch = trimmed.match(/^(SC\/PWD|LGP|LPG|VIP|Long Term)\s+(-?\d+(?:\.\d+)?):(.+)$/i);
            if (typedMatch) {
                let type = typedMatch[1].toUpperCase();
                if (type === 'SC/PWD' || type.indexOf('SC') === 0) type = 'SC/PWD';
                else if (type === 'LGP' || type === 'LPG') type = 'LGP';
                else if (type === 'LONG TERM' || /^LONG\s*TERM$/i.test(typedMatch[1])) type = 'Long Term';
                else type = 'VIP';
                return {
                    type,
                    amount: parseFloat(typedMatch[2]) || 0,
                    datetime: (typedMatch[3] || '').trim()
                };
            }
        ...
        function parseDiscountHistoryEntries(booking) {
            ...
            if (!raw) return [];
            return raw.split('|').map(parseDiscountHistoryEntryJs).filter(e => e.amount > 0);
        }

        /** SC/PWD entries only — LGP/VIP/Long Term belong in their own fields */
        function getScPwdDiscountHistoryEntries(booking) {
            return parseDiscountHistoryEntries(booking).filter(e => !e.type || e.type === 'SC/PWD');
        }

        function formatTypedDiscountHistoryEntry(type, amount, datetime) {
            const amt = parseFloat(amount) || 0;
            if (amt <= 0) return '';
            let dt = (datetime || '').trim();
            if (!dt) dt = new Date().toISOString().slice(0, 19).replace('T', ' ');
            return `${type} ${amt.toFixed(2)}:${dt}`;
        }

        function getDiscountAmountsPerPayment(booking, totalPayments, dateHist) {
            const amounts = new Array(totalPayments).fill(0);
            const allHistory = parseDiscountHistoryEntries(booking);
            // Only SC/PWD (or legacy untyped) — never LGP/VIP/Long Term
            const history = getScPwdDiscountHistoryEntries(booking);
            let totalDiscount = parseFloat(booking.discount_amount) || 0;
            // If history only has LGP/VIP/Long Term, do not treat discount_amount as SC/PWD
            if (allHistory.length > 0 && history.length === 0) {
                totalDiscount = 0;
            }
        ...
        function paymentHasMatchedDiscountHistory(booking, paymentIndex, dateHist, paymentDiscount, totalPayments) {
            ...
            const history = getScPwdDiscountHistoryEntries(booking);
            if (history.length === 0) {
                // No typed SC/PWD history — only treat as matched if booking has SC/PWD amount
                return (parseFloat(booking.discount_amount) || 0) > 0;
            }
        ...
        function appendDiscountLine(lines, discountAmount) {
            ...
            if (discount > 0) {
                lines.push({ label: 'SCW/PWD Discount', amount: -discount, category: 'discount' });
            }
            return lines;
        }

        /** Read LGP/VIP/Long Term from payment-0 fields when present, else booking row */
        function getExtraDiscountAmounts(booking) {
            const readAmt = (elId, bookingKey) => {
                const el = document.getElementById(elId);
                if (el) return parseFloat(el.value) || 0;
                return parseFloat(booking?.[bookingKey]) || 0;
            };
            return {
                lgp: readAmt('payHist_lpg_amount_0', 'lgp_discount_amount'),
                vip: readAmt('payHist_vip_amount_0', 'vip_discount_amount'),
                longTerm: readAmt('payHist_long_amount_0', 'long_discount_amount')
            };
        }

        function appendAllDiscountLines(lines, booking, scPwdAmount) {
            appendDiscountLine(lines, scPwdAmount);
            const extra = getExtraDiscountAmounts(booking);
            if (extra.lgp > 0) {
                lines.push({ label: 'LGP Discount', amount: -extra.lgp, category: 'discount' });
            }
            if (extra.vip > 0) {
                lines.push({ label: 'VIP Discount', amount: -extra.vip, category: 'discount' });
            }
            if (extra.longTerm > 0) {
                const pct = parseFloat(document.getElementById('payHist_long_percent_0')?.value)
                    || parseFloat(booking?.long_discount_percent) || 0;
                const label = pct > 0 ? `Long Term Discount (${pct}%)` : 'Long Term Discount';
                lines.push({ label, amount: -extra.longTerm, category: 'discount' });
            }
            return lines;
        }

        /** Gross payment totals for allocation (net paid + discounts that apply on payment 0) */
        function getGrossTotalsForAllocation(booking, paymentTotals, scPwdPerPayment) {
            const totals = (Array.isArray(paymentTotals) ? paymentTotals : []).map(v => parseFloat(v) || 0);
            if (totals.length === 0) return totals;
            const extra = getExtraDiscountAmounts(booking);
            const sc0 = Array.isArray(scPwdPerPayment)
                ? (parseFloat(scPwdPerPayment[0]) || 0)
                : (parseFloat(scPwdPerPayment) || 0);
            totals[0] = (totals[0] || 0) + sc0 + extra.lgp + extra.vip + extra.longTerm;
            return totals;
        }
        ...
        function buildPaymentForLines(booking, paymentIndex, totalPayments, paymentTotal, paymentDiscount, paymentTotals) {
            ...
            const extendPrice = parseFloat(booking.extend_price) || 0;
            const roomInfo = getRoomRateInfo(booking, paymentIndex);
            ...
            const applyDiscount = (lines) => {
                appendSlipperDepositLine(lines, booking, paymentIndex, totalPayments, paymentTotals, paymentTotal);
                // LGP/VIP/Long Term show on 1st payment (or sole payment); SC/PWD uses paymentDiscount
                const scAmt = paymentDiscount;
                if (paymentIndex === 0 || totalPayments <= 1) {
                    return appendAllDiscountLines(lines, booking, scAmt);
                }
                return appendDiscountLine(lines, scAmt);
            };

            const ensureRoomLine = (lines) => {
                if (!lines) lines = [];
                const hasRoom = lines.some(l => l && l.category === 'room');
                if (!hasRoom && roomInfo.roomPrice > 0 && (paymentIndex === 0 || totalPayments <= 1)) {
                    lines.unshift({ label: roomInfo.label, amount: roomInfo.roomPrice, category: 'room' });
                }
                return lines;
            };

            // 2nd (or matching) payment that is only the slipper deposit (e.g. ₱100)
            if (isPureSlipperPayment(booking, paymentIndex, paymentTotal, paymentTotals, totalPayments)) {
                const slipperLine = buildSlipperDepositChargeLine(booking);
                return appendDiscountLine(slipperLine ? [slipperLine] : [], paymentDiscount);
            }
            ...
            if (totalPayments <= 1) {
                const addOverride = document.getElementById('payHist_add_guest_0')
                    ...
                    : null;
                // Inflate net payment by discounts so room rate still allocates (e.g. 1415 + 75 LGP = 1490)
                const rawTotals = (paymentTotals && paymentTotals.length) ? paymentTotals : [paymentTotal];
                const totals = getGrossTotalsForAllocation(booking, rawTotals, paymentDiscount);
                if (addOverride) {
                    let lines = buildWalkInMultiPaymentLines(booking, 0, totals, addOverride);
                    lines = ensureRoomLine(lines);
                    appendRemovedAdditionalsLines(lines, booking, 0, totalPayments, paymentTotal, paymentTotals, addOverride);
                    if (extendPrice > 0) {
                        lines.push({ label: getExtendRateLabel(booking), amount: extendPrice, category: 'extend' });
                    }
                    if (!lines.length) {
                        lines = checkInLines.slice();
                        appendAdditionalsToLines(lines, addOverride);
                    }
                    return applyDiscount(lines);
                }
                if (extendPrice > 0) {
                    checkInLines.push({ label: getExtendRateLabel(booking), amount: extendPrice, category: 'extend' });
                }
                return applyDiscount(ensureRoomLine(checkInLines));
            }
            ...
            if (!isReservationSplit && totalPayments >= 2 && extendPrice <= 0) {
                const rawTotals = (paymentTotals && paymentTotals.length === totalPayments)
                    ? paymentTotals
                    : new Array(totalPayments).fill(paymentTotal);
                const totals = getGrossTotalsForAllocation(booking, rawTotals, paymentDiscount);
                const addOverride = document.getElementById(`payHist_add_guest_${paymentIndex}`)
                    ? getPayHistAdditionalsForIndex(paymentIndex)
                    : null;
                let lines = buildWalkInMultiPaymentLines(booking, paymentIndex, totals, addOverride);
                if (paymentIndex === 0) lines = ensureRoomLine(lines);
                if (addOverride) {
                    appendRemovedAdditionalsLines(lines, booking, paymentIndex, totalPayments, paymentTotal, totals, addOverride);
                }
                if (lines.length > 0) return applyDiscount(lines);
        ...
        function renderPaymentForBreakdownHtml(lines, paymentTotal) {
            ...
            // Slipper new_deposit is separate from room payment, so exclude it from the payment subtotal.
            const displaySubtotal = paymentTotal > 0 ? paymentTotal : expandedLines.reduce((sum, line) => {
                if (line.amount === null || line.isRemoved || line.isSlipperDeposit || line.category === 'slipper') {
                    return sum;
                }
        ...
        const PAYHIST_MISSING_ITEMS = [
            { name: 'Towel', price: 500 },
            { name: 'Pillow Cover', price: 500 },
            { name: 'Bedsheet', price: 500 },
            { name: 'Slippers', price: 100 }
        ];
        const PAYHIST_PENALTY_RULES = [
            { name: 'Lost or Damaged Linens', price: 500.00 },
            { name: 'Lost or Unreturned Room Key', price: 500.00 },
            { name: 'Outside Massage Therapist Fee', price: 500.00 },
            { name: 'Smoking Violation', price: 1000.00 }
        ];
        let payHistMissingPenaltySyncing = false;

        function parsePayHistFeeList(raw) {
            if (!raw) return [];
            try {
                const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
                return Array.isArray(parsed) ? parsed : [];
            } catch (e) {
                console.error('Failed to parse fee list', e);
                return [];
            }
        }

        function getPayHistMissingItemNames(list) {
            return parsePayHistFeeList(list).map(item => (item && (item.name || item.item || item)) || '')
                .map(n => String(n).trim())
                .filter(Boolean);
        }

        function getPayHistPenaltyNames(list) {
            return getPayHistMissingItemNames(list);
        }

        function countPayHistCards() {
            let count = 0;
            while (document.getElementById(`payHist_missing_list_${count}`)) count++;
            return count;
        }

        function getPayHistCheckoutCardIdx() {
            const count = countPayHistCards();
            return count > 0 ? count - 1 : 0;
        }

        function isPayHistStandardSlippersItem(name) {
            return String(name || '').toLowerCase().trim() === 'slippers';
        }

        function derivePayHistSlipperStatusFromChecklist(idx, missingItems, booking) {
            const additionalSlippers = Math.max(0, parseInt(booking?.additional_slippers, 10) || 0);
            const standardSlippersMissing = missingItems.some(item =>
                isPayHistStandardSlippersItem(item.name || item.item || item)
            );

            const addCheckboxes = Array.from(
                document.querySelectorAll(`#payHist_missing_list_${idx} .payHist-missing-checkbox[data-slipper="1"]`)
            );

            if (additionalSlippers > 0 && addCheckboxes.length > 0) {
                let returnedCount = 0;
                let missingCount = 0;
                addCheckboxes.forEach(cb => {
                    if (cb.checked) {
                        returnedCount++;
                    } else if (cb.dataset.slipperTouched === 'true') {
                        missingCount++;
                    }
                });
                const pendingCount = Math.max(0, addCheckboxes.length - returnedCount - missingCount);
                if (missingCount > 0 || standardSlippersMissing) return 'missing';
                if (pendingCount > 0) return 'added';
                if (returnedCount === addCheckboxes.length && !standardSlippersMissing) return 'returned';
                return 'added';
            }

            if (standardSlippersMissing) return 'missing';
            return 'added';
        }

        function buildPayHistMissingItemsForBooking(booking) {
            const items = PAYHIST_MISSING_ITEMS.map(i => ({ ...i }));
            const slippers = Math.max(0, parseInt(booking?.additional_slippers, 10) || 0);
            for (let s = 1; s <= slippers; s++) {
                items.push({ name: `Additional Slipper ${s}`, price: 100, isSlipper: true });
            }
            return items;
        }

        function initPayHistMissingPenalty(booking, count) {
            const missingNames = getPayHistMissingItemNames(booking?.missing_items_list);
            const penaltyNames = getPayHistPenaltyNames(booking?.penalty_list);
            const feesPaid = String(booking?.additional_fees_status || '').toLowerCase() === 'paid';
            const savedMissingFees = parseFloat(booking?.missing_items_fees) || 0;
            const savedPenaltyFees = parseFloat(booking?.penalty_amount) || 0;

            // Missing items / slippers only belong to the LAST payment card (checkout card).
            // All other cards should show all items as present (checked).
            const lastIdx = count - 1;

            for (let i = 0; i < count; i++) {
                const isLastCard = (i === lastIdx);
                const cardMissingNames = isLastCard ? missingNames : [];
                const cardPenaltyNames = isLastCard ? penaltyNames : [];
                const cardSlipperStatus = isLastCard ? (booking?.slipper_status || '') : '';
                const cardBooking = { ...booking, slipper_status: cardSlipperStatus };

                renderPayHistMissingList(i, cardBooking, cardMissingNames, feesPaid);
                renderPayHistPenaltyList(i, cardPenaltyNames, feesPaid);

                if (feesPaid) {
                    const missingFeesEl = document.getElementById(`payHist_missing_fees_${i}`);
                    const missingJsonEl = document.getElementById(`payHist_missing_json_${i}`);
                    const penaltyFeesEl = document.getElementById(`payHist_penalty_fees_${i}`);
                    const penaltyJsonEl = document.getElementById(`payHist_penalty_json_${i}`);
                    if (missingFeesEl) missingFeesEl.value = isLastCard ? savedMissingFees.toFixed(2) : '0';
                    if (missingJsonEl) missingJsonEl.value = isLastCard ? (booking?.missing_items_list || '[]') : '[]';
                    if (penaltyFeesEl) penaltyFeesEl.value = isLastCard ? savedPenaltyFees.toFixed(2) : '0';
                    if (penaltyJsonEl) penaltyJsonEl.value = isLastCard ? (booking?.penalty_list || '[]') : '[]';

                    const missingStatus = document.getElementById(`payHist_missing_status_${i}`);
                    if (missingStatus) {
                        if (isLastCard && savedMissingFees > 0) {
                            missingStatus.textContent = 'PAID';
                            missingStatus.style.background = '#10b981';
                        } else {
                            missingStatus.textContent = 'NO FEES';
                            missingStatus.style.background = '#10b981';
                        }
                    }
                    const penaltyStatus = document.getElementById(`payHist_penalty_status_${i}`);
                    if (penaltyStatus) {
                        if (isLastCard && savedPenaltyFees > 0) {
                            penaltyStatus.textContent = 'PAID';
                            penaltyStatus.style.background = '#10b981';
                        } else {
                            penaltyStatus.textContent = 'NO FEES';
                            penaltyStatus.style.background = '#10b981';
                        }
                    }
                } else {
                    updatePayHistMissingPenaltyTotals(i, false);
                }
            }
            // Keep all cards + breakdown in sync after initial render
            if (!feesPaid) {
                syncPayHistMissingPenaltyAcrossCards(lastIdx);
            } else if (typeof refreshPayHistPaymentForBreakdowns === 'function') {
                refreshPayHistPaymentForBreakdowns();
            }
        }

        function isMissingItemNameMatch(savedName, item) {
            if (!savedName || !item || !item.name) return false;
            const s = String(savedName).toLowerCase().trim();
            const target = String(item.name).toLowerCase().trim();

            if (s === target) return true;

            // Handle slippers - distinguish standard room slippers vs specific Additional Slipper N
            if (s.includes('slipper') && target.includes('slipper')) {
                const sIsAdd = s.includes('additional slipper');
                const targetIsAdd = item.isSlipper || target.includes('additional slipper');

                if (sIsAdd && targetIsAdd) {
                ...
                } else if (!sIsAdd && !targetIsAdd) {
                    return true;
                }
                return false;
            }

            // Normalize singular vs plural and common synonyms
            if (s.includes('towel') && target.includes('towel')) return true;
            if (s.includes('bedsheet') && target.includes('bedsheet')) return true;
            if (s.includes('cover') && target.includes('cover')) return true;

            return false;
        }

        function renderPayHistMissingList(idx, booking, missingNames, feesPaid) {
            const listEl = document.getElementById(`payHist_missing_list_${idx}`);
            if (!listEl) return;

            const items = buildPayHistMissingItemsForBooking(booking);
            listEl.innerHTML = '';
            const slipperStatus = String(booking?.slipper_status || '').toLowerCase();

            items.forEach((item, index) => {
                let isMissing;
                if (item.isSlipper) {
                    isMissing = !feesPaid && (
                        slipperStatus === 'missing' ||
                        missingNames.some(n => isMissingItemNameMatch(n, item))
                    );
                } else {
                    isMissing = missingNames.some(n => isMissingItemNameMatch(n, item));
                }

                const checkedAttr = isMissing ? '' : 'checked';
                const slipperTouchedAttr = (item.isSlipper && isMissing) ? 'data-slipper-touched="true"' : '';
                const border = index === items.length - 1 ? 'none' : '1px solid #e0e0e0';
                const row = document.createElement('div');
                row.style.cssText = `display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:${border};`;
                row.innerHTML = `
                    <label style="display:flex;align-items:center;gap:10px;font-size:12px;cursor:pointer;flex:1;font-family:'Poppins',sans-serif;">
                        <input type="checkbox" class="payHist-missing-checkbox"
                            data-item="${item.name.replace(/"/g, '&quot;')}"
                            data-price="${item.price}"
                            data-slipper="${item.isSlipper ? '1' : '0'}"
                            ${checkedAttr}
                            ${slipperTouchedAttr}
                            onchange="onPayHistMissingChange(${idx}, this)"
                            style="width:16px;height:16px;cursor:pointer;">
                        <span style="font-weight:500;color:#374151;">${item.name}</span>
                    </label>
                    <span style="font-weight:600;font-size:13px;color:#333;">₱${Number(item.price).toFixed(2)}</span>
                `;
                listEl.appendChild(row);
            });
        }

        function renderPayHistPenaltyList(idx, penaltyNames, feesPaid) {
            const listEl = document.getElementById(`payHist_penalty_list_${idx}`);
            if (!listEl) return;

            listEl.innerHTML = '';
            PAYHIST_PENALTY_RULES.forEach((rule, index) => {
                const isApplied = !feesPaid && penaltyNames.some(n => n.toLowerCase() === rule.name.toLowerCase());
                const border = index === PAYHIST_PENALTY_RULES.length - 1 ? 'none' : '1px solid #e0e0e0';
                const row = document.createElement('div');
                row.className = 'payHist-penalty-item';
                row.setAttribute('data-price', rule.price);
                row.setAttribute('data-checked', isApplied ? 'true' : 'false');
                row.style.cssText = `display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:${border};`;
                row.innerHTML = `
                    <label style="display:flex;align-items:center;gap:10px;font-size:12px;cursor:pointer;flex:1;font-family:'Poppins',sans-serif;">
                        <input type="checkbox" class="payHist-penalty-checkbox"
                            ${isApplied ? 'checked' : ''}
                            onchange="onPayHistPenaltyChange(${idx}, this)"
                            style="width:16px;height:16px;cursor:pointer;">
                        <span class="payHist-penalty-name" style="font-weight:500;color:#374151;">${rule.name}</span>
                    </label>
                    <span style="font-weight:600;font-size:13px;color:#333;">₱${Number(rule.price).toFixed(2)}</span>
                `;
                listEl.appendChild(row);
            });
        }

        function collectPayHistMissingFromCard(idx) {
            const checkboxes = document.querySelectorAll(`#payHist_missing_list_${idx} .payHist-missing-checkbox`);
            const missingItems = [];
            let totalFees = 0;
            checkboxes.forEach(cb => {
                if (cb.getAttribute('data-slipper') === '1') return; // deposit covers slipper missing
                if (cb.checked) return; // checked = present
                const name = cb.getAttribute('data-item') || 'Item';
                const price = parseFloat(cb.getAttribute('data-price') || 0) || 0;
                missingItems.push({ name, price });
                totalFees += price;
            });
            return { missingItems, totalFees };
        }

        function collectPayHistPenaltyFromCard(idx) {
            const rows = document.querySelectorAll(`#payHist_penalty_list_${idx} .payHist-penalty-item`);
            const penaltyItems = [];
            let totalPenaltyAmount = 0;
            rows.forEach(row => {
                const cb = row.querySelector('.payHist-penalty-checkbox');
                const applied = cb ? cb.checked : (row.getAttribute('data-checked') === 'true');
                if (!applied) return;
                const name = row.querySelector('.payHist-penalty-name')?.textContent || 'Penalty';
                const price = parseFloat(row.getAttribute('data-price') || 0) || 0;
                penaltyItems.push({ name, price });
                totalPenaltyAmount += price;
            });
            return { penaltyItems, totalPenaltyAmount };
        }

        function updatePayHistMissingPenaltyTotals(idx, refreshBreakdown = true) {
            const { missingItems, totalFees } = collectPayHistMissingFromCard(idx);
            const { penaltyItems, totalPenaltyAmount } = collectPayHistPenaltyFromCard(idx);

            const missingFeesEl = document.getElementById(`payHist_missing_fees_${idx}`);
            const missingJsonEl = document.getElementById(`payHist_missing_json_${idx}`);
            const penaltyFeesEl = document.getElementById(`payHist_penalty_fees_${idx}`);
            const penaltyJsonEl = document.getElementById(`payHist_penalty_json_${idx}`);
            if (missingFeesEl) missingFeesEl.value = totalFees.toFixed(2);
            if (missingJsonEl) missingJsonEl.value = JSON.stringify(missingItems);
            if (penaltyFeesEl) penaltyFeesEl.value = totalPenaltyAmount.toFixed(2);
            if (penaltyJsonEl) penaltyJsonEl.value = JSON.stringify(penaltyItems);

            const missingStatus = document.getElementById(`payHist_missing_status_${idx}`);
            if (missingStatus) {
                if (totalFees > 0) {
                    missingStatus.textContent = 'FEES APPLIED';
                    missingStatus.style.background = '#dc2626';
                } else {
                    missingStatus.textContent = 'NO FEES';
                    missingStatus.style.background = '#10b981';
                }
            ...
            const penaltyStatus = document.getElementById(`payHist_penalty_status_${idx}`);
            if (penaltyStatus) {
                if (totalPenaltyAmount > 0) {
                    penaltyStatus.textContent = 'FEES APPLIED';
                    penaltyStatus.style.background = '#dc2626';
                } else {
                    penaltyStatus.textContent = 'NO FEES';
                    penaltyStatus.style.background = '#10b981';
                }
            ...
            if (window.currentBooking) {
                const feesAlreadyPaid = String(window.currentBooking.additional_fees_status || '').toLowerCase() === 'paid';
                if (!feesAlreadyPaid) {
                    window.currentBooking.missing_items_fees = totalFees;
                    window.currentBooking.missing_items_list = JSON.stringify(missingItems);
                    window.currentBooking.penalty_amount = totalPenaltyAmount;
                    window.currentBooking.penalty_list = JSON.stringify(penaltyItems);
                    window.currentBooking.slipper_status = derivePayHistSlipperStatusFromChecklist(
                        idx,
                        missingItems,
                        window.currentBooking
                    );
                    if (totalFees > 0 || totalPenaltyAmount > 0) {
                        window.currentBooking.additional_fees_status = 'Pending';
                    } else {
                        window.currentBooking.additional_fees_status = 'None';
                    }
                }
            }

            if (refreshBreakdown && typeof refreshPayHistPaymentForBreakdowns === 'function') {
                refreshPayHistPaymentForBreakdowns();
            }
        }

        function syncPayHistMissingPenaltyAcrossCards(sourceIdx) {
            if (payHistMissingPenaltySyncing) return;
            payHistMissingPenaltySyncing = true;
            try {
                const sourceMissing = Array.from(
                    document.querySelectorAll(`#payHist_missing_list_${sourceIdx} .payHist-missing-checkbox`)
                ).map(cb => ({
                    name: cb.getAttribute('data-item') || '',
                    checked: !!cb.checked,
                    slipperTouched: cb.dataset.slipperTouched === 'true'
                }));
                const sourcePenalty = Array.from(
                    document.querySelectorAll(`#payHist_penalty_list_${sourceIdx} .payHist-penalty-checkbox`)
                ).map(cb => !!cb.checked);

                const count = countPayHistCards();
                for (let i = 0; i < count; i++) {
                    if (i !== sourceIdx) {
                        document.querySelectorAll(`#payHist_missing_list_${i} .payHist-missing-checkbox`).forEach((cb, mi) => {
                            if (sourceMissing[mi]) {
                                cb.checked = sourceMissing[mi].checked;
                                if (sourceMissing[mi].slipperTouched) cb.dataset.slipperTouched = 'true';
                            }
                        });
                        document.querySelectorAll(`#payHist_penalty_list_${i} .payHist-penalty-checkbox`).forEach((cb, pi) => {
                            cb.checked = !!sourcePenalty[pi];
                            const row = cb.closest('.payHist-penalty-item');
                            if (row) row.setAttribute('data-checked', cb.checked ? 'true' : 'false');
                        });
                    }
                    updatePayHistMissingPenaltyTotals(i, false);
                }
                if (typeof refreshPayHistPaymentForBreakdowns === 'function') {
                    refreshPayHistPaymentForBreakdowns();
                }
            } finally {
                payHistMissingPenaltySyncing = false;
            }
        }

        function onPayHistMissingChange(idx, checkbox) {
            if (isPaymentCardLocked(idx)) {
                checkbox.checked = !checkbox.checked;
                return;
            }
            if (checkbox.getAttribute('data-slipper') === '1') {
                checkbox.dataset.slipperTouched = 'true';
            }
            syncPayHistMissingPenaltyAcrossCards(idx);
        }

        function onPayHistPenaltyChange(idx, checkbox) {
            if (isPaymentCardLocked(idx)) {
                checkbox.checked = !checkbox.checked;
                return;
            }
            const row = checkbox.closest('.payHist-penalty-item');
            if (row) row.setAttribute('data-checked', checkbox.checked ? 'true' : 'false');
            syncPayHistMissingPenaltyAcrossCards(idx);
        }

        function collectPayHistMissingPenalty() {
            const idx = getPayHistCheckoutCardIdx();
            const booking = window.currentBooking || {};
            if (!document.getElementById(`payHist_missing_list_${idx}`)) {
                return {
                    missing_items_fees: parseFloat(booking.missing_items_fees) || 0,
                    missing_items_list: booking.missing_items_list || '[]',
                    penalty_amount: parseFloat(booking.penalty_amount) || 0,
                    penalty_list: booking.penalty_list || '[]',
                    additional_fees_status: booking.additional_fees_status || 'None',
                    slipper_status: booking.slipper_status || 'added'
                };
            }

            const { missingItems, totalFees } = collectPayHistMissingFromCard(idx);
            const { penaltyItems, totalPenaltyAmount } = collectPayHistPenaltyFromCard(idx);
            const slipperStatus = derivePayHistSlipperStatusFromChecklist(idx, missingItems, booking);
            const statusExisting = String(booking.additional_fees_status || '');
            let status = statusExisting || 'None';

            if (statusExisting.toLowerCase() === 'paid') {
                if (totalFees > 0 || totalPenaltyAmount > 0) {
                    status = 'Pending';
                } else {
                    status = 'Paid';
                }
            } else if (totalFees > 0 || totalPenaltyAmount > 0) {
                status = 'Pending';
            } else {
                status = 'None';
            }
            ...
            return {
                missing_items_fees: totalFees,
                missing_items_list: JSON.stringify(missingItems),
                penalty_amount: totalPenaltyAmount,
                penalty_list: JSON.stringify(penaltyItems),
                additional_fees_status: status,
                slipper_status: slipperStatus
            };
        }

        function appendPayHistMissingPenaltyLines(lines, paymentIndex, totalPayments) {
            if (!lines) lines = [];
            // Show booking-level missing/penalty on the last payment card only
            if (paymentIndex !== totalPayments - 1) return lines;

            const fees = collectPayHistMissingPenalty();
            const feesPaid = String(fees.additional_fees_status || '').toLowerCase() === 'paid';

            try {
                const missing = parsePayHistFeeList(fees.missing_items_list);
                missing.forEach(item => {
                    const name = item.name || item.item || item;
                    const price = parseFloat(item.price || 0) || 0;
                    if (!name || price <= 0) return;
                    lines.push({
                        label: feesPaid ? `${String(name)} (Paid)` : String(name),
                        amount: price,
                        category: 'missing'
                    });
                });
            ...
            try {
                const penalties = parsePayHistFeeList(fees.penalty_list);
        ...
        function renderPaymentHistory(booking) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const lpgAmountVal = (i === 0 && (parseFloat(booking.lgp_discount_amount) || 0) > 0)
                    ? escAttr(booking.lgp_discount_amount) : '';
                const lpgApproverVal = i === 0 ? escAttr(booking.lgp_approver_name || '') : '';
                const vipAmountVal = (i === 0 && (parseFloat(booking.vip_discount_amount) || 0) > 0)
                    ? escAttr(booking.vip_discount_amount) : '';
                const vipApproverVal = i === 0 ? escAttr(booking.vip_approver_name || '') : '';
                const longAmountVal = (i === 0 && (parseFloat(booking.long_discount_amount) || 0) > 0)
                    ? escAttr(booking.long_discount_amount) : '';
                const longPercentVal = (i === 0 && (parseFloat(booking.long_discount_percent) || 0) > 0)
                    ? escAttr(booking.long_discount_percent) : '';
                ...
                const addSlippersVal = parseInt(booking.additional_slippers, 10) || 0;
                ...
                <div class="payment-history-card" id="payHist_card_${i}" data-payment-index="${i}" style="display:flex;flex-direction:column;gap:12px;font-family:'Poppins',sans-serif;background:#ffffff;border:1px solid #e5e7eb;border-radius:8px;padding:14px;box-shadow:0 1px 2px rgba(15,23,42,0.04);">
                    ...
                    <div class="detail-row" id="payHist_discount_main_${i}" style="margin-bottom:0;display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">SCW/PWD Discount Amount</span>
                        ...
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">SCW/PWD Count</span>
                    ...
                    <div class="detail-row" style="margin-bottom:0;display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">LPG Discount Amount</span>
                            <input type="number" step="0.01" min="0" class="detail-input" id="payHist_lpg_amount_0"
                                value="${lpgAmountVal}" placeholder="0.00"
                                oninput="updatePayHistDiscountBreakdown(0)">
                        </div>
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">LPG Discount Approver</span>
                            <input type="text" class="detail-input" id="payHist_lpg_approver_0"
                                value="${lpgApproverVal}" placeholder="Approver name">
                        </div>
                    </div>
                    <div class="detail-row" style="margin-bottom:0;display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">VIP Discount Amount</span>
                            <input type="number" step="0.01" min="0" class="detail-input" id="payHist_vip_amount_0"
                                value="${vipAmountVal}" placeholder="0.00"
                                oninput="updatePayHistDiscountBreakdown(0)">
                        </div>
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">VIP Approver</span>
                            <input type="text" class="detail-input" id="payHist_vip_approver_0"
                                value="${vipApproverVal}" placeholder="Approver name">
                        </div>
                    </div>
                    <div class="detail-row" style="margin-bottom:0;display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">Long Term Discount Amount</span>
                            <input type="number" step="0.01" min="0" class="detail-input" id="payHist_long_amount_0"
                                value="${longAmountVal}" placeholder="0.00"
                                oninput="updatePayHistDiscountBreakdown(0)">
                        </div>
                        <div class="detail-item" style="margin-bottom:0;">
                            <span class="detail-label">Long Term Discount Percent</span>
                            <input type="number" step="0.01" min="0" max="100" class="detail-input" id="payHist_long_percent_0"
                                value="${longPercentVal}" placeholder="0"
                                oninput="updatePayHistDiscountBreakdown(0)">
                    ...
                    <!-- Additional Guest / Pet / Slippers / Food -->
                    <div class="payHist-section-box" style="${payHistSectionStyle}">
                    <div class="detail-row" style="margin-bottom:0;display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div class="detail-item" style="margin-bottom:0;">
                        ...
                        <div class="detail-item" style="margin-bottom:0;display:none;">
                            <span class="detail-label">Additional Slippers <span style="font-weight:400;color:#6b7280;">(₱100 each)</span></span>
                            <input type="number" min="0" class="detail-input" id="payHist_add_slippers_${i}"
                                value="${addSlippersVal}" data-original-value="${addSlippersVal}" placeholder="0"
                                oninput="syncPayHistSlippers(${i})">
                        </div>
                    </div>
                    ...
                    <!-- Missing Items Checklist + Penalty Fine Rules (synced with Booking checkoutModal) -->
                    <div class="payHist-section-box" style="${payHistSectionStyle}">
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                            <div>
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:8px;">
                                    <span class="detail-label" style="margin:0;">Penalty Fine Rules</span>
                                </div>
                                <p style="margin:0 0 8px 0;font-size:11px;color:#6b7280;">Check a rule to apply its penalty fee</p>
                                <div id="payHist_penalty_list_${i}" style="display:flex;flex-direction:column;gap:0;"></div>
                            </div>
                            <div>
                                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:8px;">
                                    <span class="detail-label" style="margin:0;">Missing Items Checklist</span>
                                </div>
                                <p style="margin:0 0 8px 0;font-size:11px;color:#6b7280;">Uncheck an item if it is missing</p>
                                <div id="payHist_missing_list_${i}" style="display:flex;flex-direction:column;gap:0;"></div>
                            </div>
                        </div>
                        <input type="hidden" id="payHist_missing_fees_${i}" value="0">
                        <input type="hidden" id="payHist_missing_json_${i}" value="[]">
                        <input type="hidden" id="payHist_penalty_fees_${i}" value="0">
                        <input type="hidden" id="payHist_penalty_json_${i}" value="[]">
                    </div>
            ...
            html += `
                <button type="button" onclick="addMorePayment()" 
                    style="width:100%;padding:12px;background:#6b7280;color:white;border:none;border-radius:8px;cursor:pointer;font-size:14px;font-weight:600;font-family:'Poppins',sans-serif;margin-top:8px;transition:background 0.2s ease;">
                    + Add More Payment
                </button>
            </div>`;
            ...
            initPayHistMissingPenalty(booking, count);
            refreshPayHistPaymentForBreakdowns();
        ...
        function setPaymentCardLocked(idx, locked) {
            ...
            const addSlippers = document.getElementById(`payHist_add_slippers_${idx}`);
            if (addSlippers) addSlippers.disabled = locked;
            ...
            document.querySelectorAll(`#payHist_missing_list_${idx} .payHist-missing-checkbox`).forEach(cb => {
                cb.disabled = locked;
            });
            document.querySelectorAll(`#payHist_penalty_list_${idx} .payHist-penalty-checkbox`).forEach(cb => {
                cb.disabled = locked;
            });
        ...
        function addMorePayment() {
            ...
            const currentCount = Math.max(
                cashHist.length, gcashHist.length, mayaHist.length,
                instHist.length, obHist.length, airHist.length, dateHist.length, 1
            );
            ...
            // Update booking object with new payment entries
            booking.payment_amount_cash_history = cashHist.join(' | ');
            booking.payment_amount_g_cash_history = gcashHist.join(' | ');
            booking.payment_amount_maya_history = mayaHist.join(' | ');
            booking.payment_amount_instapay_history = instHist.join(' | ');
            booking.payment_amount_online_banking_history = obHist.join(' | ');
            booking.payment_amount_airbnb_history = airHist.join(' | ');
            booking.payment_date_time = dateHist.join(' | ');

            // Re-render the payment history
            renderPaymentHistory(booking);

            // Scroll to the new payment card
            setTimeout(() => {
                const newCard = document.getElementById(`payHist_card_${currentCount}`);
                if (newCard) {
        ...
        function collectNewDepositData() {
            // No New Deposit Payment Method UI — ₱100 per Additional Slipper is automatic.
            // Preserve existing method split when still held; otherwise set Cash for the auto amount.
            const b = window.currentBooking || {};
            const addData = (typeof collectPaymentAdditionals === 'function')
                ? collectPaymentAdditionals()
                : { additional_slippers: parseInt(b.additional_slippers, 10) || 0 };
            const slippers = Math.max(0, parseInt(addData.additional_slippers, 10) || 0);
            const status = String(b.slipper_status || 'added').toLowerCase();
            const autoAmount = (status === 'returned') ? 0 : (slippers * 100);

            const existingCash = parseFloat(b.new_deposit_cash) || 0;
            const existingGcash = parseFloat(b.new_deposit_g_cash) || 0;
            const existingMaya = parseFloat(b.new_deposit_maya) || 0;
            const existingInst = parseFloat(b.new_deposit_instapay) || 0;
            const existingOb = parseFloat(b.new_deposit_online_banking) || 0;
            const existingAir = parseFloat(b.new_deposit_airbnb) || 0;
            const existingTotal = existingCash + existingGcash + existingMaya + existingInst + existingOb + existingAir
                || parseFloat(b.new_deposit) || 0;

            const result = {
                new_deposit: autoAmount,
                new_deposit_cash: 0,
                new_deposit_g_cash: 0,
                new_deposit_maya: 0,
                new_deposit_instapay: 0,
                new_deposit_online_banking: 0,
                new_deposit_airbnb: 0,
                new_deposit_details: '',
            ...
            // Keep existing payment-method split if totals still match
            if (existingTotal > 0.005 && Math.abs(existingTotal - autoAmount) <= 0.015) {
                result.new_deposit_cash = existingCash;
                result.new_deposit_g_cash = existingGcash;
                result.new_deposit_maya = existingMaya;
                result.new_deposit_instapay = existingInst;
                result.new_deposit_online_banking = existingOb;
                result.new_deposit_airbnb = existingAir;
                result.new_deposit_gcash_ref = b.new_deposit_gcash_ref || '';
                result.new_deposit_maya_ref = b.new_deposit_maya_ref || '';
                result.new_deposit_instapay_ref = b.new_deposit_instapay_ref || '';
                result.new_deposit_online_banking_ref = b.new_deposit_online_banking_ref || '';
                result.new_deposit_airbnb_ref = b.new_deposit_airbnb_ref || '';
                result.new_deposit_details = b.new_deposit_details || '';
                return result;
            }

            // Default: automatic ₱100 x slippers as Cash deposit
            result.new_deposit_cash = autoAmount;
            result.new_deposit_details = `Cash (₱${autoAmount.toFixed(2)})`;
            return result;
        }
        ...
        function updatePayHistDiscountIdFields(idx, initialIdList) {
            ...
            if (count <= 1) {
                ...
                idCol.style.display = '';
                idCol.innerHTML = `
                    <span class="detail-label">SCW/PWD Discount ID</span>
                ...
                extraWrap.innerHTML = Array.from({ length: count }, (_, j) => {
                    ...
                    return `
                    <div class="detail-item" style="margin-bottom:0;">
                        <span class="detail-label">SCW/PWD Discount ID ${n}</span>
        ...
        function collectPaymentAdditionals() {
            ...
            while (document.getElementById(`payHist_add_guest_${i}`) !== null) {
                ...
                const slipEl = document.getElementById(`payHist_add_slippers_${i}`);
                if (slipEl) {
                    totalSlippers = Math.max(totalSlippers, parseInt(slipEl.value, 10) || 0);
                }
            ...
            // Fallback to booking value if slippers UI was not rendered
            if (i === 0 && window.currentBooking) {
                totalSlippers = parseInt(window.currentBooking.additional_slippers, 10) || 0;
            }

            return {
                additional_guest: totalGuest,
                additional_pet: totalPet,
                additional_slippers: totalSlippers,
        ...
        function syncModalAdditionalsFromPayments() {
            ...
            const slippersEl = document.getElementById('modalAdditionalSlippers');
            const chargesEl = document.getElementById('modalAdditionalCharges');

            if (guestEl) guestEl.value = data.additional_guest;
            if (petEl) petEl.value = data.additional_pet;
            if (slippersEl) slippersEl.value = data.additional_slippers;
            if (chargesEl) {
        ...
        function syncPayHistSlippers(sourceIdx) {
            const sourceEl = document.getElementById(`payHist_add_slippers_${sourceIdx}`);
            if (!sourceEl) return;
            const value = Math.max(0, parseInt(sourceEl.value, 10) || 0);
            sourceEl.value = value;

            // Additional Slippers is booking-level — keep all payment cards in sync
            let i = 0;
            while (document.getElementById(`payHist_add_slippers_${i}`) !== null) {
                if (i !== sourceIdx) {
                    const el = document.getElementById(`payHist_add_slippers_${i}`);
                    if (el) el.value = value;
                }
                i++;
            }
            updatePayHistAdditionals(sourceIdx);
        }
        ...
        function updatePayHistAdditionals(idx) {
            ...
            // Slipper count changes affect Charges Breakdown (₱100 each, automatic)
            if (typeof refreshPayHistPaymentForBreakdowns === 'function') {
                refreshPayHistPaymentForBreakdowns();
            } else {
        ...
        function refreshPayHistPaymentForBreakdowns() {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const lines = buildPaymentForLines(booking, i, count, paymentTotal, paymentDiscount, allPaymentTotals);
                appendPayHistMissingPenaltyLines(lines, i, count);
                container.innerHTML = renderPaymentForBreakdownHtml(lines, paymentTotal);
            }
        }
        ...
        /** Collect per-payment SC/PWD + LGP/VIP/Long Term into typed discount_amount_history */
        function collectPaymentDiscounts() {
            ...
            while (document.getElementById(`payHist_discount_amt_${i}`) !== null) {
                ...
                if (amt > 0) {
                    ...
                    const scEntry = formatTypedDiscountHistoryEntry('SC/PWD', amt, dtFmt);
                    if (scEntry) entries.push(scEntry);
                }
                i++;
            }

            const histDt = firstPaymentDt || new Date().toISOString().slice(0, 19).replace('T', ' ');
            const lgpAmount = parseFloat(document.getElementById('payHist_lpg_amount_0')?.value) || 0;
            const vipAmount = parseFloat(document.getElementById('payHist_vip_amount_0')?.value) || 0;
            const longAmount = parseFloat(document.getElementById('payHist_long_amount_0')?.value) || 0;
            const lgpEntry = formatTypedDiscountHistoryEntry('LGP', lgpAmount, histDt);
            const vipEntry = formatTypedDiscountHistoryEntry('VIP', vipAmount, histDt);
            const longEntry = formatTypedDiscountHistoryEntry('Long Term', longAmount, histDt);
            if (lgpEntry) entries.push(lgpEntry);
            if (vipEntry) entries.push(vipEntry);
            if (longEntry) entries.push(longEntry);
        ...
        function formatPricingDurationOptionLabel(hours, price, unit) {
            const h = parseFloat(hours) || 0;
            const p = Number(price);
            const priceText = Number.isFinite(p) ? p.toFixed(2) : String(price ?? '');
            if (unit === 'days') {
                const days = h / 24;
                return `${days} Day${days !== 1 ? 's' : ''} - ₱${priceText}`;
            }
            return `${h} hours - ₱${priceText}`;
        }

        function syncPayHistDurationSelectsFromModal(preferredHours) {
            const modalDur = document.getElementById('modalDuration');
            if (!modalDur || !modalDur.options.length) return;
            const booking = window.currentBooking;
            let totalPayments = 0;
            while (document.getElementById(`payHist_duration_${totalPayments}`) !== null) totalPayments++;

            let i = 0;
            while (document.getElementById(`payHist_duration_${i}`) !== null) {
                const durSel = document.getElementById(`payHist_duration_${i}`);
                durSel.innerHTML = modalDur.innerHTML;
                if (booking) {
                    applyPayHistDurationSelect(i, booking, totalPayments, preferredHours);
                }
                i++;
            }
            if (typeof refreshPayHistPaymentForBreakdowns === 'function') {
        ...
        function loadDurationOptions(roomType, roomId, currentDuration, booking) {
            ...
            const currentHours = booking && typeof getModDurationHours === 'function'
                ? getModDurationHours(booking)
                : (parseFloat(currentDuration) || 0);
                ...
                .then(data => {
                    ...
                    if (data.success && data.pricing && data.pricing.length > 0) {
                        ...
                        data.pricing.forEach(price => {
                            ...
                            const hours = parseFloat(price.hours) || 0;
                            const unit = (price.unit === 'days' || price.duration_unit === 'days') ? 'days' : 'hours';
                            option.value = hours;
                            option.dataset.price = price.price;
                            option.dataset.unit = unit;
                            option.dataset.displayAmount = unit === 'days' ? (hours / 24) : hours;
                            option.textContent = formatPricingDurationOptionLabel(hours, price.price, unit);
                            if (hours == currentHours || hours == currentDuration) {
                        ...
                        if (!durationFound && currentHours > 0) {
                            const customOption = document.createElement('option');
                            customOption.value = currentHours;
                            const customUnit = (currentHours >= 24 && currentHours % 24 === 0
                                && booking?.rates?.some(r =>
                                    parseFloat(r.duration_hours) === currentHours
                                    && (r.unit === 'days' || r.duration_unit === 'days')
                                )) ? 'days' : 'hours';
                            customOption.dataset.unit = customUnit;
                            customOption.dataset.price = booking?.room_price || '';
                            customOption.textContent = formatPricingDurationOptionLabel(
                                currentHours,
                    ...
                    } else if (currentHours > 0 || (currentDuration != 0 && currentDuration != '0')) {
                        const hours = currentHours || parseFloat(currentDuration) || 0;
                        const option = document.createElement('option');
                        option.value = hours;
                        option.dataset.unit = 'hours';
                        option.textContent = hours ? `${hours} hours (Custom)` : 'No pricing available';
                        option.selected = true;
                        durationSelect.appendChild(option);
                    }
                    ...
                    if (currentDuration == 0 || currentDuration == '0' || currentHours === 0) {
                        if (![...durationSelect.options].some(o => o.selected && o.value !== '0')) {
                            optionPromo.selected = true;
                        }
                    }
                    durationSelect.appendChild(optionPromo);

                    // Payment History duration selects copy from modal — refresh after async load
                    syncPayHistDurationSelectsFromModal(currentHours);
                })
                .catch(error => {
        ...
        function handleGuestTypeChange() {
            ...
            // Re-generate fields using current booking data
            if (window.currentBooking) {
                generateModalGuestNameFields(window.currentBooking);
            }
        }

        // ── Guest Name Field Generators ────────────────────────────────────────
        function _modalGuestNameCapacity(booking) {
            const guestType = document.getElementById('modalGuestType')?.value || booking?.guest_type || 'Solo';
            if (guestType === 'Solo' || guestType === 'Company') return 1;
            if (guestType === 'Duo') return 2;
            // For Family/Group use saved names count (at least 1, up to all names)
            const allBaseNames = _parseModalBaseGuestNames(booking);
            return Math.max(1, allBaseNames.length);
        }

        function _parseModalBaseGuestNames(booking) {
            // Returns array: [guest_name, ...second_guest_name split by |]
            const names = [];
            const primary = (booking?.guest_name || booking?.guest_names || '').trim();
            if (primary) names.push(primary);
            const secondary = (booking?.second_guest_name || '').trim();
            if (secondary) {
                secondary.split('|').forEach(n => { const t = n.trim(); if (t) names.push(t); });
            }
            return names;
        }

        function generateModalGuestNameFields(booking) {
            const container = document.getElementById('modalGuestNamesContainer');
            if (!container) return;

            const guestType = document.getElementById('modalGuestType')?.value || booking?.guest_type || 'Solo';
            const isCompany = guestType === 'Company';
            const capacity = _modalGuestNameCapacity(booking);
            const savedNames = _parseModalBaseGuestNames(booking);
            ...
            for (let i = 0; i < capacity; i++) {
                ...
                if (capacity > 1) {
                    ...
                    lbl.style.cssText = 'font-size:11px;font-weight:600;color:#6b7280;margin-bottom:2px;display:block;';
                    lbl.textContent = `Guest Name ${i + 1}`;
                    wrapper.appendChild(lbl);
                }

                const input = document.createElement('input');
                input.type = 'text';
                input.id = `modalGuestName_${i}`;
                input.className = 'detail-input modal-guest-name-input';
                ...
                if (isCompany) {
                    ...
                    input.placeholder = `Enter Guest Name ${i + 1}`;
                }
                input.value = (savedNames[i] || '').toUpperCase();
        ...
        function generateModalAdditionalGuestNameFields(booking) {
            const container = document.getElementById('modalAdditionalGuestNamesContainer');
            if (!container) return;

            const additionalCount = parseInt(booking?.additional_guest || 0, 10);
            const savedNames = (booking?.additional_guest_names || '')
                .split('|').map(n => n.trim()).filter(Boolean);

            container.innerHTML = '';
            for (let i = 0; i < additionalCount; i++) {
                const wrapper = document.createElement('div');
                wrapper.style.cssText = 'display:flex;flex-direction:column;gap:4px;width:100%;margin-top:6px;';

                const lbl = document.createElement('span');
                lbl.className = 'detail-label';
                lbl.style.cssText = 'font-size:11px;font-weight:600;color:#6b7280;margin-bottom:2px;display:block;';
                lbl.textContent = `Additional Guest Name ${i + 1}`;

                const input = document.createElement('input');
                input.type = 'text';
                input.id = `modalAddGuestName_${i}`;
                input.className = 'detail-input modal-add-guest-name-input';
                input.style.width = '100%';
                input.placeholder = `Enter Additional Guest Name ${i + 1}`;
                input.value = (savedNames[i] || '').toUpperCase();
                input.addEventListener('input', function () {
                    const pos = this.selectionStart;
                    this.value = this.value.toUpperCase();
        ...
        function collectModalGuestNames() {
            // Base guest names (Guest Name, Guest Name 2, ...)
            const baseInputs = document.querySelectorAll('#modalGuestNamesContainer .modal-guest-name-input');
            const baseNames = Array.from(baseInputs).map(i => (i.value || '').trim()).filter(Boolean);
            const guestName = baseNames[0] || '';
            const secondGuestName = baseNames.length > 1 ? baseNames.slice(1).join(' | ') : '';
            // Combined for guest_names field
            const guestNames = baseNames.join(' | ');

            // Additional guest names
            const addInputs = document.querySelectorAll('#modalAdditionalGuestNamesContainer .modal-add-guest-name-input');
            const additionalGuestNames = Array.from(addInputs)
                .map(i => (i.value || '').trim()).filter(Boolean).join(' | ');

            return { guestName, secondGuestName, guestNames, additionalGuestNames };
        }
        ...
        function updateBooking() {
            ...
            const updatedData = {
                ...
                ...(() => {
                    const gn = collectModalGuestNames();
                    return {
                        guest_names: gn.guestNames,
                        guest_name: gn.guestName,
                        second_guest_name: gn.secondGuestName,
                        additional_guest_names_from_modal: gn.additionalGuestNames
                    };
                })(),
                ...
                checked_out_at: document.getElementById('modalCheckedOutAt').value,
                payment_method: '',
            };
            ...
            // Missing Items Checklist + Penalty Fine Rules (must run before new_deposit / slipper_status)
            const missingPenaltyData = collectPayHistMissingPenalty();
            Object.assign(updatedData, missingPenaltyData);
            if (window.currentBooking && missingPenaltyData.slipper_status) {
                window.currentBooking.slipper_status = missingPenaltyData.slipper_status;
                window.currentBooking.missing_items_fees = missingPenaltyData.missing_items_fees;
                window.currentBooking.missing_items_list = missingPenaltyData.missing_items_list;
                window.currentBooking.penalty_amount = missingPenaltyData.penalty_amount;
                window.currentBooking.penalty_list = missingPenaltyData.penalty_list;
                window.currentBooking.additional_fees_status = missingPenaltyData.additional_fees_status;
            }

            // Per-payment additionals (guest, pet, slippers, food/item)
            ...
            Object.assign(updatedData, {
                ...
                additional_slippers: addData.additional_slippers,
                slipper_status: missingPenaltyData.slipper_status || (window.currentBooking && window.currentBooking.slipper_status) || 'added',
            ...
            if (document.getElementById('modalAdditionalSlippers')) {
                document.getElementById('modalAdditionalSlippers').value = addData.additional_slippers;
            }
            ...
            // Additional Slippers → automatic ₱100 each as new_deposit (no separate deposit method UI)
            const newDepositData = collectNewDepositData();
            Object.assign(updatedData, newDepositData);
            ...
            // Per-payment discount details (overrides modal SCW/PWD discount fields)
            ...
            // LPG / VIP / Long Term discounts from 1st payment Discount Details
            const lgpAmount = parseFloat(document.getElementById('payHist_lpg_amount_0')?.value) || 0;
            const lgpApprover = (document.getElementById('payHist_lpg_approver_0')?.value || '').trim();
            const vipAmount = parseFloat(document.getElementById('payHist_vip_amount_0')?.value) || 0;
            const vipApprover = (document.getElementById('payHist_vip_approver_0')?.value || '').trim();
            const longAmount = parseFloat(document.getElementById('payHist_long_amount_0')?.value) || 0;
            const longPercent = parseFloat(document.getElementById('payHist_long_percent_0')?.value) || 0;
            updatedData.lgp_discount_amount = lgpAmount;
            updatedData.lgp_approver_name = lgpApprover;
            updatedData.lgp_discount_enabled = lgpAmount > 0 ? 1 : 0;
            updatedData.vip_discount_amount = vipAmount;
            updatedData.vip_approver_name = vipApprover;
            updatedData.vip_discount_enabled = vipAmount > 0 ? 1 : 0;
            updatedData.long_discount_amount = longAmount;
            updatedData.long_discount_percent = longPercent;
            updatedData.long_discount_enabled = longAmount > 0 || longPercent > 0 ? 1 : 0;
                    ...
                    .then(data => {
                        ...
                        if (data.success && data.pricing && data.pricing.length > 0) {
                            data.pricing.forEach(price => {
                                ...
                                if (unit === 'days') {
                                    const days = hours / 24;
                                    option.textContent = `${days} Day${days !== 1 ? 's' : ''} - ₱${Number(price.price).toFixed(2)}`;
                                } else {
                                    option.textContent = `${hours} hours - ₱${Number(price.price).toFixed(2)}`;
                                }
                                durationSelect.appendChild(option);
                            });
                            ...
                            if (existingRate > 0) {
                                ...
                                if (rateMatchedHours === 0 && existingTotalHours > 0) {
                                    const customOption = document.createElement('option');
                                    const customUnit = (existingTotalHours >= 24 && existingTotalHours % 24 === 0) ? 'days' : 'hours';
                                    customOption.value = existingTotalHours;
                                    customOption.dataset.price = existingRate;
                                    customOption.dataset.unit = customUnit;
                                    if (customUnit === 'days') {
                                        const days = existingTotalHours / 24;
                                        customOption.textContent = `${days} Day${days !== 1 ? 's' : ''} - \u20b1${existingRate.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (Extended)`;
                                    } else {
    ...
    <div id="extendDurationModal" class="modal-overlay" style="display:none;">
        <div class="extend-modal-container">
            ...
            <div class="extend-modal-body">
                ...
                <div id="extendAdditionalGuestSection"
                    style="display: none; margin-top: 15px; border-top: 1px solid #e5e7eb; padding-top: 15px; margin-bottom: 10px; flex-direction: column; gap: 10px;">
                </div>
            </div>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="detail-row">
                    <div class="detail-item" style="grid-column: 1 / -1;">
                        ...
                        <!-- Individual guest name fields generated dynamically -->
                        <div id="modalGuestNamesContainer" style="display:flex;flex-direction:column;gap:6px;"></div>
                        <!-- Additional guest name fields -->
                        <div id="modalAdditionalGuestNamesContainer"
                ...
                <div class="detail-row">
                    ...
                    <div class="detail-item">
                        <span class="detail-label">Check-Out (RAW)</span>
                        <input type="datetime-local" class="detail-input" id="modalCheckedOutAt">
                    </div>
                </div>
                ...
                <input type="hidden" id="modalAdditionalSlippers" value="">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="modalCancelCheckoutBtn" class="modal-btn"
                    style="background: #e65100; color: white; display: none;"
                    onclick="triggerModalCancelCheckout()">Cancel Check-Out</button>
