...
logActivity($conn, 'report', 'REPORT_VIEW_PDF',
    "Payment Type Raw PDF viewed by {$_exp_user} for {$startDate} to {$endDate}",
...
ob_start();
include 'export_payment_type_raw_pdf_html_backup.php';
...
$pdf->Cell(0, 10, 'PAYMENT TYPE REPORT', 0, 1, 'C');
...
$w = [28, 32, 60, 18, 22, 30, 24]; // Column widths
$totalW = array_sum($w);
...
$pdf->Cell($w[1], 7, 'Date', 1, 0, 'C', true);
...
if (empty($dataRows)) {
    ...
    foreach ($dataRows as $row) {
        ...
        $pdf->SetXY($currentX, $startY);
        $pdf->Cell($w[1], $cellHeight, $row['date'], 1, 0, 'C');
        $currentX += $w[1];
...
// Output PDF
$pdf->Output('I', 'Payment_Type_Raw_Report_' . $startDate . '_to_' . $endDate . '.pdf');
?>
