                ...
                <?php else: ?>
                    ...
                    foreach ($records as $index => $row) {
                        ...
                        if (strtolower(trim($guestType)) === 'company') {
                            ...
                            // Use consolidated guest name instead of just guest_name
                            $displayNames[$index] = !empty($row['consolidated_guest_name']) ? $row['consolidated_guest_name'] : ($row['guest_name'] ?? '—');
                        }
                    }
                    ...
                    usort($records, function ($a, $b) use ($guestBookingCounts) {
                        ...
                        if (strtolower(trim($guestTypeA)) === 'company') {
                            ...
                            $displayNameA = !empty($a['consolidated_guest_name']) ? $a['consolidated_guest_name'] : ($a['guest_name'] ?? '');
                        }
                        ...
                        if (strtolower(trim($guestTypeB)) === 'company') {
                            ...
                            $displayNameB = !empty($b['consolidated_guest_name']) ? $b['consolidated_guest_name'] : ($b['guest_name'] ?? '');
                        }
                    ...
                    foreach ($records as $row):
                        ...
                        // Calculate actual amount paid using payment history (mirrors export_payment_type_raw_pdf.php)
                        // History columns store each individual payment as pipe-separated values.
                        // Summing them gives the true total; fall back to deposit/downpayment when history is absent.
                        $historyMethodMap = [
                        ...
                        foreach ($historyMethodMap as $histCol => [$depCol, $downCol]) {
                            $histRaw = $row[$histCol] ?? '';
                            if (!empty($histRaw)) {
                                // Sum every pipe-separated entry in the history column
                                foreach (explode('|', (string) $histRaw) as $seg) {
                                    $totalAmt += floatval(trim($seg));
                                }
                            } else {
                                // No history: use the larger of deposit vs downpayment (legacy fallback)
                        ...
                        $companyName = '—';
                        $displayGuestName = !empty($row['consolidated_guest_name']) ? $row['consolidated_guest_name'] : ($row['guest_name'] ?? '—');
                        $guestType = $row['guest_type'] ?? '';
                        ...
                        <tr>
                            ...
                            <td><?php echo formatDateTime($row['checked_out_at'] ?? '—'); ?></td>
