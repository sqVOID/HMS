...
try {
    ...
    $stmt->execute();
    logActivity($conn, 'update', 'PROMO_UPDATE',
        "Promo #{$promoId} '{$title}' updated by {$_actor} — 12h: ₱" . number_format($price12, 2) . ", 24h: ₱" . number_format($price24, 2) . ($replacementImagePath !== '' ? ' (with new image)' : ''),
        [
            'promo_id'     => $promoId,
            'title'        => $title,
            'price_12hrs'  => $price12,
            ...
            'room_size'    => $roomSize,
            'bed_type'     => $bedType,
            'image_update' => ($replacementImagePath !== ''),
            'updated_by'   => $_actor
        ], $promoId
    );
