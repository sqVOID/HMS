    ...
    try {
        ...
        if ($update_image) {
            ...
            if ($stmt->execute()) {
                ...
                logActivity($conn, 'menu', 'UPDATE', "Breakfast item '{$food_name}' updated with new image (Price: ₱" . number_format($price, 2) . ") by {$encoder}", [
                    'food_id'       => $id,
                    'food_name'     => $food_name,
                    'price'         => $price,
            ...
            if ($stmt->execute()) {
                $response['success'] = true;
                $response['message'] = 'Food item updated successfully!';
                logActivity($conn, 'menu', 'UPDATE', "Breakfast item '{$food_name}' updated (Price: ₱" . number_format($price, 2) . ") by {$encoder}", [
                    'food_id'     => $id,
                    'food_name'   => $food_name,
                    'price'       => $price,
                    'description' => $description,
                ...
                $response['message'] = 'Failed to update food item!';
                logError($conn, 'menu', 'UPDATE', "Failed to update breakfast item '{$food_name}'", [
                    'food_id'   => $id,
                    'food_name' => $food_name
                ], $id);
            }
        ...
        logError($conn, 'menu', 'UPDATE', "Error updating breakfast item '{$food_name}': " . $e->getMessage(), [
            'food_id' => $id,
            'error'   => $e->getMessage()
        ], $id);
    }
