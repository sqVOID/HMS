...
try {
    ...
    // Ensure LGP/VIP/Long Term discount columns exist on bookings + reports
    $extraDiscountCols = [
    ...
    foreach (['bookings', 'reports'] as $table) {
        foreach ($extraDiscountCols as $col => $def) {
            try {
                ...
                // ignore missing table / already exists
            }
        }
    }
    ...
    $query = "SELECT 
                ...
                CAST(COALESCE(second_guest_name, '') AS CHAR) as second_guest_name,
                CAST(COALESCE(additional_guest_names, '') AS CHAR) as additional_guest_names,
                ...
                CAST('' AS CHAR) as checked_out_at,
                ...
                CAST(COALESCE(additional_slippers, 0) AS CHAR) as additional_slippers,
                CAST(COALESCE(slipper_status, 'added') AS CHAR) as slipper_status,
                ...
                CAST(COALESCE(new_deposit, 0) AS CHAR) as new_deposit,
                CAST(COALESCE(new_deposit_cash, 0) AS CHAR) as new_deposit_cash,
                CAST(COALESCE(new_deposit_g_cash, 0) AS CHAR) as new_deposit_g_cash,
                CAST(COALESCE(new_deposit_maya, 0) AS CHAR) as new_deposit_maya,
                CAST(COALESCE(new_deposit_instapay, 0) AS CHAR) as new_deposit_instapay,
                CAST(COALESCE(new_deposit_online_banking, 0) AS CHAR) as new_deposit_online_banking,
                CAST(COALESCE(new_deposit_airbnb, 0) AS CHAR) as new_deposit_airbnb,
                CAST(COALESCE(new_deposit_details, '') AS CHAR) as new_deposit_details,
                CAST(COALESCE(new_deposit_gcash_ref, '') AS CHAR) as new_deposit_gcash_ref,
                CAST(COALESCE(new_deposit_maya_ref, '') AS CHAR) as new_deposit_maya_ref,
                CAST(COALESCE(new_deposit_instapay_ref, '') AS CHAR) as new_deposit_instapay_ref,
                CAST(COALESCE(new_deposit_online_banking_ref, '') AS CHAR) as new_deposit_online_banking_ref,
                CAST(COALESCE(new_deposit_airbnb_ref, '') AS CHAR) as new_deposit_airbnb_ref,
                ...
                CAST(COALESCE(lgp_discount_enabled, 0) AS CHAR) as lgp_discount_enabled,
                CAST(COALESCE(lgp_discount_amount, 0) AS CHAR) as lgp_discount_amount,
                CAST(COALESCE(lgp_approver_name, '') AS CHAR) as lgp_approver_name,
                CAST(COALESCE(vip_discount_enabled, 0) AS CHAR) as vip_discount_enabled,
                CAST(COALESCE(vip_discount_amount, 0) AS CHAR) as vip_discount_amount,
                CAST(COALESCE(vip_approver_name, '') AS CHAR) as vip_approver_name,
                CAST(COALESCE(long_discount_enabled, 0) AS CHAR) as long_discount_enabled,
                CAST(COALESCE(long_discount_amount, 0) AS CHAR) as long_discount_amount,
                CAST(COALESCE(long_discount_percent, 0) AS CHAR) as long_discount_percent,
                ...
                CAST(COALESCE(missing_items_fees, 0) AS CHAR) as missing_items_fees,
                CAST(COALESCE(missing_items_list, '') AS CHAR) as missing_items_list,
                CAST(COALESCE(penalty_amount, 0) AS CHAR) as penalty_amount,
                CAST(COALESCE(penalty_list, '') AS CHAR) as penalty_list,
                CAST(COALESCE(additional_fees_status, 'None') AS CHAR) as additional_fees_status,
              ...
              SELECT 
                ...
                CAST(COALESCE(room_price, 0) AS CHAR) as room_price,
                CAST(check_out AS CHAR) as check_out,
                CAST(COALESCE(checked_out_at, '') AS CHAR) as checked_out_at,
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (isset($row['additional_slippers']) && $row['additional_slippers'] > 0) {
            $addList[] = $row['additional_slippers'] . ' Slipper(s)';
        }
        ...
        if ($finalMayaAmount > 0)
            $paymentMethods[] = 'Maya';
        if ($finalInstapayAmount > 0)
            $paymentMethods[] = 'Instapay';
        if ($finalOnlineBankingAmount > 0)
            $paymentMethods[] = 'Online Banking';
        if ($finalAirbnbAmount > 0)
        ...
        $modifications[] = [
            ...
            'second_guest_name' => $row['second_guest_name'] ?? '',
            'additional_guest_names' => $row['additional_guest_names'] ?? '',
            ...
            'additional_slippers' => $row['additional_slippers'] ?? '0',
            'slipper_status' => $row['slipper_status'] ?? 'added',
            ...
            'new_deposit' => $row['new_deposit'] ?? '0',
            'new_deposit_cash' => $row['new_deposit_cash'] ?? '0',
            'new_deposit_g_cash' => $row['new_deposit_g_cash'] ?? '0',
            'new_deposit_maya' => $row['new_deposit_maya'] ?? '0',
            'new_deposit_instapay' => $row['new_deposit_instapay'] ?? '0',
            'new_deposit_online_banking' => $row['new_deposit_online_banking'] ?? '0',
            'new_deposit_airbnb' => $row['new_deposit_airbnb'] ?? '0',
            'new_deposit_details' => $row['new_deposit_details'] ?? '',
            'new_deposit_gcash_ref' => $row['new_deposit_gcash_ref'] ?? '',
            'new_deposit_maya_ref' => $row['new_deposit_maya_ref'] ?? '',
            'new_deposit_instapay_ref' => $row['new_deposit_instapay_ref'] ?? '',
            'new_deposit_online_banking_ref' => $row['new_deposit_online_banking_ref'] ?? '',
            'new_deposit_airbnb_ref' => $row['new_deposit_airbnb_ref'] ?? '',
            ...
            'lgp_discount_enabled' => $row['lgp_discount_enabled'] ?? '0',
            'lgp_discount_amount' => $row['lgp_discount_amount'] ?? '0',
            'lgp_approver_name' => $row['lgp_approver_name'] ?? '',
            'vip_discount_enabled' => $row['vip_discount_enabled'] ?? '0',
            'vip_discount_amount' => $row['vip_discount_amount'] ?? '0',
            'vip_approver_name' => $row['vip_approver_name'] ?? '',
            'long_discount_enabled' => $row['long_discount_enabled'] ?? '0',
            'long_discount_amount' => $row['long_discount_amount'] ?? '0',
            'long_discount_percent' => $row['long_discount_percent'] ?? '0',
            ...
            'missing_items_fees' => $row['missing_items_fees'] ?? '0',
            'missing_items_list' => $row['missing_items_list'] ?? '',
            'penalty_amount' => $row['penalty_amount'] ?? '0',
            'penalty_list' => $row['penalty_list'] ?? '',
            'additional_fees_status' => $row['additional_fees_status'] ?? 'None',
    ...
    // Attach room duration rates (hours/days unit) so Modification duration display matches Booking
    $durationsMap = [];
    $roomIdToDbId = [];
    try {
        try {
        ...
        while ($r = $roomStmt->fetch(PDO::FETCH_ASSOC)) {
            if (!empty($r['room_id'])) {
                $roomIdToDbId[(string) $r['room_id']] = $r['id'];
            }
        }
    ...
    foreach ($modifications as &$mod) {
        $roomKey = isset($mod['room_id']) ? (string) $mod['room_id'] : '';
        $dbId = $roomIdToDbId[$roomKey] ?? null;
        $mod['room_db_id'] = $dbId;
        $mod['rates'] = ($dbId !== null && isset($durationsMap[$dbId])) ? $durationsMap[$dbId] : [];
    }
    unset($mod);
