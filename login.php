        ...
        try {
            ...
            if (!$user || !isset($user['id']) || empty($user['id'])) {
                ...
                if ($inputPassword === '' || $dbPassword === '') {
                    $error = 'Invalid username or password.';
                    logWarning($conn, 'auth', 'LOGIN_FAILED', "Failed login attempt for username '{$username}' (empty password input)", ['username' => $username]);
                } elseif (strlen($inputPassword) !== strlen($dbPassword)) {
                    // Different lengths - definitely wrong
                    $error = 'Invalid username or password.';
                    logWarning($conn, 'auth', 'LOGIN_FAILED', "Failed login attempt for username '{$username}' (password mismatch)", ['username' => $username]);
                    ...
                    if ($accountStatus === 'Inactive') {
                        ...
                        logWarning($conn, 'auth', 'LOGIN_BLOCKED', "Deactivated user '{$username}' attempted login", ['username' => $username], $user['id']);
                    } else {
                        ...
                        logActivity($conn, 'auth', 'LOGIN', "User '{$user['username']}' logged in successfully ({$user['access_level']})", [
                            'user_id'      => $user['id'],
